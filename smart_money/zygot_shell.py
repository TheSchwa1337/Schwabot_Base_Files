...
def compute_stability_index(Z: float, N: float, params: Optional[Dict[str, float]] = None) -> float:
    ...
    stability_index = B / A
    logger.debug(f"Computed ZygotShell Stability Index: {stability_index:.4f} (Z={Z}, N={N})")
...
def check_shell_trade_signal(volume_signal: float, volatility_map: float, config: Optional[Dict[str, Any]] = None) -> str:
    ...
    if stability_index > config['stability_threshold_high']:
        logger.info(f"ZygotShell: ENTER_AGGRESSIVE (Stability Index: {stability_index:.2f})")
        return "ENTER_AGGRESSIVE"
    elif stability_index > config['stability_threshold_low']:
        logger.info(f"ZygotShell: ENTER_SCALED (Stability Index: {stability_index:.2f})")
        return "ENTER_SCALED"
    else:
...
if __name__ == "__main__":
    ...
    custom_params = {'alpha_v': 16.0, 'alpha_s': 18.0, 'alpha_c': 0.7, 'alpha_a': 23.0, 'alpha_p': 12.5}
    custom_config = {'stability_threshold_high': 7.6, 'stability_threshold_low': 6.1, 'params': custom_params}
