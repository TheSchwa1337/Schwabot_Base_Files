...
class EntropyTradingPipeline:
    ...
    def generate_mock_order_book(self) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]]]:
    ...
    def process_tick_cycle(self):
        ...
        try:
            ...
            logger.info(f"Tick Cycle - Entropy: {entropy_signal.entropy_value:.6f}, "
    ...
    def process_routing_cycle(self):
        ...
        try:
            ...
            logger.info(f"Routing Cycle - State: {current_state.get('current_entropy_state', 'UNKNOWN')}, "
    ...
    def _make_trading_decision(self, entropy_signal):
        ...
        try:
            ...
            routing_config = self.entropy_integrator.config.get("timing_cycles", {}).get("routing_cycle", {}).get("routing_decisions", {})
        ...
        try:
            ...
            if action_type == "Buy":
                if position_size <= self.available_balance:
                    ...
                    logger.info(f"BUY executed: {btc_amount:.6f} BTC at ${self.btc_price:.2f}, "
                ...
                if btc_amount <= self.current_position:
                    ...
                    logger.info(f"SELL executed: {btc_amount:.6f} BTC at ${self.btc_price:.2f}, "
                              f"Confidence: {confidence:.3f}, Entropy: {action.get('entropy_value', 0):.6f}")
    ...
    def _update_performance_metrics(self):
        ...
        try:
            ...
            if self.total_trades % 10 == 0 and self.total_trades > 0:
                ...
                logger.info(f"Performance Update - Trades: {self.total_trades}, "
