...
from core.advanced_drift_shell_integration import ShiftPatternEngine, AdvancedDriftShellIntegration
...
class ShiftPatternDemo:
    ...
    def demo_thermal_shift_logic(self):
        ...
        ax4.scatter(volume_emas, thermal_pressures, c=self.volatilities, cmap='viridis', alpha=0.6)
        ...
        safe_print(f"Pressure range: [{np.min(thermal_pressures):.4f}, {np.max(thermal_pressures):.4f}]")
    ...
    def demo_entropy_coherence_shifts(self):
        ...
        ax2.axhline(y=-self.engine.coherence_threshold, color='red', linestyle='--', 
        ...
        ax3.scatter(trigger_indices, [1] * len(trigger_indices), c='red', s=100, alpha=0.7)
    ...
    def demo_api_penalty_decay(self):
        ...
        # Error counts
        ax1.plot(self.tick_counts, self.error_counts, 'red', linewidth=2, marker='o', markersize=3)
        ax1.set_ylabel('Error Count')
        ax1.set_title('API Error Counts Over Time')
        ax1.grid(True, alpha=0.3)
        
        # Confidence evolution
        ax2.plot(self.tick_counts, confidences, 'blue', linewidth=2, label='Current Confidence')
        ax2.plot(self.tick_counts, penalized_confidences, 'orange', linewidth=2, label='Penalized Confidence')
        ...
        ax3.scatter(self.error_counts, penalized_confidences, c=self.tick_counts, cmap='viridis', alpha=0.6)
        ...
        safe_print(f"Total confidence loss: {initial_confidence - penalized_confidences[-1]:.4f}")
    ...
    def demo_time_lock_phase_drift(self):
        ...
        for i in range(len(self.tick_counts)):
            ...
            drift_magnitude, drift_direction = self.engine.compute_time_lock_phase_drift(
        ...
        # Individual phases
        short_phases = [self.engine.compute_ferris_wheel_phase(i, period=16) for i in self.tick_counts]
        mid_phases = [self.engine.compute_ferris_wheel_phase(i, period=72) for i in self.tick_counts]
        long_phases = [self.engine.compute_ferris_wheel_phase(i, period=672) for i in self.tick_counts]
        ...
        ax2.axhline(y=0.5, color='red', linestyle='--', alpha=0.7, label='Sync Threshold')
        ...
        # Drift direction
        ax3.plot(self.tick_counts, drift_directions, 'orange', linewidth=2, marker='o', markersize=3)
        ...
        trigger_indices = [i for i, trigger in enumerate(synchronization_triggers) if trigger]
        ax4.scatter(trigger_indices, [1] * len(trigger_indices), c='red', s=100, alpha=0.7)
        ...
        safe_print(f"Drift direction distribution: {np.bincount(np.array(drift_directions) + 1)}")
    ...
    def demo_asset_specific_durations(self):
        ...
        for i in range(len(assets)):
            for j in range(len(shift_types)):
                text = ax.text(j, i, duration_matrix[i][j], ha="center", va="center", 
    ...
    def run_comprehensive_demo(self):
        ...
        success("✅ Comprehensive demo completed! Check the generated PNG files for visualizations.")
        
        # Print summary statistics
        safe_print("\n📈 Demo Summary Statistics:")
        safe_print(f"  Total time steps: {self.time_steps}")
        safe_print(f"  Price range: [{np.min(self.prices):.2f}, {np.max(self.prices):.2f}]")
        safe_print(f"  Volume range: [{np.min(self.volumes):.0f}, {np.max(self.volumes):.0f}]")
