...
class TensorIntegrationDemo:
    ...
    def __init__(self):
        ...
        # Demo data
        self.btc_prices = [
            45678.90, 46234.15, 47890.23, 46123.78, 48900.45,
            49567.89, 47234.56, 50123.67, 48890.12, 51234.78
        ]
    ...
    async def run_basic_tensor_analysis(self):
        ...
        for i, btc_price in enumerate(self.btc_prices):
            ...
            print(f"  🌊 Entropy Variation: {result.sp_integration['entropy_variation']:.6f}")
    ...
    def _analyze_trading_signals(self, result):
        ...
        if quantum_score > 0.3 and phase_bucket == "ascent" and gut_stability > 0.995:
            print("  🟢 SIGNAL: Strong BUY (Quantum alignment + Ascent phase + High stability)")
    ...
    async def run_strategy_integration_demo(self):
        ...
        try:
            ...
            from core.strategy_logic import StrategyLogic, StrategyConfig, StrategyType, TradingSignal, SignalType, SignalStrength
    ...
    def _generate_tensor_signal(self, tensor_result, btc_price) -> Any:
        ...
        if quantum_score > 0.3 and phase_bucket in ["ascent", "peak"]:
            ...
            strength = SignalStrength.STRONG if quantum_score > 0.5 else SignalStrength.MODERATE
            confidence = min(0.95, 0.6 + abs(quantum_score))
        elif quantum_score < -0.2 and phase_bucket in ["descent", "trough"]:
            signal_type = SignalType.SELL
            strength = SignalStrength.STRONG if quantum_score < -0.4 else SignalStrength.MODERATE
            confidence = min(0.95, 0.6 + abs(quantum_score))
        else:
    ...
    async def run_performance_analysis(self):
        ...
        for i, entry in enumerate(history[-5:]):
            print(f"  {i+1}. BTC ${entry['btc_price']:.0f} → "
