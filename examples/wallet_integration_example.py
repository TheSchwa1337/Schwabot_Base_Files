...
def demonstrate_portfolio_integration():
    ...
    if recent_signals:
        for signal in recent_signals:
            print(f"   {signal['timestamp']}: {signal['action']} {signal['quantity']:.4f} {signal['asset']} @ ${signal['price']:.2f} (confidence: {signal['confidence']:.2f}, source: {signal['source']})")
    else:
...
def demonstrate_api_operations(wallet):
    ...
    for exchange_name, exchange_balances in balances.items():
        ...
        if exchange_balances:
            for asset, balance_data in exchange_balances.items():
                print(f"     {asset}: Free={balance_data['free']:.4f}, Used={balance_data['used']:.4f}, Total={balance_data['total']:.4f}")
        else:
    ...
    for balance_key, balance_obj in wallet.exchange_balances.items():
        print(f"   {balance_key}: {balance_obj.asset.value} - Free: {balance_obj.free:.4f}, Total: {balance_obj.total:.4f}")
