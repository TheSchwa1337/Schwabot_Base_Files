...
class TradingExecutionEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.max_daily_loss = self.config.get('max_daily_loss', 0.5)  # 5% daily loss limit
        self.max_drawdown = self.config.get('max_drawdown', 0.15)  # 15% max drawdown
        self.stop_loss_pct = self.config.get('stop_loss_pct', 0.2)  # 2% stop loss
        self.take_profit_pct = self.config.get('take_profit_pct', 0.4)  # 4% take profit
