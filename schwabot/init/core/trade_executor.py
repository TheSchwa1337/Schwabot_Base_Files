...
class TradeExecutor:
    ...
    def execute(self, side: str = "buy") -> None:
        ...
        if self.mode == "dry":
            ...
            print(f"[TradeExecutor] 🟣 DRY RUN: ENTRY {side.upper()} {self.amount} {self.symbol} @ {price} (cost ~{cost:.2f})")
            ...
            if hasattr(self, 'exit_strategy') and self.exit_strategy:
                ...
                print(f"[TradeExecutor] 🟣 DRY RUN: EXIT {opp.upper()} {self.amount} {self.symbol} @ {exit_price} after {exit_time}s")
            return
        ...
        if hasattr(self, 'exit_strategy') and self.exit_strategy:
            ...
            while True:
                ...
                if elapsed >= exit_time:
                    print(f"[TradeExecutor] ⚠️  EXIT TTL reached ({elapsed:.0f}s). Placing limit exit to preserve profit.")
