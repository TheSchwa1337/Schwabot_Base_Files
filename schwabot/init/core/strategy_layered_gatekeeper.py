...
from .vector_band_gatekeeper import (
    confirm_short_drift,
    confirm_mid_vector, 
    confirm_long_trend
)
...
class StrategyLayeredGatekeeper:
    ...
    def evaluate_all_gates(self, tick_blob: str) -> Tuple[bool, str, float]:
        ...
        if indicator_score >= 0.66:
            ...
            # No clear signal
            return False, f"Weak signals (TA: {indicator_score:.2f}, Override: {override_confidence:.2f})", 0.0
    ...
    def record_profitable_trade(self, 
                              tick_blob: str,
                              entry_price: float,
                              exit_price: float,
                              time_to_exit: int,
        ...
        self.profit_registry.add_profitable_trade(
            tick_blob, entry_price, exit_price, time_to_exit, strategy_id
        ) 