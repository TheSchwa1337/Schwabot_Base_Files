...
class ProfitOracle:
    ...
    def forecast_profit(self, market_data: Dict[str, Any]) -> Optional[ProfitForecast]:
        ...
        try:
            ...
            confidence = self.unified_math.min(
                1.0,
            ...
            risk_adjusted_return = self.unified_math.divide(
                predicted_profit, self.unified_math.add(1.0, risk)
            )

            # Optimal entry/exit prices (simplified)
            optimal_entry = self.unified_math.subtract(
                current_price, self.unified_math.multiply(current_price, 0.001)
            )
            optimal_exit = self.unified_math.add(
                current_price,
                self.unified_math.multiply(current_price, predicted_profit * 0.0001),
            )
            ...
            logger.info(
                f"📈 Profit forecast generated: {forecast.predicted_profit_usd:.2f} USD with {forecast.confidence_score:.2f} confidence."
            )
            return forecast
...
if __name__ == "__main__":
    ...
    try:
        ...
        for f in oracle.get_historical_forecasts(5):
            print(
                f"  [{datetime.fromtimestamp(f.timestamp).strftime('%Y-%m-%d %H:%M')}] Profit: {f.predicted_profit_usd:.2f}, Conf: {f.confidence_score:.2f}"
            )
