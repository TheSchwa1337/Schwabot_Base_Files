...
class VisualizationExample:
    ...
    async def setup_visualization(self):
        ...
        self.hub = await create_schwabot_hub(
            initial_capital=100000.0, debug=True, enable_visualization=True
        )
    ...
    def on_trading_event(self, event_type: str, data: Dict[str, Any]):
        ...
        if event_type == "trade_execution":
            self.logger.info(
                f"Trade executed: {data.get('symbol')} {data.get('side')} {data.get('amount')}"
            )
    ...
    def on_math_event(self, event_type: str, data: Dict[str, Any]):
        ...
        if event_type == "calculation_completed":
            self.logger.info(
                f"Math calculation: {data.get('operation')} completed in {data.get('duration', 0):.3f}s"
            )
        elif event_type == "tensor_operation":
            self.logger.info(
                f"Tensor operation: {data.get('operation')} with {data.get('flops', 0)} FLOPs"
            )
    ...
    def on_performance_event(self, metrics: Dict[str, Any]):
        ...
        self.logger.info(
            f"Performance: CPU {system_metrics.get('cpu_percent', 0):.1f}%, "
    ...
    async def simulate_trading_activity(self, duration: int = 60):
        ...
        while time.time() - start_time < duration:
            ...
            if self.hub and self.hub.trading_visualizer:
                self.hub.trading_visualizer.add_market_data(
                    symbol,
                    {
                        "price": price,
    ...
    async def run_demonstration(self, duration: int = 120):
        ...
        try:
            ...
            if "visualization" in status:
                ...
                self.logger.info(
                    f"    Total events: {viz_status['visualizer'].get('performance_metrics', {}).get('total_events', 0)}"
                )
                self.logger.info(
                    f"    Trading summary: {viz_status['trading'].get('trading_summary', {}).get('total_trades', 0)} trades"
                )
                self.logger.info(
                    f"    Math calculations: {viz_status['math'].get('calculations', {}).get('total_calculations', 0)}"
                )
