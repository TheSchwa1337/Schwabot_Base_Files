...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Schwabot Trading System with Schwafit Integration"
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run in dry-run mode (no actual trading)"
    )
    parser.add_argument(
        "--demo", action="store_true", help="Run in demo mode with simulated trading"
    )
    parser.add_argument(
        "--symbols",
        type=str,
    ...
    for cycle in range(cycle_count):
        try:
            ...
            for symbol in symbols:
                ...
                if portfolio:
                    logger.info(
                        f"💰 Portfolio Health: {portfolio.schwafit_health_score:.3f} | "
...
async def main() -> int:
    ...
    try:
        ...
        if not init_result.get("success", False):
            logger.error(
                f"Failed to initialize Schwabot: {init_result.get('error', 'Unknown error')}"
            )
            return 1
