...
class PriceFeedConfig:
    ...
    fallback_sequence: List[PriceSource] = field(
    ...
    async def get_price(self, symbol: str) -> Optional[PriceData]:
        ...
        try:
            ...
            price_data = PriceData(
                ...
                metadata={"price_change": price_change, "is_mock": True},
            )
...
class UnifiedPriceFeed:
    ...
    async def get_price(
        self, symbol: str, force_source: Optional[PriceSource] = None
    ) -> Optional[PriceData]:
            ...
            for source in sources_to_try:
                ...
                try:
                    ...
                    if price_data:
                        ...
                        logger.debug(
                            f"Price for {symbol}: ${price_data.price:.2f} from {source.value}"
                        )
                        return price_data
