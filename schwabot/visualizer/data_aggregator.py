...
class DataAggregator:
    ...
    def add_strategy_data(self, strategy_hash: str, operation: str, status: str, data: Dict[str, Any] = None):
        ...
        if len(self.strategy_data[strategy_hash]) > self.max_data_points:
            self.strategy_data[strategy_hash] = self.strategy_data[strategy_hash][-self.max_data_points:]
    ...
    def get_aggregated_trading_data(self) -> Dict[str, Any]:
        ...
        for symbol, data_points in self.trading_data.items():
            ...
            aggregated[symbol] = {
                ...
                'avg_price': sum(dp['price'] for dp in recent_data) / len(recent_data) if recent_data else 0,
    ...
    def clear_old_data(self, max_age_seconds: int = 3600):
        ...
        for data_dict in [self.math_data, self.trading_data, self.gpu_data, self.strategy_data, self.performance_data]:
            for key in list(data_dict.keys()):
                data_dict[key] = [
                    dp for dp in data_dict[key]
