...
class MathVisualizer:
    ...
    def add_calculation(
        ...
        self._update_complexity_metrics(
            operation, duration, complexity, calculation_data["input_size"]
        )
    ...
    def add_mathematical_pattern(
        self, pattern_type: str, pattern_data: Dict[str, Any], confidence: float
    ):
    ...
    def _estimate_flops(
        self, operation: str, input_shape: List[int], output_shape: List[int]
    ) -> int:
    ...
    def _update_complexity_metrics(
        self, operation: str, duration: float, complexity: str, input_size: int
    ):
    ...
    def get_calculation_summary(self) -> Dict[str, Any]:
        ...
        if not self.calculation_history:
            return {"total_calculations": 0, "avg_duration": 0, "total_operations": 0}
    ...
    def get_math_alerts(self) -> List[Dict[str, Any]]:
        ...
        recent_calculations = [
            c for c in self.calculation_history if current_time - c["timestamp"] <= 60
        ]  # Last minute
        if recent_calculations:
            slow_calculations = [
                c for c in recent_calculations if c["duration"] > 1.0
            ]  # > 1 second
        ...
        if self.enable_tensor_visualization:
            ...
            high_memory_ops = [
                op for op in recent_tensor_ops if op["memory_usage"] > 1000000000
            ]  # > 1GB
            if high_memory_ops:
                alerts.append(
                    {
