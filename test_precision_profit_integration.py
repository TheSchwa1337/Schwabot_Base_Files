...
class AdvancedBTCMarketSimulator:
    ...
    def generate_market_tick(self) -> Dict[str, Any]:
        ...
        harmonic_component = np.sin(self.cycle_position) * 0.002  # 0.2% harmonic swing
        
        # Generate trend momentum
        trend_change = np.random.normal(0, 0.05)
        self.trend_momentum = np.clip(self.trend_momentum + trend_change, -0.01, 0.01)
