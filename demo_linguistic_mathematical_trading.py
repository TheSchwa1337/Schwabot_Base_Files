...
async def linguistic_trading_demo_async(): # Changed to async function
    ...
    try:
        ...
        from core.lantern_news_intelligence_bridge import LanternNewsIntelligenceBridge, NewsCategory, NewsItem # Import news components
        
        # Initialize trading engines
        thought_engines = DualisticThoughtEngines()
        lantern_news_bridge = LanternNewsIntelligenceBridge() # Initialize LanternNewsIntelligenceBridge
        ...
        print(f"💹 Current Market (for News Context): BTC ${simulated_market_data['btc_price']:,}")
        ...
        if recent_news_items:
            for i, news_item in enumerate(recent_news_items):
                ...
                news_decision_result = thought_engines.process_news_for_decision(news_item, simulated_market_data)
                
                if news_decision_result.get('success'):
                    tv = news_decision_result['thought_vector']
                    la = news_decision_result['linguistic_analysis']
                    print(f"   🧠 News-Driven Decision: {tv['decision']} (Confidence: {tv['confidence']:.2f}, State: {tv['state']})")
                    print(f"   ⚡ News Linguistic Bit State: {la['bit_state']:02b} | Weight: {la['weight']:.3f} | Hash: {la['sha_hash'][:8]}...")
                    print(f"   🏷️ Generated Tags: {', '.join(tv['tags'])}")
                else:
                    print(f"   ❌ Failed to process news for decision: {news_decision_result['error']}")
        else:
        ...
        for scenario_num, scenario in enumerate(trading_scenarios, 1):
            ...
            for cmd_num, command in enumerate(scenario['commands'], 1):
                ...
                print(f"💹 Market: BTC ${market_data['btc_price']:,} | Vol: {market_data['volume']:,}")
                ...
                print(f"   ⚡ ASIC State: {linguistic_result['bit_state']:02b} | Entropy: {linguistic_result['entropy_overlay']:.3f}")
                print(f"   💰 Profit Delta: ${trade_vector.profit_delta:.2f}")
                print(f"   🔮 Glyph: {trade_vector.glyph_signature} | Processing: {processing_time:.1f}ms")
            ...
            print(f"🌀 Fractal Energy: {np.sum(np.abs(linguistic_engine.fractal_memory)):.1f}")
            print()
        ...
        for i, (glyph, state) in enumerate(zip(recent_glyphs[-5:], recent_states[-5:]), 1):
            bit_meaning = {
