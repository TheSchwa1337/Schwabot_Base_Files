...
class SimulatedCCXTIntegration:
    ...
    def calculate_profit_vector(self, order_book: Dict[str, Any], walls: List[Dict[str, Any]]) -> Dict[str, Any]:
...
class CompleteIntegrationDemo:
    ...
    def __init__(self, initial_capital: float = 100_000.0):
        ...
        logger.info("🚀 Complete Integration Demo initialized with capital: $%.2f", initial_capital)
    ...
    def _calculate_mathematical_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'complexity': 1.0 - matrix_analysis.get('stability_score', 0.5),  # Inverse of stability
                ...
                'eigenvalues': matrix_analysis.get('eigenvalues', np.array([1.0])).tolist()
            }
    ...
    def _analyze_market_conditions(self, price: float, volume: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate momentum
            momentum = (price - self.price_history[0]) / self.price_history[0] if self.price_history[0] > 0 else 0.0
            ...
            return {
                ...
                'price_range': (min(self.price_history[-10:]), max(self.price_history[-10:]))
            }
            ...
            if brain_confidence > 0.5:
                ...
                if profit_potential > 0.002:  # 0.2% profit potential
                    return {
                        'action': brain_action,
                        'confidence': brain_confidence,
                        'quantity': risk_metrics.get('position_size', 0.0) * 0.5,  # Reduced size
                        'price': price,
    ...
    def _execute_trade(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                ...
                action, quantity, price, profit, self.current_capital
            )
    ...
    def run_demo(self, num_ticks: int = 200) -> Dict[str, Any]:
        ...
        for i in range(num_ticks):
            ...
            base_price *= (1 + drift + volatility_factor + np.random.uniform(-0.0005, 0.0005))

            # Clamp price within the 20,000 - 50,000 range
            base_price = max(20000.0, min(50000.0, base_price))

            # Generate volume (more dynamic)
            volume = max(100, 500 + np.random.uniform(-400, 1000) * (base_price / 35000.0))
            ...
            # Log progress
            if i % 10 == 0:
                logger.info("Demo progress: %d/%d (%.1f%%)", i, num_ticks, i/num_ticks*100)

        # Calculate summary statistics
        total_trades = len([r for r in results if r.get('trade_result', {}).get('executed', False)])
        winning_trades = len([r for r in results if r.get('trade_result', {}).get('profit', 0) > 0])
        ...
        logger.info("Demo completed: %.2f%% return, %d trades, %.1f%% win rate",
                   total_return*100, total_trades, summary['win_rate']*100)
    ...
    def print_detailed_results(self, results: Dict[str, Any]):
        ...
        if strategy_perf:
            ...
            for strategy, perf in strategy_perf.items():
                if perf['total_trades'] > 0:
                    print(f"    {strategy}: {perf['success_rate']:.1%} win rate, {perf['total_trades']} trades")
        ...
        print(f"  Avg Profit:      ${brain_metrics.get('avg_profit_per_signal', 0):.4f}")
        ...
        for i, tick in enumerate(sample_ticks):
            if 'ghost_state' in tick:
                print(f"  Tick {i}: {tick['ghost_state']['branch']} "
