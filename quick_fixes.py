...
def fix_basic_formatting():
    ...
    try:
        result = subprocess.run(
            ['black', 'core', 'schwabot', 'utils', 'config', '--line-length', '100', '--target-version', 'py311'],
            capture_output=True,
            text=True
        )
...
def remove_unused_imports():
    ...
    try:
        result = subprocess.run(
            ['autoflake', '--in-place', '--remove-all-unused-imports', '--remove-unused-variables', 'core', 'schwabot', 'utils', 'config'],
            capture_output=True,
            text=True
        )
