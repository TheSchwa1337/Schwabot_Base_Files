        ...
        if line.strip().startswith('try:') and line.strip() == 'try:':
            ...
            if next_line_idx < len(lines):
                ...
                if (not next_line.startswith(' ') and 
                    ...
                    not next_line.strip().startswith('finally') and
    ...
    while i < len(lines):
        ...
        if (re.match(r'^\\s*if\\s+.*:\\s*$', line) and 
            ...
            if next_line_idx < len(lines):
                ...
                if (not next_line.startswith(' ') and 
                    not next_line.startswith('\t') and
                    not next_line.strip().startswith('else') and
                    not next_line.strip().startswith('elif') and
                    not next_line.strip().startswith('#')):
                    fixed_lines.append('    pass')
...
def fix_missing_for_blocks(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if (re.match(r'^\\s*for\\s+.*:\\s*$', line) and 
            line.strip().endswith(':')):
            # Look ahead to see if next non-empty line is indented
            next_line_idx = i + 1
            while next_line_idx < len(lines) and lines[next_line_idx].strip() == '':
...
def fix_missing_while_blocks(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if (re.match(r'^\\s*while\\s+.*:\\s*$', line) and 
            line.strip().endswith(':')):
            # Look ahead to see if next non-empty line is indented
            next_line_idx = i + 1
            while next_line_idx < len(lines) and lines[next_line_idx].strip() == '':
