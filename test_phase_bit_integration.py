...
from core.phase_bit_integration import (
    PhaseBitIntegration, BitPhase, StrategyType, FerrisPhase,
...
def test_phase_bit_integration():
    ...
    for phase in [FerrisPhase.LOW, FerrisPhase.MID, FerrisPhase.HIGH]:
        ...
        print(f"  {phase.value.upper():4} → {mapping.bit_phase.value}-bit "
    ...
    for phase in [FerrisPhase.LOW, FerrisPhase.MID, FerrisPhase.HIGH]:
        ...
        print(f"    Allocation: BTC {strategy['allocation']['BTC']:.1%}, "
    ...
    print(f"  Bit Weights: α={status['bit_phase_weights']['alpha_weight']:.1f}, "
    ...
    print(f"  Expected φ₄: {expected_phi_4}, Actual: {bit_result.phi_4} "
          f"({'✅' if expected_phi_4 == bit_result.phi_4 else '❌'})")
    print(f"  Expected φ₈: {expected_phi_8}, Actual: {bit_result.phi_8} "
          f"({'✅' if expected_phi_8 == bit_result.phi_8 else '❌'})")
    print(f"  Expected φ₄₂: {expected_phi_42}, Actual: {bit_result.phi_42} "
