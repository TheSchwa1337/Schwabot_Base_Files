...
class SchwabotStartupOrchestrator:
    ...
    def _display_startup_summary(self, success: bool) -> None:
        ...
        success_rate = (self.successful_components / self.total_components * 100) if self.total_components > 0 else 0
        
        print("\n" + "=" * 80)
        print("📊 STARTUP SEQUENCE COMPLETE")
        print("=" * 80)
        print(f"⏱️  Total startup time: {total_time:.2f} seconds")
        print(f"🎯 Component success rate: {success_rate:.1f}% ({self.successful_components}/{self.total_components})")
    ...
    def _launch_coinbase_integration(self) -> None:
        ...
        print(f"  📊 Core Components: {self.successful_components}/{self.total_components} operational ({(self.successful_components/self.total_components*100):.1f}%)")
        print(f"  🧮 Mathematical Core: {'✅ OPERATIONAL' if 'clean_unified_math.py' in [c.name for c in self.component_status.values() if c.test_result] else '❌ NEEDS REPAIR'}")
        print(f"  ⚡ Trading Engine: {'✅ OPERATIONAL' if 'trading_engine_integration.py' in [c.name for c in self.component_status.values() if c.test_result] else '❌ NEEDS REPAIR'}")
        print("")
    ...
    def _show_trading_system_status(self) -> None:
        ...
        operational_count = sum(1 for file, _ in key_components 
                              if file in self.component_status and self.component_status[file].test_result)
