...
def demo_basic_interpretation():
    ...
    for scenario in test_scenarios:
        print(f"\n📊 {scenario['name']}: ${scenario['price']:,.2f} | Vol: {scenario['volume']:,.0f}")
...
def demo_alpha_lantern_integration():
    ...
    print(f"   Entropy Correlation: {abs(hash_block.entropy_block.entropy_score - alpha_result.total_entropy):.4f}")
    
    if hash_block.semantic_interpretation:
        print(f"   Semantic Confidence: {hash_block.semantic_interpretation.confidence_score:.3f}")
        print(f"   Pattern Strength: {hash_block.semantic_interpretation.pattern_strength:.3f}")
...
def demo_memory_building():
    ...
    for phase in phases:
        ...
        for i in range(phase['periods']):
            ...
            import random
            base_price *= phase['trend'] * (1 + random.uniform(-phase['volatility'], phase['volatility']))
    ...
    for phase, interpretations in phase_interpretations.items():
        if interpretations:
            avg_confidence = sum(i['confidence'] for i in interpretations) / len(interpretations)
            avg_profit = sum(i['profit_potential'] for i in interpretations) / len(interpretations)
    ...
    try:
        ...
        if significant_signals:
            ...
            for signal in significant_signals[-3:]:  # Show last 3
                print(f"     • {signal['meaning'][:50]}... (conf: {signal['confidence']:.3f})")
