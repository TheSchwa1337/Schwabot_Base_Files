...
class AutomatedTradingPipelineCLI:
    ...
    def init(self, risk_tolerance: float = 0.02, profit_target: float = 0.05,
             position_size: float = 0.1, processing_mode: str = "hybrid",
             registry_path: str = "data/trading_vector_registry.json") -> bool:
    ...
    def process_tick(self, price: float, volume: float = 0.0, 
                    bid: float = 0.0, ask: float = 0.0) -> Optional[str]:
        ...
        try:
            ...
            if decision:
                return (f"🎯 TRADING DECISION MADE:\n"
...
Examples:
  ...
  %(prog)s stop                           # Stop continuous pipeline
    ...
    init_parser.add_argument('--risk-tolerance', type=float, default=0.02,
                           help='Risk tolerance (default: 0.02)')
    init_parser.add_argument('--profit-target', type=float, default=0.05,
                           help='Profit target (default: 0.05)')
    init_parser.add_argument('--position-size', type=float, default=0.1,
                           help='Position size (default: 0.1)')
    init_parser.add_argument('--mode', choices=['gpu', 'cpu', 'hybrid', 'safe'], 
                           default='hybrid', help='Processing mode (default: hybrid)')
    init_parser.add_argument('--registry-path', default='data/trading_vector_registry.json',
                           help='Vector registry path')
    ...
    stream_parser.add_argument('--max-decisions', type=int, default=10,
                              help='Maximum decisions to make (default: 10)')
    ...
    decisions_parser.add_argument('--count', type=int, default=10,
                                 help='Number of decisions to show (default: 10)')
    ...
    try:
        if args.command == 'init':
            success = cli.init(args.risk_tolerance, args.profit_target, 
                             args.position_size, args.mode, args.registry_path)
