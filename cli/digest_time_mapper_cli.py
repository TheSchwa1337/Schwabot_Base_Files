...
class DigestTimeMapperCLI:
    ...
    def init(self, frame_window_ms: int = 1000, phase_period_ms: int = 60000) -> bool:
        ...
        try:
            ...
            # Initialize mapper
            self.mapper = DigestTimeMapper(
                frame_window_ms=frame_window_ms,
                phase_period_ms=phase_period_ms
            )
    ...
    def process_tick(self, price: float, volume: float = 0.0, 
        ...
        try:
            frame = self.mapper.process_millisecond_tick(
                price=price,
                volume=volume,
                bid=bid,
                ask=ask
    ...
    def generate_digest(self, frame_count: int = 16) -> Optional[str]:
        ...
        try:
            ...
            if digest_result:
                ...
                return (f"✅ Generated digest: {digest_result.digest_hex[:16]}...\n"
    ...
    def ferris_wheel(self, duration_seconds: int = 60) -> bool:
        ...
        try:
            ...
            for digest_result in self.mapper.ferris_wheel_loop(price_stream()):
                ...
                digest_count += 1
                print(f"🎡 Digest {digest_count}: {digest_result.digest_hex[:16]}... "
                      f"(entropy: {digest_result.entropy_score:.3f})")
...
Examples:
  ...
  %(prog)s stop                           # Stop processing loop
    ...
    init_parser.add_argument('--frame-window', type=int, default=1000,
                           help='Frame window in milliseconds (default: 1000)')
    init_parser.add_argument('--phase-period', type=int, default=60000,
    ...
    digest_parser.add_argument('--frame-count', type=int, default=16,
                              help='Number of frames to use (default: 16)')
