  ...
  python cli/fill_handler_cli.py export state_backup.json
    ...
    async def cmd_orders(self):
        ...
        for order_id, order in self.fill_handler.active_orders.items():
            status_icon = "✅" if order.status == FillStatus.COMPLETE else \
                         "🔄" if order.status == FillStatus.PARTIAL else \
                         "⏳" if order.status == FillStatus.PENDING else "❌"
    ...
    async def cmd_handle_partial(self, order_id: str):
        ...
        result = await self.fill_handler.handle_partial_fill(order_id, {
            "orderId": order_id,
            "status": "partial"
        })
