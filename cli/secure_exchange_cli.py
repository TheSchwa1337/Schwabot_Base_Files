...
from core.secure_exchange_manager import (
    SecureExchangeManager, ExchangeType, get_exchange_manager
)
...
class SecureExchangeCLI:
    ...
    def setup_exchange(self, exchange_name: str, api_key: str = None, 
                      secret: str = None, passphrase: str = None, 
                ...
                for exchange_name, status in summary['exchanges'].items():
                    ...
                    for key, value in status.items():
                        if key == 'error' and value:
                            result += f"  ❌ {key}: {value}\n"
                        elif key in ['connected', 'authenticated', 'trading_enabled', 'balance_available']:
