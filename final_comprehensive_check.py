...
def main():
    ...
    install_checks = [
        ...
        run_command('pip install black isort flake8 mypy bandit autoflake chardet', 'Installing code quality tools'),
    ]
    ...
    checks = [
        ...
        run_command(f'flake8 {" ".join(valid_dirs)} --max-line-length=100 --count', 'Running Flake8 style check'),
        run_command(f'black {" ".join(valid_dirs)} --check --line-length=100 --target-version py311', 'Checking code formatting with Black'),
        run_command(f'isort {" ".join(valid_dirs)} --check-only --profile black', 'Checking import sorting'),
        
        # Type checking
        run_command(f'mypy {" ".join(valid_dirs)} --ignore-missing-imports', 'Running MyPy type checking'),
        ...
        run_command('python -m py_compile schwabot/__init__.py', 'Testing schwabot module compilation'),
    ]
