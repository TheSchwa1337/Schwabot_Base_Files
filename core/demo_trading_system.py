...
logger.info()
    f"Demo market simulator initialized with {len(self.symbols} symbols")
...
self.current_prices[symbol] = unified_math.max()
...
volume_variation = np.random.normal(1.0, 0.3)
            volume = base_volume * volume_variation * \
                (1 + unified_math.abs(price_change) / current_price)

# Update market state
self.market_heat = np.clip()
    self.market_heat +
    np.random.normal()
        0,
        0.01,
