...
class SignalCollector:
    ...
    def collect_core_signals(
        ...
        try:
            ...
            return TradingSignalMetrics(
                triplet_entropy=0.0, theta_drift=0.0, coherence=0.0,
                loop_volatility=0.0, profit_decay=0.0, harmony=0.0,
                drift_penalty=1.0, liquidity_score=0.0, projected_profit=0.0,
                timestamp=time.time()
            )
    ...
    def collect_btc_signals(
        ...
        try:
            ...
            return BTCInvestmentSignals(
                v_btc=0.0, eta_btc=0.0, xi_btc=0.0, price_pressure=0.0,
                volume_profile=0.0, hash_correlation=0.0, network_strength=0.0
            )
    ...
    def get_signal_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_projected_profit": np.mean([s.projected_profit for s in recent_signals]),
...
def main() -> None:
    ...
    core_signals, btc_signals = collect_unified_signals(
        mock_cursor_state, mock_fractal_state, mock_collapse_state,
        mock_market_data, mock_btc_data
    )
