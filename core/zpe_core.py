...
class ZPECore:
    ...
    self._initialize_backlog_trajectories()
        ...
        logger.info(f"ZPE Core initialized with {self.active_math_system.value} math system")
    ...
    def _select_optimal_math_system(self, operation_name: str) -> MathSystemType:
    ...
    def _should_use_legacy_for_warm(self) -> bool:
        ...
        legacy_avg = sum(p.profitability for p in legacy_performance) / len(legacy_performance)
        unified_avg = sum(p.profitability for p in unified_performance) / len(unified_performance)
    ...
    def _select_by_performance(self, operation_name: str) -> MathSystemType:
        ...
        legacy_score = self._calculate_system_score(recent_performance, MathSystemType.LEGACY)
        unified_score = self._calculate_system_score(recent_performance, MathSystemType.UNIFIED)

        # Apply thermal efficiency weighting
        thermal_metrics = self._get_current_thermal_metrics()
        thermal_factor = 1.0 - (thermal_metrics.cpu_temp / 100.0)  # Lower temp = higher factor
    ...
    def _calculate_system_score(self, performance_list: List[MathPerformanceMetrics],
                               system_type: MathSystemType) -> float:
        """Calculate recommendation score for a math system."""
        system_performance = [p for p in performance_list if p.system_type == system_type]
        ...
        avg_profitability = sum(p.profitability for p in system_performance) / len(system_performance)
        avg_accuracy = sum(p.accuracy for p in system_performance) / len(system_performance)
        avg_thermal = 1.0 - (sum(p.thermal_impact for p in system_performance) / len(system_performance))
    ...
    def _execute_with_performance_tracking(self, operation_name: str, operation_func, *args, **kwargs):
        ...
        try:
            if selected_system == MathSystemType.UNIFIED:
                ...
                result = self._thermal_fallback_operation(operation_name, *args, **kwargs)
            else:
                # Hybrid approach
                result = self._hybrid_operation(operation_func, *args, **kwargs)
            ...
            performance_metric = MathPerformanceMetrics(
                ...
                accuracy=self._estimate_accuracy(operation_name, result),
                thermal_impact=thermal_metrics.cpu_temp / 100.0,
                profitability=self._estimate_profitability(operation_name, result),
                timestamp=datetime.now()
            )
            ...
            if selected_system != self.active_math_system:
                logger.info(f"Switching math system from {self.active_math_system.value} to {selected_system.value}")
            ...
            return self._legacy_fallback_operation(operation_name, *args, **kwargs)

    def _thermal_fallback_operation(self, operation_name: str, *args, **kwargs):
        ...
        if operation_name == "calculate_zpe_work":
            ...
            return math.tanh(trend_strength) * entry_exit_range * 0.5  # Simplified calculation
        elif operation_name == "calculate_rotational_torque":
            liquidity_depth, trend_change_rate = args[0], args[1]
            return (1.0 / (1.0 + liquidity_depth)) * math.atan(trend_change_rate) * 0.5
        else:
    ...
    def _hybrid_operation(self, operation_func, *args, **kwargs):
        ...
        thermal_metrics = self._get_current_thermal_metrics()
        thermal_weight = thermal_metrics.cpu_temp / 100.0  # Higher temp = more legacy weight
        
        return unified_result * (1.0 - thermal_weight) + legacy_result * thermal_weight

    def _legacy_fallback_operation(self, operation_name: str, *args, **kwargs):
        """Legacy fallback operation."""
        if operation_name == "calculate_zpe_work":
            trend_strength, entry_exit_range = args[0], args[1]
            return self.legacy_math.multiply(math.tanh(trend_strength), entry_exit_range).value
        elif operation_name == "calculate_rotational_torque":
            liquidity_depth, trend_change_rate = args[0], args[1]
    ...
    def _estimate_profitability(self, operation_name: str, result: float) -> float:
    ...
    def calculate_zpe_work(self, trend_strength: float, entry_exit_range: float) -> float:
        ...
        return self._execute_with_performance_tracking("calculate_zpe_work", _zpe_work_operation, trend_strength, entry_exit_range)
    ...
    def calculate_rotational_torque(self, liquidity_depth: float, trend_change_rate: float) -> float:
        ...
        return self._execute_with_performance_tracking("calculate_rotational_torque", _torque_operation, liquidity_depth, trend_change_rate)
    ...
    def calculate_thermal_efficiency(self, profit_generated: float, capital_exposure: float) -> float:
        ...
        return self._execute_with_performance_tracking("calculate_thermal_efficiency", _efficiency_operation, profit_generated, capital_exposure)
    ...
    def calculate_elastic_resonance(self, price_derivative: float, frequency: float, phase_offset: float, time_window: float) -> float:
        ...
        def _resonance_operation(math_system, pd, freq, phase, tw):
            if hasattr(math_system, 'calculate_elastic_resonance'):
                return math_system.calculate_elastic_resonance(pd, freq, phase, tw)
            else:
                # Legacy system fallback
                dt = 0.001
        ...
        return self._execute_with_performance_tracking("calculate_elastic_resonance", _resonance_operation, price_derivative, frequency, phase_offset, time_window)
    ...
    def calculate_multi_vector_alignment(self, strategy_vectors: Dict[str, Dict], weights: Dict[str, float]) -> Dict:
        ...
        return self._execute_with_performance_tracking("calculate_multi_vector_alignment", _alignment_operation, strategy_vectors, weights)
    ...
    def get_math_system_recommendations(self) -> Dict[str, Any]:
        ...
        recommendations = {
            ...
            'thermal_state': self._assess_thermal_state(self._get_current_thermal_metrics()).value,
            'system_scores': {},
            'recommendations': []
        }
        ...
        for system_type in MathSystemType:
            score = self._calculate_system_score(self.performance_history, system_type)
            recommendations['system_scores'][system_type.value] = score
        
        # Generate recommendations
        thermal_metrics = self._get_current_thermal_metrics()
        if thermal_metrics.cpu_temp > 80:
            recommendations['recommendations'].append("High CPU temperature - consider switching to legacy system")
        
        if len(self.performance_history) > 10:
            recent_profitability = [p.profitability for p in self.performance_history[-10:]]
            avg_profitability = sum(recent_profitability) / len(recent_profitability)
            if avg_profitability < 0.3:
                recommendations['recommendations'].append("Low profitability - consider system optimization")
    ...
    def update_recursive_cycle_depth(self, tick_interval: float, price_trigger: float) -> int:
        ...
        def _recursion_operation(math_system, ti, pt):
            ...
            complexity = math_system.min(16.0, 1.0 + math_system.abs(pt) * 10.0)
            return int(complexity)
        
    self.recursion_depth = self._execute_with_performance_tracking(
            "update_recursive_cycle_depth", _recursion_operation, tick_interval, price_trigger
        )
    ...
    def calculate_temporal_fault_correction(self, expected_phase: float, actual_phase: float) -> float:
        ...
        phase_difference = self._execute_with_performance_tracking(
            "calculate_temporal_fault_correction", _temporal_operation, expected_phase, actual_phase
        )
    ...
    def calculate_profit_reinjection(self, profit_delta: float, market_heat: float) -> float:
        ...
        def _reinjection_operation(math_system, pd, mh):
            reinjection_coefficient = math_system.min(1.0, math_system.max(0.0, mh))
