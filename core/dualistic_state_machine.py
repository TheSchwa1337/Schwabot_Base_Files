...
class DualisticStateMachine:
    ...
    def force_transition(self, target_state: StateType, reason: str = "manual") -> bool:
        ...
        if success:
            logger.info(f"🔄 Forced transition to {target_state.value}: {reason}")
    ...
    def get_state_recommendations(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "should_transition": optimal_state != self.current_state and confidence > 0.3,
            "transition_urgency": confidence if optimal_state != self.current_state else 0.0
        }
    
    def add_transition_callback(self, callback: Callable[[TransitionEvent], None]) -> None:
    ...
    def _evaluate_transition(self) -> None:
        ...
        # Check cooldown
        if current_time - self.last_transition_time < self.transition_cooldown_ms / 1000.0:
            return
        ...
        if transition_score > 0.7:  # Strong signal for transition
            target_state = StateType.ALIF if self.current_state == StateType.ALEPH else StateType.ALEPH
    ...
    def _calculate_state_suitability(self, state: StateType) -> float:
        ...
        if state == StateType.ALEPH:
            # ALEPH favors: low entropy, high nibble, low volatility, structured patterns
            suitability = (
            ...
            # ALIF favors: high entropy, high rittle, high volatility, adaptive patterns
            suitability = (
                self.entropy_level * 0.3 +
    ...
    def _execute_transition(self, 
                          ...
                          trigger: TransitionTrigger,
                          trigger_values: Dict[str, Any]) -> bool:
        ...
        logger.info(f"🎭 State transition: {old_state.value} → {target_state.value} "
                   f"(trigger: {trigger.value}, confidence: {event.confidence:.3f})")
    ...
    def _calculate_current_confidence(self) -> float:
        ...
        time_factor = min(1.0, state_duration / 10.0)  # Max confidence after 10 seconds
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if total_transitions > 0:
            recent_transitions = list(self.transition_history)[-10:]  # Last 10 transitions
            trigger_counts = {}
...
def main():
    ...
    def on_transition(event: TransitionEvent):
        print(f"  🔄 Transition callback: {event.from_state.value} → {event.to_state.value}")
        print(f"      Trigger: {event.trigger.value}, Confidence: {event.confidence:.3f}")
