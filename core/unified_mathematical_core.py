...
class UnifiedMathCore:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Unified Math Core initialized - GPU: {}, Device: {}".format(
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            'precision': FIT_PROFILE.precision if hasattr(FIT_PROFILE, 'precision') else 'float32',
            'matrix_size': FIT_PROFILE.matrix_size if hasattr(FIT_PROFILE, 'matrix_size') else 1024,
            'gpu_enabled': FIT_PROFILE.can_run_gpu_logic if hasattr(FIT_PROFILE, 'can_run_gpu_logic') else False,
    ...
    def calculate_zpe(self, frequency: float, uncertainty: Optional[float] = None) -> ZPECalculation:
        ...
        try:
            ...
            if not (min_freq <= frequency <= max_freq):
                logger.warning("Frequency {} Hz outside valid range [{}, {}]".format(
    ...
    def calculate_zbe(self, probability_distribution: np.ndarray) -> ZBECalculation:
        ...
        try:
            ...
            non_zero_probs = probability_distribution[probability_distribution > self.config['zbe_probability_threshold']]
    ...
    def optimize_basket_tiers(self, portfolio_weights: np.ndarray, 
