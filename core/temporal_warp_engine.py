...
class TemporalWarpEngine:
    ...
    def __init__(self, default_alpha: float = 100.0, max_warp_duration: int = 3600):
        ...
        logger.info("Temporal Warp Engine initialized (alpha: {0}, max_duration: {1}s)".format(default_alpha, max_warp_duration))
    ...
    def update_window(self, strategy_id: str, drift: float, alpha: Optional[float] = None) -> WarpWindow:
        ...
        try:
            ...
            logger.debug("Updated warp window for {0}: delay={1}s, confidence={2}".format(strategy_id, warp_delay:.1f, confidence:.3f))
            return warp_window
...
def test_temporal_warp_engine():
    ...
    print("  Created warp window: delay={0}s".format(drift * 60:.1f))
    print("  Open time: {0}".format(window.open_time))
    print("  Close time: {0}".format(window.close_time))
    print("  Confidence: {0}".format(window.confidence:.3f))
