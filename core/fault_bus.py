...
class FaultBus:
    ...
    def get_listener_count(self, topic: str = None, pattern_hash: str = None) -> int:
        """
    ...
    def get_active_topics(self) -> List[str]:
        ...
        return [topic for topic, listeners in self._listeners.items() if listeners]

    def get_active_hashes(self) -> List[str]:
        """Get list of hashes that have active listeners."""
        return [hash_val for hash_val, listeners in self._hash_listeners.items() if listeners]
    ...
    def clear_hash(self, pattern_hash: str) -> None:
        ...
        self._hash_listeners[pattern_hash].clear()
            logger.info(f"Cleared all listeners for hash '{pattern_hash[:10]}...'.")

    def get_bus_stats(self) -> Dict[str, Any]:
        """Get comprehensive statistics about the fault bus."""
        total_topic_listeners = sum(len(listeners) for listeners in self._listeners.values())
        total_hash_listeners = sum(len(listeners) for listeners in self._hash_listeners.values())
...
async def main():
    ...
    async def trading_executor_listener(pattern_hash: str, **_):
        print(f"[Executor] Received confirmed profitable hash: {pattern_hash[:10]}... Preparing to execute trade.")

    async def another_trade_listener(pattern_hash: str, **_):
        print(f"[Executor2] Also saw hash {pattern_hash[:10]}... Logging for confirmation.")
