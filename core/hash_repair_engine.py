...
class HashRepairEngine:
        ...
        try:
            ...
            for repair_method in repair_methods:
                try:
                    ...
                    logger.warning(f"Repair method {repair_method.__name__} failed: {e}")
                    continue
            
            # If no repair found, use fallback
            if not best_repair:
                best_repair = self._fallback_repair(failed_hash, historical_hashes)
            ...
            repair_result = RepairResult(
                ...
                repair_method="similarity_based" if best_confidence > 0.7 else "fallback",
                confidence_score=best_confidence,
                similarity_score=self._calculate_hash_similarity(failed_hash, best_repair),
            ...
            logger.info(f"Hash repair completed: {failed_hash[:8]}... -> {best_repair[:8]}... "
    ...
    def _interpolate_hashes(self, hash1: str, hash2: str, 
        ...
        """Interpolate between two hash values."""
        try:
            if len(hash1) != len(hash2):
                return hash1
    ...
    def _calculate_repair_confidence(self, original_hash: str, 
        ...
        try:
            ...
            similarity_score = self._calculate_hash_similarity(original_hash, repaired_hash)
            
            # Frequency confidence
            frequency_confidence = 0.0
            if historical_hashes:
                hash_count = historical_hashes.count(repaired_hash)
                frequency_confidence = min(hash_count / len(historical_hashes), 1.0)
    ...
    def get_repair_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            confidence_scores = [r.confidence_score for r in self.repair_history]
            similarity_scores = [r.similarity_score for r in self.repair_history]
