...
class PortfolioSubstitutionMatrix:
    ...
    def calculate_substitution(self, parameters: SubstitutionParameters,
                             phase_mode: PhaseMode,
                             rebalance_reason: RebalanceReason,
                             anomaly_context: Optional[Dict[str, Any]] = None) -> SubstitutionResult:
        ...
        try:
            ...
            result = SubstitutionResult(
                ...
                estimated_slippage=self._estimate_total_slippage(trade_orders, parameters),
                execution_time_estimate=self._estimate_execution_time(trade_orders, parameters),
                substitution_rationale=rationale,
                risk_metrics=self._calculate_risk_metrics(target_allocation, parameters),
            ...
            logger.debug(f"📊 Portfolio substitution calculated in {calculation_time:.4f}s")
            
            return result
            
        except Exception as e:
            logger.error(f"❌ Portfolio substitution calculation failed: {e}")
            return self._create_fallback_substitution(parameters, phase_mode, rebalance_reason)
    
    def _determine_substitution_strategy(self, parameters: SubstitutionParameters,
                                       phase_mode: PhaseMode,
                                       rebalance_reason: RebalanceReason,
                                       anomaly_context: Optional[Dict[str, Any]]) -> SubstitutionStrategy:
        ...
        if parameters.risk_tolerance < 0.2:
            ...
            if base_strategy in [SubstitutionStrategy.BALANCED, SubstitutionStrategy.AGGRESSIVE]:
                return SubstitutionStrategy.AGGRESSIVE
        
        return base_strategy
    
    def _get_substitution_weights(self, phase_mode: PhaseMode,
                                strategy: SubstitutionStrategy,
                                parameters: SubstitutionParameters,
                                anomaly_context: Optional[Dict[str, Any]]) -> np.ndarray:
    ...
    def _apply_risk_adjustments(self, weights: np.ndarray,
                              parameters: SubstitutionParameters) -> np.ndarray:
        ...
        if parameters.volatility_levels:
            vol_scores = np.array([
                parameters.volatility_levels.get(asset, 0.02) for asset in self.asset_order
            ])
        ...
        if parameters.liquidity_scores:
            liq_scores = np.array([
                parameters.liquidity_scores.get(asset, 0.5) for asset in self.asset_order
            ])
    ...
    def _apply_market_regime_adjustments(self, weights: np.ndarray,
                                       anomaly_context: Dict[str, Any]) -> np.ndarray:
    ...
    def _apply_correlation_adjustments(self, weights: np.ndarray,
                                     parameters: SubstitutionParameters) -> np.ndarray:
    ...
    def _calculate_target_allocation(self, weights: np.ndarray,
                                   parameters: SubstitutionParameters) -> Dict[AssetType, float]:
    ...
    def _generate_trade_orders(self, current_allocation: Dict[AssetType, float],
                             target_allocation: Dict[AssetType, float],
                             parameters: SubstitutionParameters) -> List[Dict[str, Any]]:
    ...
    def _calculate_trade_priority(self, asset: AssetType, amount: float,
                                parameters: SubstitutionParameters) -> float:
    ...
    def _estimate_slippage(self, asset: AssetType, amount: float,
                         parameters: SubstitutionParameters) -> float:
    ...
    def _estimate_total_slippage(self, trade_orders: List[Dict[str, Any]],
                               parameters: SubstitutionParameters) -> float:
    ...
    def _estimate_execution_time(self, trade_orders: List[Dict[str, Any]],
                               parameters: SubstitutionParameters) -> float:
        ...
        urgency_multiplier = 2.0 - parameters.execution_urgency  # Higher urgency = faster
        
        # Adjust based on number of trades (parallel execution)
        parallel_factor = min(len(trade_orders) / 4, 1.0)  # Up to 4 parallel trades
        
        total_time = (len(trade_orders) * base_time_per_trade * urgency_multiplier) / (1 + parallel_factor)
        
        return total_time
    
    def _calculate_expected_return(self, target_allocation: Dict[AssetType, float],
                                 parameters: SubstitutionParameters) -> float:
    ...
    def _calculate_risk_adjustment(self, target_allocation: Dict[AssetType, float],
                                 parameters: SubstitutionParameters) -> float:
    ...
    def _calculate_confidence_score(self, strategy: SubstitutionStrategy,
                                  parameters: SubstitutionParameters,
                                  anomaly_context: Optional[Dict[str, Any]]) -> float:
    ...
    def _generate_substitution_rationale(self, strategy: SubstitutionStrategy,
                                       rebalance_reason: RebalanceReason,
                                       target_allocation: Dict[AssetType, float],
                                       parameters: SubstitutionParameters) -> List[str]:
        """Generate human-readable rationale for substitution."""
        
        rationale = []
        
        # Strategy rationale
        rationale.append(f"Selected {strategy.value} strategy based on current conditions")
        ...
        rationale.append(reason_explanations.get(rebalance_reason, "Portfolio rebalancing"))
        ...
        if total_value > 0:
            for asset, amount in target_allocation.items():
                weight = amount / total_value
                rationale.append(f"{asset.value}: {weight:.1%} allocation (${amount:,.0f})")
    ...
    def _calculate_risk_metrics(self, target_allocation: Dict[AssetType, float],
                              parameters: SubstitutionParameters) -> Dict[str, float]:
    ...
    def _select_performance_based_variant(self, parameters: SubstitutionParameters) -> int:
    ...
    def _update_performance_metrics(self, result: SubstitutionResult) -> None:
        ...
        if len(self.substitution_history) >= 5:
            recent_returns = [r.expected_return for r in self.substitution_history[-10:]]
            recent_confidence = [r.confidence_score for r in self.substitution_history[-10:]]
    ...
    def _create_fallback_substitution(self, parameters: SubstitutionParameters,
                                    phase_mode: PhaseMode,
                                    rebalance_reason: RebalanceReason) -> SubstitutionResult:
        ...
        target_allocation = {}
        for i, asset in enumerate(self.asset_order):
            target_allocation[asset] = fallback_weights[i] * parameters.total_portfolio_value
...
def execute_portfolio_substitution(current_allocation: Dict[str, float],
    ...
    return {
        'target_allocation': {asset.value: amount for asset, amount in result.target_allocation.items()},
