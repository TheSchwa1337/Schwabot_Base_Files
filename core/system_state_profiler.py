...
class SystemStateProfiler:
    ...
    def build_full_system_profile(self) -> SystemProfile:
        ...
        logger.info(f"✅ System Profile Complete - Tier: {system_tier.value}")
        logger.info(f"🔧 CPU: {cpu.model} ({cpu.cpu_tier.value})")
        logger.info(f"🎮 GPU: {gpu.renderer} ({gpu.gpu_tier.value})")
        logger.info(f"📊 Matrix Size: {gpu.max_matrix_size}x{gpu.max_matrix_size}")
    ...
    def _save_profile(self, profile_data: Dict[str, Any]):
        ...
        try:
            ...
            timestamped_path = os.path.join(
                self.profiles_dir, 
                f"profile_{timestamp}_{profile_data['system_hash'][:8]}.json"
            )
            with open(timestamped_path, "w") as f:
                json.dump(profile_data, f, indent=2)
            
            logger.info(f"💾 Profile saved: {latest_path}")
...
def get_system_profile(force_rebuild: bool = False) -> SystemProfile:
    ...
    if not force_rebuild:
        ...
        if existing_profile:
            logger.info(f"📋 Loaded existing profile: {existing_profile.system_tier.value}")
