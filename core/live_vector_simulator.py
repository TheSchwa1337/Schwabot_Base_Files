    ...
    def _evolve_price(self) -> float:
        ...
        try:
            ...
            mean_reversion_component = (
                ...
                (self.config.initial_price - self.current_price) / self.current_price
            )
    ...
    def _calculate_entropy(self, price_change: float, volume_change: float) -> float:
        ...
        try:
            ...
            composite_entropy = (
                ...
                volume_irregularity + 
                spread_factor
            ) * regime_params["entropy_multiplier"]
...
async def test_live_vector_simulator():
    ...
    async def tick_callback(snapshot: MarketSnapshot, hash_triggered: bool):
        if hash_triggered:
            logger.info(f"🔗 Tick {simulator.total_ticks}: Price=${snapshot.price:,.2f}, Entropy={snapshot.entropy:.4f}, Hash Triggered")
        elif simulator.total_ticks % 100 == 0:  # Log every 100th tick
            logger.info(f"📊 Tick {simulator.total_ticks}: Price=${snapshot.price:,.2f}, Entropy={snapshot.entropy:.4f}")
