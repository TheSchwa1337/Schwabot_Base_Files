...
class LiveVectorSimulator:
    ...
    def _evolve_price(self) -> float:
        ...
        mean_reversion_component = -self.config.mean_reversion * (self.current_price - self.config.initial_price) / self.config.initial_price * dt
    ...
    def _evolve_volume(self) -> float:
        ...
        base_volume = self.config.base_volume * (1 + self.config.volume_trend * self.total_ticks)
    ...
    def _calculate_entropy(self, price_change: float, volume_change: float) -> float:
        ...
        # Price-based entropy
        price_entropy = abs(price_change / self.current_price) if self.current_price > 0 else 0
        
        # Volume-based entropy
        volume_entropy = abs(volume_change / self.current_volume) if self.current_volume > 0 else 0
    ...
    async def run_simulation(self, callback: Optional[Callable] = None) -> None:
        """
        ...
        logger.info(f"✅ Simulation completed: {self.total_ticks} ticks, {self.hash_triggers} hash triggers")
    ...
    def get_simulation_summary(self) -> Dict[str, Any]:
        ...
        # Price statistics
        price_return = (price_array[-1] - price_array[0]) / price_array[0]
        price_volatility = np.std(np.diff(price_array) / price_array[:-1])
        
        # Volume statistics
        avg_volume = np.mean(volume_array)
        volume_volatility = np.std(volume_array)
        
        # Entropy statistics
        avg_entropy = np.mean(entropy_array)
        entropy_volatility = np.std(entropy_array)
    ...
    def export_data(self, filename: str) -> None:
        ...
        data = {
            ...
            "regime_history": [regime.value for regime in self.regime_history],
...
# Factory function
def create_live_vector_simulator(config: Optional[SimulationConfig] = None) -> LiveVectorSimulator:
    """Create a LiveVectorSimulator instance."""
    if config is None:
        config = SimulationConfig()
    return LiveVectorSimulator(config) 