    ...
    def initialize(self, cli_truncated_hash: bool = False, cli_hash_length: Optional[int] = None) -> None:
        ...
        try:
            ...
            if cli_truncated_hash or cli_hash_length is not None:
                ...
                logger.info(f"🔧 Hash config: CLI override - truncated={self.config.truncated_hash}, length={self.config.hash_length}")
                return
            ...
            logger.info(f"🔧 Hash config: Auto-detected {hardware_tier.value} - truncated={self.config.truncated_hash}, length={self.config.hash_length}")
...
def generate_hash(data: bytes) -> str:
    """Generate hash using global configuration."""
    return hash_config_manager.generate_hash(data)

def generate_hash_from_string(data: str) -> str:
    """Generate hash from string using global configuration."""
