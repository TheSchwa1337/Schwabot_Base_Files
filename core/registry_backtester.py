...
class RegistryBacktester:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.trading_engine = BTCTradingEngine(config={
                'testnet': True,
    ...
    async def run_backtest(self) -> BacktestResult:
        ...
        try:
            ...
            async for data_point in self.historical_data_manager.get_historical_data(source="mock"):
            ...
            for hash_id, strategy in self.strategies.items():
                try:
                    ...
                    if self.config.enable_feedback and self.feedback_engine:
                        ...
                        feedback_result = self.feedback_engine.process(strategy_result, market_data)
                        
                        if 'error' not in feedback_result:
                            self.cycle_count += 1
                            
                            # Log cycle result
                            logger.debug(f"Cycle {self.cycle_count}: Strategy {hash_id[:8]} | "
    ...
    def _create_tick_data(self, data_point: Dict[str, Any]) -> TickData:
        ...
        try:
            ...
            timestamp = int(datetime.fromisoformat(data_point['datetime']).timestamp() * 1000)
            
            # Get mathematical analysis from trading engine
            btc_price_data = self.trading_engine.process_btc_price(
                price=price,
                volume=volume,
                thermal_state=65.0  # Default thermal state
            )
    ...
    def _generate_backtest_result(self) -> BacktestResult:
        ...
        try:
            ...
            successful_cycles = len([r for r in successful_results if r.actual_profit > 0])
            ...
            performance_summary = {
                ...
                'average_confidence': sum(r.confidence for r in successful_results) / total_cycles,
                'strategy_performance': {
                    hash_id: {
                        'executions': len([r for r in self.results if r.strategy_hash == hash_id]),
                        'success_rate': len([r for r in self.results if r.strategy_hash == hash_id and r.actual_profit > 0]) / 
    ...
    def print_summary(self, result: BacktestResult) -> None:
        ...
        try:
            ...
            print("\n📋 STRATEGY PERFORMANCE BREAKDOWN:")
            for hash_id, perf in result.performance_summary['strategy_performance'].items():
                print(f"  {hash_id[:8]}... | Executions: {perf['executions']} | "
