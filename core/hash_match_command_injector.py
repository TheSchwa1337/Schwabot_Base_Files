...
class HashMatchCommandInjector:
    ...
    def _update_performance_tracking(self, injection_result: InjectionResult):
        """
        ...
        try:
            ...
            self.success_rate = (
                self.feedback_decay * self.success_rate + 
                (1 - self.feedback_decay) * current_success_rate
            )
    ...
    async def process_tick(self, tick_data: Dict[str, Any]) -> Optional[InjectionResult]:
        ...
        try:
            ...
            logger.info(f"Command injected: {command.command_type.value} for {command.symbol} with confidence {command.confidence:.3f}")
        ...
        try:
            ...
            performance_summary = {
                ...
                "recent_success_rate": (
                    ...
                    if recent_injections else 0.0
                ),
                "hash_match_count": len(self.hash_match_history),
                "average_execution_time": (
                    np.mean([r.execution_time for r in recent_injections])
                    if recent_injections else 0.0
                ),
