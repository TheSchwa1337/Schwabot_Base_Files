...
# Find optimal tier
optimal_tier_name = unified_math.max()
    tier_scores.keys(),
        key = lambda k: tier_scores[k]
...
price_norm=(btc_price - 10000) / 90000  # Normalize to [0,1]
    ...
    volume / 1000000, 1.0  # Normalize volume
...
# Penalize if not 1.0
            weight_balance = 1.0 - unified_math.abs(weight_sum - 1.0)

price_quality = unified_math.min()
    btc_data.current_price / 50000.0,
        1.0  # Normalize BTC price
...
btc_data = trading.process_real_btc_price(50000.0, 1000000.0)
    safe_print()
        f"\\u2705 BTC Data: ${btc_data.current_price} -> 16 - bit: {btc_data.mapped_16bit}"
