...
class DemoMemoryCore:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        safe_print("🧠 Demo Memory Core initialized")
    
    def set_simulation_mode(self, mode: SimulationMode) -> None:
        """Set simulation mode."""
        self.simulation_mode = mode
        safe_print(f"🔄 Simulation mode set to: {mode.value}")
    ...
    def store_memory_entry(
        ...
        try:
            ...
            safe_print(f"✅ Memory entry stored: {hash_id[:8]}...")
            return hash_id
            
        except Exception as e:
            safe_print(f"❌ Memory storage failed: {safe_format_error(e, 'memory_storage')}")
            return ""
    ...
    def retrieve_memory_entry(
        ...
        try:
            ...
            safe_print(f"❌ Memory retrieval failed: {safe_format_error(e, 'memory_retrieval')}")
            return None
    ...
    def _generate_hash_id(self, tick_id: int, timestamp: datetime, market_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            safe_print(f"⚠️ Hash generation failed: {safe_format_error(e, 'hash_generation')}")
    ...
    def _calculate_confidence_score(
        ...
        try:
            ...
            profit_confidence = min(1.0, max(0.0, profit_result / 100.0))
            
            # Phase alignment confidence
            phase_confidence = 1.0 - abs(phase_compression)
            
            # Entropy stability confidence
            entropy_confidence = 1.0 - abs(entropy_field - 0.5) * 2.0
            
            # ZPE resonance confidence
            resonance_confidence = abs(zpe_resonance)
            ...
            safe_print(f"⚠️ Confidence calculation failed: {safe_format_error(e, 'confidence_calculation')}")
            return 0.5
    ...
    def _find_similar_memory(
        ...
        try:
            ...
            safe_print(f"⚠️ Similarity search failed: {safe_format_error(e, 'similarity_search')}")
            return None
    ...
    def _calculate_similarity_score(
        ...
        try:
            ...
            for asset in ['btc_price', 'eth_price', 'xrp_price']:
                if asset in current_conditions and asset in historical_conditions:
                    ...
                    if historical_price > 0:
                        price_diff = abs(current_price - historical_price) / historical_price
            ...
            for asset in ['volume_btc', 'volume_eth', 'volume_xrp']:
                if asset in current_conditions and asset in historical_conditions:
                    ...
                    if historical_volume > 0:
                        volume_diff = abs(current_volume - historical_volume) / historical_volume
                        volume_similarity = max(0.0, 1.0 - volume_diff)
            ...
            safe_print(f"⚠️ Similarity calculation failed: {safe_format_error(e, 'similarity_calculation')}")
            return 0.0
    ...
    def _cleanup_memory(self) -> None:
        ...
        try:
            ...
            safe_print(f"🗑️ Memory cleanup completed: {len(low_confidence_entries)} long-term, {len(old_lantern_entries)} lantern entries removed")
            
        except Exception as e:
            safe_print(f"⚠️ Memory cleanup failed: {safe_format_error(e, 'memory_cleanup')}")
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hit_rate': self.memory_hits / max(self.memory_hits + self.memory_misses, 1),
    ...
    def clear_memory(self, memory_type: Optional[MemoryType] = None) -> None:
        ...
        try:
            if memory_type is None:
                ...
                safe_print("🗑️ All memory cleared")
            else:
                ...
                if memory_type == MemoryType.SHORT_TERM:
                    ...
                    self.memory.lantern_memory.clear()
                safe_print(f"🗑️ {memory_type.value} memory cleared")
                
        except Exception as e:
            safe_print(f"⚠️ Memory clear failed: {safe_format_error(e, 'memory_clear')}")
...
if __name__ == "__main__":
    ...
    print("🧪 Testing Demo Memory Core...")
    ...
    print(f"✅ Memory entry stored: {hash_id}")
    ...
    if retrieved_entry:
        print(f"✅ Memory entry retrieved: {retrieved_entry.hash_id[:8]}...")
        print(f"   Profit Result: {retrieved_entry.profit_result}")
        print(f"   Confidence Score: {retrieved_entry.confidence_score:.3f}")
