...
class APIIntegrationManager:
    ...
    def _initialize_exchanges(self):
        ...
        try:
            ...
            error(f"❌ Failed to initialize exchanges: {e}")
    ...
    async def get_price_data(self, symbol: str, sources: List[APISource] = None) -> Optional[PriceData]:
        ...
        for source in sources:
            try:
                ...
                warn(f"⚠️ Failed to fetch price from {source.value}: {e}")
                continue
        
        error(f"❌ Failed to get price data for {symbol} from all sources")
        return None
    ...
    async def get_order_book(self, symbol: str, exchange: str = 'coinbase', depth: int = 20) -> Optional[OrderBookData]:
        ...
        try:
            ...
            order_book = await exchange_instance.fetch_order_book(f"{symbol}/USD", depth)
    ...
    async def calculate_position_size(self, symbol: str, entry_price: float, 
                                    risk_amount: float, stop_loss_pct: float) -> float:
...
class APIIntegrationCLI:
    ...
    async def initialize(self):
        ...
        try:
            ...
            error(f"❌ Failed to initialize API manager: {e}")
            return False
    ...
    async def cmd_price(self, symbol: str):
        ...
        info(f"💰 Fetching price for {symbol}...")
        price_data = await self.api_manager.get_price_data(symbol)
        
        if price_data:
            success(f"✅ {symbol} Price: ${price_data.price:,.2f}")
            info(f"   Source: {price_data.source}")
            info(f"   24h Change: {price_data.price_change_percent_24h:+.2f}%")
            info(f"   Volume: ${price_data.volume_24h:,.0f}")
        else:
    ...
    async def cmd_portfolio(self):
        ...
        if portfolio:
            success(f"✅ Portfolio Total: ${portfolio.total_balance:,.2f}")
            info(f"   Available: ${portfolio.available_balance:,.2f}")
            info(f"   Positions: {len(portfolio.positions)}")
            for asset, value in portfolio.positions.items():
                info(f"     {asset}: ${value:,.2f}")
        else:
    ...
    async def cmd_position_size(self, symbol: str, entry_price: float, risk_amount: float, stop_loss_pct: float):
        ...
        info(f"🧮 Calculating position size for {symbol}...")
        position_size = await self.api_manager.calculate_position_size(
            symbol, entry_price, risk_amount, stop_loss_pct
        )
        
        if position_size > 0:
            success(f"✅ Recommended position size: {position_size:.4f} {symbol}")
            info(f"   Entry price: ${entry_price:,.2f}")
            info(f"   Risk amount: ${risk_amount:,.2f}")
            info(f"   Stop loss: {stop_loss_pct:.1%}")
        else:
