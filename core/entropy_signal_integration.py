...
class EntropySignalIntegrator:
    ...
    def _calculate_entropy(self, bids: List[Tuple[float, float]], asks: List[Tuple[float, float]]) -> Dict[str, float]:
    ...
    def _update_performance_metrics(self, signal: EntropySignal) -> None:
        ...
        try:
            if hasattr(self, 'signal_history') and len(self.signal_history) > 0:
                ...
                successful_signals = sum(1 for signal in self.signal_history if getattr(signal, 'profit', 0) > 0)
