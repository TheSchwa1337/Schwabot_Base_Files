...
from .entropy_math import (
    bit_entropy, hamming_distance, vector_similarity, 
    shannon_entropy, get_backend_info
)
...
class VectorRegistry:
    ...
    def __init__(self, registry_path: Optional[str] = None):
        ...
        logger.info(f"Vector Registry initialized with {len(self.digest_vectors)} vectors")
    ...
    def register_digest(self, digest: bytes, strategy_vector: StrategyVector) -> bool:
        ...
        try:
            ...
            logger.debug(f"Registered digest {digest_hex[:16]}... with strategy {strategy_vector.strategy_id}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to register digest: {e}")
            return False
    
    def find_similar_digests(self, digest: bytes, threshold: float = 0.7, 
                           max_results: int = 10) -> List[DigestMatch]:
        ...
        try:
            ...
            logger.debug(f"Found {len(matches)} similar digests in {search_time:.4f}s")
            return matches
    ...
    def update_profit_outcome(self, digest: bytes, pnl: float, success: bool = None) -> bool:
        ...
        try:
            ...
            if digest_hex not in self.digest_vectors:
                logger.warning(f"Digest {digest_hex[:16]}... not found in registry")
                return False
            ...
            logger.debug(f"Updated digest {digest_hex[:16]}... success_rate={vector.success_rate:.3f}, avg_profit={vector.avg_profit:.4f}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to update profit outcome: {e}")
            return False
    ...
    def _gpu_similarity_search(self, query_digest: str, query_embedding: List[float], 
                             threshold: float, max_results: int) -> List[DigestMatch]:
    ...
    def _cpu_similarity_search(self, query_digest: str, query_embedding: List[float], 
                             threshold: float, max_results: int) -> List[DigestMatch]:
        """CPU similarity search."""
        matches = []
    ...
    def _load_registry(self):
        ...
        try:
            if self.registry_path.exists():
                ...
                logger.info(f"Loaded {len(self.digest_vectors)} vectors from registry")
