...
class EnhancedPortfolioTracker(PortfolioTracker):
    ...
    async def execute_rebalancing(self, rebalancing_actions: List[RebalancingAction]):
        ...
        try:
            if not rebalancing_actions:
                logger.info("No rebalancing actions to execute")
                return True
            
            logger.info(f"🔄 Executing {len(rebalancing_actions)} rebalancing actions")
            
            executed_actions = []
            total_cost = Decimal('0')
            
            # Sort actions by priority
            sorted_actions = sorted(rebalancing_actions, key=lambda x: x.priority)
            ...
            for action in sorted_actions:
                try:
                    ...
                    if not best_exchange:
                        logger.warning(f"No suitable exchange found for {action.symbol}")
                        continue
                    
                    # Calculate order parameters
                    current_price = self._get_current_price(action.symbol)
                    if not current_price:
                        logger.warning(f"No price data for {action.symbol}")
                        continue
                    ...
                    if result.get('success'):
                        ...
                        self.total_rebalancing_cost += Decimal(str(cost))
                        
                        logger.info(f"✅ Rebalancing order executed: {action.symbol} {action.action} {quantity:.6f}")
                        
                        # Notify callbacks
                        await self._notify_rebalancing_callbacks(action, result)
                        
                    else:
                        logger.error(f"❌ Rebalancing order failed: {result.get('error')}")
                        
                except Exception as e:
                    logger.error(f"❌ Error executing rebalancing action for {action.symbol}: {e}")
            
            self.last_rebalance_execution = time.time()
            
            logger.info(f"✅ Rebalancing completed: {len(executed_actions)}/{len(rebalancing_actions)} actions executed")
            return len(executed_actions) > 0
    ...
    async def _find_best_exchange(self, symbol: str):
        ...
        try:
            ...
            for exchange_name, connection in self.exchange_manager.connections.items():
                if connection.status == "CONNECTED":
                    try:
                        ...
                        symbol_exists = any(market['symbol'] == symbol for market in markets)
                        
                        if symbol_exists:
                            available_exchanges.append((exchange_name, connection))
                    except Exception as e:
                        logger.debug(f"Could not check {exchange_name} for {symbol}: {e}")
            ...
            return available_exchanges[0][1]
            
        except Exception as e:
            logger.error(f"Error finding best exchange for {symbol}: {e}")
            return None
    ...
    async def _notify_rebalancing_callbacks(self, action: RebalancingAction, result: Dict[str, Any]):
        ...
        for callback in self.rebalancing_callbacks:
            try:
                ...
                logger.error(f"Rebalancing callback error: {e}")
    ...
    def add_rebalancing_callback(self, callback: Callable[[RebalancingAction, Dict[str, Any]], None]):
