        ...
        try:
            ...
            if resonance_analysis["active_harmonics"]:
                avg_resonance = np.mean(
                    [h["resonance_strength"] for h in resonance_analysis["active_harmonics"]]
                )
                resonance_analysis["resonance_strength"] = avg_resonance

                avg_coherence = np.mean(
                    [h["coherence_product"] for h in resonance_analysis["active_harmonics"]]
                )
            ...
            for freq, wave in self.frequency_waves.items():
                if wave.coherence > 0.7 and abs(wave.profit_velocity) > 0.1 and wave.current_position > 0.8:  # Near wave peak
