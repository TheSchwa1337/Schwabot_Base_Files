#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schwabot Integrated Launcher - Complete System Integration.
...
6. Safe file path management and installation handling
"""
...
class SchwabotIntegratedLauncher:
    """Complete integrated launcher for Schwabot system."""
    ...
    def _create_dashboard_tab(self):
        ...
        self.system_status_text = tk.Text(
            status_frame, height=10, bg="#2a2a2a", fg="#00ff00"
        )
    ...
    def _create_api_management_tab(self):
        ...
        self.api_entries = {}
        self._create_api_service_ui(services_frame, "CoinMarketCap", "coinmarketcap", SecurityLevel.LOW)
        ...
        ttk.Button(crwm_frame, text="🌤️ Enable CRWM", command=self._toggle_crwm).pack(side="left", padx=5, pady=5)
        ttk.Button(crwm_frame, text="📈 View Weather Data", command=self._view_weather_data).pack(side="left", padx=5, pady=5)

    def _create_api_service_ui(self, parent, name: str, key: str, security_level: SecurityLevel):
        ...
        # Test button
        ttk.Button(service_frame, text="Test", command=lambda: self._test_api(key, status_var)).pack(side="left", padx=5)

        # Store references
        self.api_entries[key] = {"status_var": status_var, "api_key_var": api_key_var, "security_level": security_level}

    def _create_data_pipeline_tab(self):
        """Create data pipeline visualization tab."""
        ...
        ttk.Button(controls_frame, text="🗑️ Cleanup Pipeline", command=self._cleanup_pipeline).pack(side="left", padx=5, pady=5)
    ...
    def _create_settings_tab(self):
        ...
        ttk.Entry(path_frame, textvariable=self.install_path_var, width=60).pack(fill="x", padx=5, pady=2)

        ttk.Button(path_frame, text="📁 Change Path", command=self._change_install_path).pack(anchor="w", padx=5, pady=5)
        ...
        self.update_interval_var = tk.IntVar(value=self.config["visualization"]["update_interval_ms"])
        ttk.Scale(
            perf_frame,
            from_=500,
            to=5000,
        ...
        ttk.Button(settings_frame, text="💾 Save Settings", command=self._save_settings).pack(anchor="w", padx=10, pady=10)
    ...
    def _create_monitoring_tab(self):
        ...
        scrollbar = ttk.Scrollbar(metrics_text_frame, orient="vertical", command=self.metrics_text.yview)
    ...
    def _update_system_status(self):
        ...
        try:
            status_lines = []
            status_lines.append(f"🚀 Schwabot Status: {'Active' if self._is_system_active() else 'Inactive'}")
            status_lines.append(f"💰 Profit Engine: {'✅' if PROFIT_COMPONENTS_AVAILABLE else '❌'}")
            ...
            if self.profit_engine:
                ...
                status_lines.append(f"Optimizations: {perf.get('total_optimizations', 0)}")
                status_lines.append(f"Success Rate: {perf.get('success_rate', 0):.1%}")
    ...
    def _update_pipeline_visualization(self):
        """Update data pipeline visualization."""
        try:
            ...
            self.memory_canvas.create_rectangle(10, 50, 10 + short_width, 80, fill="#ff6b6b", outline="#000000")
            ...
            self.memory_canvas.create_rectangle(mid_x, 50, mid_x + mid_width, 80, fill="#ffd93d", outline="#000000")
            self.memory_canvas.create_text(mid_x + (canvas_width / 6), 35, text="Mid-term", fill="#00ff00")
            self.memory_canvas.create_text(
            ...
            self.memory_canvas.create_rectangle(long_x, 50, long_x + long_width, 80, fill="#6bcf7", outline="#000000")
            self.memory_canvas.create_text(long_x + (canvas_width / 6), 35, text="Long-term", fill="#00ff00")
            self.memory_canvas.create_text(
    ...
    def _update_metrics(self):
        ...
        try:
            ...
            if self.profit_engine:
                perf = self.profit_engine.get_performance_summary()
                metrics_lines.append(f"Profit Optimizations: {perf.get('total_optimizations', 0)}")
                metrics_lines.append(f"Average Confidence: {perf.get('avg_confidence', 0):.3f}")

            # API status
            if self.secure_api_manager:
                api_stats = self.secure_api_manager.get_api_statistics()
                metrics_lines.append(f"API Requests: {api_stats.get('total_requests', 0)}")
                metrics_lines.append(f"API Success Rate: {api_stats.get('success_rate', 0):.1%}")

            # CRWM status
            if self.chrono_mapper:
                weather_data = self.chrono_mapper.get_weather_signature("1h")
                if weather_data:
                    metrics_lines.append(f"Weather Gradient: {weather_data.get('price_gradient', 0):.4f}")
    ...
    def _generate_report(self):
        ...
        try:
            ...
            report_lines.append(f"  Profit Engine: {'Available' if PROFIT_COMPONENTS_AVAILABLE else 'Not Available'}")
            report_lines.append(f"  API Manager: {'Available' if API_COMPONENTS_AVAILABLE else 'Not Available'}")
            report_lines.append(f"  Data Pipeline: {'Available' if DATA_PIPELINE_AVAILABLE else 'Not Available'}")
            report_lines.append("")
            ...
            if self.memory_manager:
                ...
                report_lines.append(f"  Total Size: {usage.total_size_bytes / (1024 * 1024):.1f} MB")
                report_lines.append(f"  Compression Savings: {usage.compression_savings:.1f}%")
            report_lines.append("")
    ...
    def _refresh_status(self):
        ...
        self._add_recent_action("🔄 Status refreshed")
        self._update_system_status()
        self._update_pipeline_visualization()
        ...
        try:
            ...
            if self.secure_api_manager:
                ...
                if success:
                    ...
                    messagebox.showinfo("Success", f"API credentials for {api_key} stored securely")
                else:
    ...
    def _test_api(self, api_key: str, status_var: tk.StringVar):
        """Test API connection."""
        try:
    ...
    def _toggle_crwm(self):
        """Toggle ChronoResonance Weather Mapping."""
        try:
            if self.chrono_mapper:
                # Toggle CRWM status
    ...
    def _view_weather_data(self):
        """View CRWM weather data."""
        try:
            if self.chrono_mapper:
                weather_data = self.chrono_mapper.get_weather_signature("1h")
                if weather_data:
    ...
    def _view_short_term_data(self):
        """View short-term data."""
        self._view_data_by_type("short-term")

    def _view_mid_term_data(self):
        """View mid-term data."""
        self._view_data_by_type("mid-term")

    def _view_long_term_data(self):
        """View long-term data."""
        self._view_data_by_type("long-term")

    def _view_data_by_type(self, data_type: str):
        """View data by retention type."""
        try:
    ...
    def _get_api_type_from_key(self, api_key: str):
