...
try:
    from core.ghost_core import GhostCore, StrategyBranch, GhostState
...
class UnifiedTradingPipeline:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("🚀 Unified Trading Pipeline initialized with capital: $%.2f", self.initial_capital)
    ...
    def _initialize_components(self) -> None:
        ...
        self.ghost_core = GhostCore(
            memory_depth=ghost_config.get('memory_depth', 1000)
        )
    ...
    async def process_market_data(
        ...
        try:
            ...
            profit_vector = self.ccxt_integration.calculate_profit_vector(order_book, walls)
    ...
    async def _fetch_order_book_data(self, symbol: str) -> Optional[OrderBookSnapshot]:
        ...
        try:
            ...
            for exchange_id in self.ccxt_integration.exchanges.keys():
                order_book = await self.ccxt_integration.fetch_order_book(exchange_id, symbol)
                if order_book:
                    return order_book
    ...
    async def _generate_trading_decision(
        ...
        try:
            ...
            decision = TradingDecision(
                ...
                exchange=list(self.ccxt_integration.exchanges.keys())[0],  # Use first exchange
    ...
    def _calculate_optimal_quantity(
        ...
        try:
            ...
            quantity = base_quantity * brain_position_size * profit_multiplier * brain_confidence
            
            # Apply limits
            max_quantity = self.state.current_capital * 0.1 / order_book.mid_price  # Max 10% of capital
    ...
    def _calculate_decision_confidence(
        ...
        try:
            ...
            profit_confidence = min(1.0, profit_vector.get('wall_enhanced_profit', 0.0) * 1000)
    ...
    async def execute_trade(self, decision: TradingDecision) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                ...
                decision.action, decision.quantity, decision.symbol, 
                decision.price, profit
            )
    ...
    async def run_backtest(
        ...
        for i, (timestamp, price) in enumerate(price_series):
            ...
            # Log progress
            if i % 100 == 0:
                logger.info("Backtest progress: %d/%d (%.1f%%)", i, len(price_series), i/len(price_series)*100)
        ...
        results = {
            ...
            'winning_trades': sum(1 for trade in trades_executed if trade.get('profit', 0) > 0),
            'total_profit': sum(trade.get('profit', 0) for trade in trades_executed),
            'pipeline_status': self.get_pipeline_status()
        }
        
        logger.info("Backtest completed: %.2f%% return, %d trades", total_return*100, len(trades_executed))
        return results
...
async def demo_unified_pipeline():
    ...
    try:
        ...
        print(f"  Win Rate: {results['winning_trades']/max(results['total_trades'], 1):.1%}")
