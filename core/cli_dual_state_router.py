...
class DualStateRouterCLI:
    ...
    async def _test_system(self):
        ...
        info(f"Router test result: {router_result.get('compute_mode', 'unknown')}")
    ...
    async def show_system_status(self):
        ...
        info(f"  CPU Utilization: {performance.get('cpu_utilization', 0):.1f}%")
        info(f"  GPU Utilization: {performance.get('gpu_utilization', 0):.1f}%")
    ...
    async def show_profit_registry(self, limit: int = 10):
        ...
        for i, entry in enumerate(registry_entries, 1):
            ...
            info(f"   Preferred Mode: {entry.get('preferred_compute_mode', 'Unknown')}")
    ...
    async def process_btc_price(self, price: float, volume: float, volatility: float):
        ...
        for strategy in strategies:
            ...
            info(f"  Compute Mode: {routing_result.get('compute_mode', 'Unknown')}")
    ...
    async def show_strategy_performance(self, strategy_name: Optional[str] = None):
        ...
        if strategy_name:
            ...
            if performance:
                ...
                info(f"Total Executions: {performance.get('total_executions', 0)}")
                info(f"Success Rate: {performance.get('success_rate', 0):.1f}%")
                info(f"Average Profit: {performance.get('avg_profit', 0):.3f}")
                info(f"Preferred Compute Mode: {performance.get('preferred_compute_mode', 'Unknown')}")
                info(f"Last Execution: {performance.get('last_execution', 'Unknown')}")
            else:
            ...
            for strategy, metrics in all_performance.items():
                ...
                info(f"  Compute Mode: {metrics.get('preferred_compute_mode', 'Unknown')}")
...
async def main():
    ...
    parser.add_argument("--btc", nargs=3, metavar=("PRICE", "VOLUME", "VOLATILITY"), type=float, help="Process BTC price data")
    ...
    if args.init or any([args.status, args.registry, args.test, args.btc, args.performance, args.reset, args.interactive]):
