...
class MathematicalDecisionEngine:
        ...
        try:
            ...
            math_signal = MathSignal(
                ...
                metadata={
                    ...
                    'tensor_result': tensor_result.tolist() if hasattr(tensor_result, 'tolist') else str(tensor_result)
                }
            )
            # Update history
    ...
    def _create_price_tensor(self, price_data: np.ndarray) -> np.ndarray:
        ...
        if len(price_data.shape) == 1:
            return price_data.reshape(-1, 1, 1)
        return price_data

    def _create_volume_tensor(self, volume_data: np.ndarray) -> np.ndarray:
        """Create volume tensor from volume data."""
        if len(volume_data.shape) == 1:
            return volume_data.reshape(-1, 1, 1)
        return volume_data

    def _compute_collapse_function(self, price_tensor: np.ndarray, volume_tensor: np.ndarray) -> float:
        """Compute collapse function for tensors."""
        try:
            price_norm = np.linalg.norm(price_tensor)
            volume_norm = np.linalg.norm(volume_tensor)
            if price_norm == 0 or volume_norm == 0:
                return 0.0
            return price_norm / volume_norm
        except Exception:
            return 0.0
    ...
    def _classify_tensor_state(self, eigenvalues: np.ndarray, tensor_norm: float,
        ...
        if tensor_norm < self.config.norm_threshold:
        ...
        elif np.std(eigenvalues) < self.config.eigenvalue_threshold:
            return TensorState.STABLE_TENSOR
        elif collapse_function > self.config.collapse_threshold:
    ...
    def _make_math_decision(self, tensor_state: TensorState, eigenvalues: np.ndarray,
                            tensor_norm: float, cosine_similarity: float, collapse_function: float) -> MathDecision:
        """Make mathematical trading decision."""
        if tensor_state == TensorState.COLLAPSING_TENSOR:
            return MathDecision.EMERGENCY_TENSOR_COLLAPSE
        elif tensor_state == TensorState.ALIGNING_TENSOR and cosine_similarity > self.config.tensor_alignment_threshold:
            return MathDecision.ENTER_TENSOR_ALIGNMENT
        elif tensor_state == TensorState.STABLE_TENSOR and tensor_norm > self.config.tensor_stability_threshold:
            return MathDecision.HOLD_TENSOR_STABILITY
        elif tensor_state == TensorState.DECOMPOSING_TENSOR and collapse_function > self.config.tensor_decomposition_threshold:
    ...
    def _calculate_math_confidence(self, eigenvalues: np.ndarray, tensor_norm: float,
        ...
        """Calculate confidence level for mathematical analysis."""
        confidence = 0.0
        if np.std(eigenvalues) < self.config.eigenvalue_threshold:
            confidence += 0.3
        if tensor_norm > self.config.norm_threshold:
            confidence += 0.3
        confidence += min(abs(cosine_similarity), 1.0) * 0.2
        confidence += min(abs(collapse_function), 1.0) * 0.2
        return min(confidence, 1.0)

    def _calculate_math_risk(self, eigenvalues: np.ndarray, tensor_norm: float,
                             cosine_similarity: float, collapse_function: float) -> float:
        """Calculate risk level for mathematical analysis."""
        risk = 0.0
        if np.std(eigenvalues) > self.config.eigenvalue_threshold:
            risk += 0.3
        if tensor_norm < self.config.norm_threshold:
            risk += 0.3
        if abs(cosine_similarity) < 0.2:
            risk += 0.2
        if collapse_function > self.config.collapse_threshold:
            risk += 0.2
        return min(risk, 1.0)
