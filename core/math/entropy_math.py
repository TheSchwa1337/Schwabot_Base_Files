...
class EntropyMath:
    ...
    def calculate_zbe_entropy(self, price_changes: List[float]) -> float:
        ...
        try:
            ...
            total_positive = np.sum(positive_changes) if len(positive_changes) > 0 else 0
            total_negative = np.sum(np.abs(negative_changes)) if len(negative_changes) > 0 else 0

            if total_positive == 0 and total_negative == 0:
                return 0.0

            # Calculate ZBE
            zbe = total_positive / (total_positive + total_negative) if (total_positive + total_negative) > 0 else 0.5
