...
class MatrixOverlay:
    ...
    def __init__(
        self, history_size: int = 256, low_frequency_threshold: float = 0.05, confidence_threshold: float = 0.7
    ):
        """
...
if __name__ == "__main__":
    ...
    # Example Usage
    matrix_overlay = MatrixOverlay(history_size=64, low_frequency_threshold=0.08, confidence_threshold=0.5)
    ...
    for i in range(200):
        alignment = 0.5 + 0.2 * np.sin(t_vals[i] / (2 * np.pi * 5)) + np.random.normal(0, 0.05)  # Slow sine + noise
        drift = 0.1 + 0.05 * np.cos(t_vals[i] / (2 * np.pi * 2)) + np.random.normal(0, 0.02)  # Faster sine + noise
