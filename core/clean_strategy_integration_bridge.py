...
async def _create_integrated_signal(self, wall_street_signal: Dict[str, Any], mathematical_analysis: Dict[str, Any], asset: str, price: float, volume: float) -> Optional[IntegratedTradingSignal]:
        ...
        try:
            ...
            composite_confidence = (ws_confidence * ws_weight) + (math_confidence * math_weight) + (risk_factor * risk_weight)

            # Risk scoring
            risk_score = risk_assessment.get("risk_score", 0.5)

            # Position sizing based on integrated analysis
            position_sizing = self._calculate_integrated_position_sizing(wall_street_signal, mathematical_analysis, composite_confidence)

            # Execution priority based on signal quality and correlation
            execution_priority = self._calculate_execution_priority(wall_street_signal, correlation_score, composite_confidence)
