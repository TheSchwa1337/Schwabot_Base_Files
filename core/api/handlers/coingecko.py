...
class CoinGeckoHandler(BaseAPIHandler):
    ...
    async def _fetch_price_data(self, headers: Dict) -> Dict:
        ...
        try:
            ...
            if aiohttp:
                session = await self._get_session()
                async with session.get(f"{BASE_URL}/simple/price", params=params, headers=headers) as resp:
                    resp.raise_for_status()
                    return await resp.json()
            elif requests:
                loop = asyncio.get_running_loop()
                response = await loop.run_in_executor(
                    None,
                    lambda: requests.get(f"{BASE_URL}/simple/price", params=params, headers=headers, timeout=15)
                )
                response.raise_for_status()
                return response.json()
        except Exception as e:
    ...
    async def _fetch_trending_data(self, headers: Dict) -> Dict:
        ...
        try:
            if aiohttp:
                ...
                response = await loop.run_in_executor(
                    None,
                    lambda: requests.get(f"{BASE_URL}/search/trending", headers=headers, timeout=15)
                )
                response.raise_for_status()
                return response.json()
        except Exception as e:
    ...
    async def _fetch_dominance_data(self, headers: Dict) -> Dict:
        ...
        try:
            ...
            if aiohttp:
                session = await self._get_session()
                async with session.get(f"{BASE_URL}/coins/markets", params=params, headers=headers) as resp:
                    resp.raise_for_status()
                    return await resp.json()
            elif requests:
                loop = asyncio.get_running_loop()
                response = await loop.run_in_executor(
                    None,
                    lambda: requests.get(f"{BASE_URL}/coins/markets", params=params, headers=headers, timeout=15)
                )
                response.raise_for_status()
                return response.json()
        except Exception as e:
    ...
    async def _parse_raw(self, raw: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if "dominance" in raw and isinstance(raw["dominance"], list):
                total_market_cap = sum(coin.get("market_cap", 0) for coin in raw["dominance"][:10])
                for coin in raw["dominance"][:10]:
                    coin_id = coin.get("id", "")
                    market_cap = coin.get("market_cap", 0)
                    dominance_percent = (market_cap / total_market_cap * 100) if total_market_cap > 0 else 0
    ...
    def _calculate_market_sentiment(self, data: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            btc_dominance = data.get("global_metrics", {}).get("market_cap_percentage", {}).get("btc", 50)
            if btc_dominance > 60:
