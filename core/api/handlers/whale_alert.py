...
class WhaleAlertHandler(BaseAPIHandler):
    ...
    async def _parse_raw(self, raw: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"{self.NAME}: failed to parse Whale Alert data – {exc}")
            return {
                "timestamp": int(time.time()),
                "transactions": [],
                "blockchain_stats": {},
    ...
    def _calculate_blockchain_stats(self, transactions: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for tx in transactions:
                ...
                type_amounts[tx_type] = type_amounts.get(tx_type, 0) + tx["amount_usd"]
    ...
    def _calculate_whale_activity_scores(self, transactions: List[Dict]) -> Dict[str, float]:
        ...
        try:
            ...
            activity_level = min(100, (tx_count / 50) * 100)  # Normalize to 50 transactions = 100%
            
            # Volume intensity based on total volume
            total_volume = sum(tx["amount_usd"] for tx in transactions)
            volume_intensity = min(100, (total_volume / 1000000000) * 100)  # Normalize to $1B = 100%
            ...
            frequency_score = min(100, (transactions_per_hour / 10) * 100)  # Normalize to 10 tx/hour = 100%
            
            # Size score based on average transaction size
            avg_size = total_volume / tx_count if tx_count > 0 else 0
            size_score = min(100, (avg_size / 10000000) * 100)  # Normalize to $10M avg = 100%
            ...
            logger.error(f"Error calculating whale activity scores: {e}")
            return {
                "activity_level": 0.0,
                "volume_intensity": 0.0,
                "frequency_score": 0.0,
    ...
    def _calculate_market_impact(self, transactions: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            buying_pressure = (buying_volume / total_volume * 100) if total_volume > 0 else 0
            selling_pressure = (selling_volume / total_volume * 100) if total_volume > 0 else 0
            
            # Net flow (positive = net buying, negative = net selling)
            net_flow = buying_volume - selling_volume
            
            # Overall impact score
            impact_score = min(100, (total_volume / 500000000) * 100)  # Normalize to $500M = 100%
    ...
    def get_thermal_integration_data(self) -> Dict[str, Any]:
        ...
        try:
            if not hasattr(self, '_last_parsed_data'):
                return {"thermal_ready": False, "error": "No data available"}
            
            data = self._last_parsed_data
            whale_scores = data.get("whale_activity_scores", {})
            market_impact = data.get("market_impact_analysis", {})
            ...
            logger.error(f"Error getting thermal integration data: {e}")
            return {"thermal_ready": False, "error": str(e)}

    def get_profit_scheduler_data(self) -> Dict[str, Any]:
        """Get data formatted for profit scheduler integration."""
        try:
            if not hasattr(self, '_last_parsed_data'):
                return {"scheduler_ready": False, "error": "No data available"}
            ...
            logger.error(f"Error getting profit scheduler data: {e}")
            return {"scheduler_ready": False, "error": str(e)}
    ...
    def _calculate_whale_signal(self, market_impact: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Determine whale signal based on pressure and impact
            if impact_score < 20:
            ...
            logger.error(f"Error calculating whale signal: {e}")
            return "neutral" 