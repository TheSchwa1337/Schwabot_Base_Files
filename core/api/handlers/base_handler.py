...
class BaseAPIHandler(ABC):Abstract base-class for external API handlers.# --- Class-level configuration -------------------------------------------------
    ...
    async def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        Return cached data, refreshing from the remote API if needed.if force_refresh or (time.time() - self._last_refresh > self.REFRESH_INTERVAL):
            try: raw = await self._fetch_raw()
    ...
    async def _fetch_raw(self) -> Any:  # pragma: no cover – implemented by subclass
        Fetch raw data from the remote API (network call).async def _parse_raw(self, raw: Any) -> Dict[str, Any]:Transform raw payload into a normalised JSON-serialisable dict.

        Sub-classes may override for custom parsing.  The default
        implementation assumes the payload is already JSON-compatible.return raw  # type: ignore[return-value]
    ...
    async def _write_cache(self, data: Dict[str, Any]) -> None: path = self._cache_file
