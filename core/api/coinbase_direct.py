...
class CoinbaseDirectAPI:
        ...
        try:
            ...
            async with session.get(f"{self.base_url}{path}") as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to get ticker for {product_id}: {response.status}")
                    return None
        ...
        try:
            ...
            async with session.get(f"{self.base_url}{path}", headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to get order {order_id}: {response.status}")
                    return None
    ...
    async def get_open_orders(self, product_id: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        """Get open orders."""
        try:
            session = await self._get_session()
        ...
        try:
            ...
            async with session.get(f"{self.base_url}{path}") as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to get candles for {product_id}: {response.status}")
                    return None
