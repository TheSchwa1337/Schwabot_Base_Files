...
class ApiIntegrationManager:
    ...
    def load_configuration(self):
        ...
        try:
            if not self.config_path.exists():
                logger.warning(f"Configuration file not found: {self.config_path}. No exchanges will be loaded.")
                return
    ...
    async def _heartbeat_check(self):
        ...
        for name, conn in self.connections.items():
            if conn.status == ConnectionStatus.ERROR or \
               (conn.status == ConnectionStatus.CONNECTED and time.time() - conn.last_heartbeat > self.heartbeat_interval):
                
                if conn.status != ConnectionStatus.RECONNECTING:
                    logger.warning(f"Connection issue with {name}. Attempting to reconnect...")
    ...
    async def get_market_data(self, exchange_name: str, symbol: str) -> Optional[MarketData]:
        ...
        logger.warning(f"Cannot get market data: exchange '{exchange_name}' is not available.")
        return None
        ...
        return {
            ...
            'connections': {
                name: {
                    ...
                    'last_heartbeat': datetime.fromtimestamp(conn.last_heartbeat).isoformat() if conn.last_heartbeat else None,
