...
class CleanTradingPipeline:
    ...
    def __init__(
        ...
        logger.info(f"Clean Trading Pipeline initialized with ${initial_capital:,.2f} capital")
    ...
    async def process_market_data(self, market_data: MarketData) -> Optional[TradingDecision]:
        ...
        try:
            ...
            if optimal_strategy != self.state.active_strategy:
                logger.info(f"Strategy switch: {self.state.active_strategy.value} -> {optimal_strategy.value}")
    ...
    def _analyze_market_regime(self, market_data: MarketData) -> MarketRegime:
        ...
        try:
            ...
            trend_strength = abs(trend_direction) / np.std(recent_prices) if np.std(recent_prices) > 0 else 0
    ...
    def _generate_trading_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Optional[Dict[str, Any]]:
    ...
    def _mean_reversion_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
    ...
    def _momentum_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
    ...
    def _arbitrage_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
    ...
    def _scalping_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
        ...
        short_term_change = (recent_prices[-1] - recent_prices[-3]) / recent_prices[-3] if recent_prices[-3] > 0 else 0
        ...
        if abs(short_term_change) > 0.005:  # 0.5% quick movement
            ...
            quantity = self.state.current_capital * 0.2 * confidence  # Larger positions for quick trades
        else:
            action = TradingAction.HOLD
            confidence = 0
            quantity = 0
    ...
    def _swing_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
        ...
        return {
            ...
            "quantity": quantity,
            "confidence": confidence * profit_vector.confidence_score,
            "metadata": {"position_in_range": position_in_range, "range_high": high, "range_low": low}
        }
    ...
    def _default_signal(self, market_data: MarketData, profit_vector: ProfitVector) -> Dict[str, Any]:
    ...
    def _apply_risk_management(self, signal: Dict[str, Any], market_data: MarketData) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if market_data.volatility > self.risk_params.volatility_threshold:
                ...
                logger.info(f"Position size reduced due to high volatility: {market_data.volatility}")
            
            # Check daily loss limits
            daily_pnl = self._calculate_daily_pnl()
            if daily_pnl < -self.state.current_capital * self.risk_params.max_daily_loss:
    ...
    try:
        while (time.time() - start_time) < duration_seconds:
            ...
            if decision:
                ...
                logger.info(f"Decision: {decision.action.value} {decision.quantity:.2f} @ {decision.price:.2f}")
