...
class StrategyTriggerRouter:
    ...
    async def process_market_data(self, 
                                ...
                                force_analysis: bool = False) -> List[TriggerEvent]:
        """
        ...
        try:
            ...
            two_gram_signals = await self.two_gram_detector.analyze_sequence(
                market_sequence, 
                context=market_data
            )
            ...
            for trigger in trigger_events:
                trigger_id = f"{trigger.trigger_type.value}_{trigger.strategy_name}_{trigger.timestamp}"
                self.active_triggers[trigger_id] = trigger
            
            # Update statistics
            self.trigger_statistics["total_triggers"] += len(trigger_events)
            self.trigger_statistics["pattern_triggers"] += len([t for t in trigger_events if t.trigger_type == TriggerType.TWO_GRAM_PATTERN])
            
            if trigger_events:
                info(f"🎯 Generated {len(trigger_events)} strategy triggers from market data")
                for trigger in trigger_events:
                    debug(f"  {trigger.trigger_type.value}: {trigger.strategy_name} (priority: {trigger.execution_priority})")
            
            return trigger_events
            
        except Exception as e:
            error(f"Error processing market data for triggers: {e}")
            return []
    ...
    async def _create_trigger_from_2gram_signal(self, 
                                              signal: TwoGramSignal,
                                              market_data: Dict[str, Any]) -> Optional[TriggerEvent]:
        ...
        try:
            ...
            if not strategy_config:
                warn(f"Unknown strategy: {signal.strategy_trigger}")
                return None
            ...
            logger.error(f"Error creating trigger from 2-gram signal: {e}")
            return None

    async def _create_t_cell_trigger(self, 
        ...
        try:
            ...
            warn(f"🛡️ T-cell trigger activated: {health_check.get('anomalies', [])}")
            
            return trigger_event
            
        except Exception as e:
            logger.error(f"Error creating T-cell trigger: {e}")
            return None

    async def _check_phantom_zone_triggers(self, 
                                         market_data: Dict[str, Any],
        ...
        try:
            ...
            logger.error(f"Error checking phantom zone triggers: {e}")

    async def _check_portfolio_triggers(self, 
                                      market_data: Dict[str, Any],
                                      trigger_events: List[TriggerEvent]):
        """Check for portfolio rebalancing triggers."""
        try:
            ...
            logger.error(f"Error checking portfolio triggers: {e}")
    ...
    async def execute_trigger(self, trigger_event: TriggerEvent) -> ExecutionResult:
        ...
        try:
            info(f"🎯 Executing trigger: {trigger_event.strategy_name} ({trigger_event.trigger_type.value})")
            ...
            trigger_id = f"{trigger_event.trigger_type.value}_{trigger_event.strategy_name}_{trigger_event.timestamp}"
            self.active_triggers.pop(trigger_id, None)
            
            if execution_result.execution_success:
                success(f"✅ Strategy executed successfully: {trigger_event.strategy_name}")
            else:
                warn(f"⚠️ Strategy execution failed: {trigger_event.strategy_name}")
            
            return execution_result
            
        except Exception as e:
            error(f"Error executing trigger: {e}")
    ...
    async def _execute_2gram_strategy(self, trigger_event: TriggerEvent) -> Dict[str, Any]:
        ...
        try:
            ...
            router_result = await self.dual_state_router.route_task(
                f"2gram_{trigger_event.strategy_name}",
                strategy_metadata
            )
            ...
            if (self.btc_usdc_integration and 
                trigger_event.asset_target == "BTC/USDC" and
                self.execution_mode == ExecutionMode.LIVE_TRADING):
            ...
            logger.error(f"Error executing 2-gram strategy: {e}")
    ...
    async def _execute_phantom_strategy(self, trigger_event: TriggerEvent) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error executing phantom strategy: {e}")
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            return {
                ...
                "system_health_impact": {
                    ...
                    "trading_restrictions": protection_actions
                },
                "profit_estimate": 0.0,
            ...
            logger.error(f"Error executing protection strategy: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def get_router_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_executions = self.trigger_statistics["successful_executions"] + self.trigger_statistics["failed_executions"]
            success_rate = (self.trigger_statistics["successful_executions"] / total_executions) if total_executions > 0 else 0.0
            ...
            return {
                ...
                "avg_execution_time": np.mean([e.execution_time_ms for e in recent_executions]) if recent_executions else 0.0
            }
            
        except Exception as e:
            logger.error(f"Error getting router statistics: {e}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error in router health check: {e}")
            return {"router_status": "error", "error": str(e)}
...
async def test_strategy_trigger_router():
    ...
    for trigger in triggers:
        print(f"  {trigger.trigger_type.value}: {trigger.strategy_name}")
        print(f"    Priority: {trigger.execution_priority}, Risk: {trigger.risk_level}")
        print(f"    Confidence: {trigger.confidence_score:.2f}")
    
    # Execute triggers
    for trigger in triggers:
        result = await router.execute_trigger(trigger)
        print(f"\nExecution result for {trigger.strategy_name}:")
        print(f"  Success: {result.execution_success}")
        print(f"  Time: {result.execution_time_ms:.1f}ms")
        print(f"  Output: {result.strategy_output}")
    
    # Get statistics
    stats = await router.get_router_statistics()
    print(f"\nRouter Statistics:")
    print(f"  Success rate: {stats['success_rate']:.1%}")
    print(f"  Total triggers: {stats['trigger_statistics']['total_triggers']}")
    print(f"  Active triggers: {stats['active_triggers']}")
    
    # Health check
    health = await router.health_check()
    print(f"\nHealth Status: {health['router_status']}")
