...
class ComprehensiveAnomalyFilter:
    ...
    def detect_all_anomalies(self, current_state: SystemState) -> List[AnomalySignal]:
        ...
        try:
            ...
            math_anomalies = self.math_protector.check_mathematical_safety(current_state)
            all_anomalies.extend(math_anomalies)
            
            # 4. Execution anomalies
            exec_anomalies = self.execution_monitor.detect_execution_issues(current_state)
            all_anomalies.extend(exec_anomalies)
            
            # 5. Portfolio state anomalies
            portfolio_anomalies = self.portfolio_guardian.check_portfolio_health(current_state)
            ...
            self.detection_history = [
                signal for signal in self.detection_history 
                if signal.detected_at > cutoff_time
            ]
            
            detection_time = time.time() - start_time
            logger.debug(f"🔍 Anomaly detection completed in {detection_time:.4f}s, found {len(all_anomalies)} anomalies")
    ...
    def should_block_execution(self, anomalies: List[AnomalySignal]) -> Tuple[bool, str]:
        ...
        if critical_anomalies:
            return True, f"Critical anomalies detected: {[a.anomaly_type.value for a in critical_anomalies]}"
        
        # Check for high severity anomalies
        high_severity_count = len([a for a in anomalies if a.severity == 'high'])
        if high_severity_count >= 3:
            return True, f"Multiple high-severity anomalies detected: {high_severity_count}"
    ...
    def get_recommended_actions(self, anomalies: List[AnomalySignal]) -> List[str]:
        ...
        for anomaly in anomalies:
            if anomaly.recommended_action:
                actions.append(f"{anomaly.anomaly_type.value}: {anomaly.recommended_action}")
                    ...
                    if last_price > 0:
                        ...
                        if price_change > self.price_jump_threshold:
                            anomalies.append(AnomalySignal(
                                anomaly_type=AnomalyType.DATA_FEED_CORRUPTION,
                                severity='high',
                                confidence=min(price_change / self.price_jump_threshold, 1.0),
                                detected_at=current_time,
...
class MathematicalSafetyProtector:
    ...
    def check_mathematical_safety(self, state: SystemState) -> List[AnomalySignal]:
        ...
        for matrix_name, condition_number in state.matrix_conditions.items():
            if condition_number > self.matrix_condition_limit:
                anomalies.append(AnomalySignal(
                    anomaly_type=AnomalyType.MATHEMATICAL_SINGULARITY,
                    severity='high',
                    confidence=min(condition_number / self.matrix_condition_limit, 1.0),
                    detected_at=current_time,
        ...
        for component, confidence in state.confidence_scores.items():
            if not (self.confidence_validation_range[0] <= confidence <= self.confidence_validation_range[1]):
                anomalies.append(AnomalySignal(
                    anomaly_type=AnomalyType.MATHEMATICAL_SINGULARITY,
                    severity='medium',
                    confidence=1.0,
...
class ExecutionAnomalyMonitor:
    ...
    def detect_execution_issues(self, state: SystemState) -> List[AnomalySignal]:
        ...
        if state.execution_latencies:
            recent_latency = np.mean(state.execution_latencies[-10:]) * 1000  # Convert to ms
            
            if recent_latency > self.normal_latency_ms * self.latency_spike_threshold:
                anomalies.append(AnomalySignal(
                    anomaly_type=AnomalyType.EXECUTION_TIMING,
                    severity='medium',
                    confidence=min(recent_latency / (self.normal_latency_ms * self.latency_spike_threshold), 1.0),
                    detected_at=current_time,
        ...
        if state.rejection_rate > self.rejection_rate_threshold:
            anomalies.append(AnomalySignal(
                anomaly_type=AnomalyType.EXECUTION_TIMING,
                severity='high',
                confidence=min(state.rejection_rate / self.rejection_rate_threshold, 1.0),
                detected_at=current_time,
...
class PortfolioStateGuardian:
    ...
    def check_portfolio_health(self, state: SystemState) -> List[AnomalySignal]:
            ...
            if total_portfolio_value > 0:
                ...
                if margin_ratio < self.margin_cushion_threshold:
                    anomalies.append(AnomalySignal(
                        ...
                        severity='high',
                        confidence=1.0 - (margin_ratio / self.margin_cushion_threshold),
                        detected_at=current_time,
        ...
        if state.unrealized_pnl < 0:
            drawdown_pct = abs(state.unrealized_pnl) / max(sum(abs(pos) for pos in state.positions.values()), 1)
            
            if drawdown_pct > self.drawdown_alert_threshold:
                anomalies.append(AnomalySignal(
                    anomaly_type=AnomalyType.PORTFOLIO_STATE,
                    severity='high',
                    confidence=min(drawdown_pct / self.drawdown_alert_threshold, 1.0),
                    detected_at=current_time,
...
def validate_system_safety(current_state: SystemState) -> Tuple[bool, List[AnomalySignal], List[str]]:
...
if __name__ == "__main__":
    ...
    safe_to_execute, detected_anomalies, recommended_actions = validate_system_safety(test_state)
