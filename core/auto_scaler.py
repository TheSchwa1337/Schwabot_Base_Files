...
# Calculate confidence multiplier
            confidence_excess = unified_math.max()
                0.0, dlt_adjusted_confidence - confidence_threshold
...
# Combine multipliers with mathematical weighting
            scale_factor = base_scale * \
                (1.0 + confidence_multiplier + profit_multiplier)
...
# Apply risk management constraints
            ...
            risk_limited_position = unified_math.min()
                scaled_position, max_position

# Apply minimum position constraint
            final_position = unified_math.max()
                MIN_POSITION_SIZE, risk_limited_position
...
# Test scaling calculations
    ...
    safe_print("\\u1f9ee Testing Auto Scaler with DLT Integration")
    safe_print("=" * 50)

    for confidence, profit, description in test_cases:
# Validate inputs
        if not scaler.validate_scaling_inputs()
                confidence, profit, base_position, account_balance:
...
# Get performance summary
    ...
    safe_print()
        f"   Average Scale Factor: {"}
            summary['average_scale_factor']:.3f""
    safe_print()
        f"   Scaling Applied Rate: {"}
            summary['scaling_applied_rate']:.2%""
