...
class DualStateRouter:
    ...
    def register_strategy(self, strategy_metadata: StrategyMetadata):
    ...
    def select_compute_mode(self, strategy_metadata: StrategyMetadata) -> ComputeMode:
        ...
        try:
            ...
            if routing_score >= self.zbe_threshold:
                ...
                reasoning = f"High routing score ({routing_score:.3f}) >= ZBE threshold ({self.zbe_threshold})"
            elif routing_score >= self.zpe_threshold:
                selected_mode = ComputeMode.ZPE
                reasoning = f"Medium routing score ({routing_score:.3f}) >= ZPE threshold ({self.zpe_threshold})"
            else:
                # Default to preferred mode or ZPE
                selected_mode = strategy_metadata.preferred_mode or ComputeMode.ZPE
                reasoning = f"Low routing score ({routing_score:.3f}), using preferred mode"
            
            # Load balancing override
            if self.load_balancing:
                selected_mode = self._apply_load_balancing(selected_mode, routing_score)
    ...
    def _apply_load_balancing(self, selected_mode: ComputeMode, routing_score: float) -> ComputeMode:
        """
        ...
        try:
            ...
            if selected_mode == ComputeMode.ZBE and zbe_load > zpe_load * (1 + load_threshold):
    ...
    def route_strategy(self, strategy_id: str, input_data: Dict[str, Any] = None) -> RoutingDecision:
        ...
        try:
            ...
            # Select compute mode
            selected_mode = self.select_compute_mode(strategy_metadata)
            ...
            self.logger.error(f"Error routing strategy: {e}")
            self.state = RouterState.ERROR
    ...
    def execute_strategy(self, strategy_id: str, input_data: Dict[str, Any] = None) -> ExecutionResult:
        ...
        try:
            ...
            # Route the strategy
            routing_decision = self.route_strategy(strategy_id, input_data)
            
            # Execute based on selected mode
            if routing_decision.selected_mode == ComputeMode.ZPE:
                success, result_data = self._execute_zpe_mode(strategy_id, input_data)
            else:  # ZBE mode
                success, result_data = self._execute_zbe_mode(strategy_id, input_data)
    ...
    def _execute_zpe_mode(self, strategy_id: str, input_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error in ZPE execution: {e}")
    ...
    def _execute_zbe_mode(self, strategy_id: str, input_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            time.sleep(0.005)  # Simulate faster computation time
            ...
            self.logger.error(f"Error in ZBE execution: {e}")
            return False, {'error': str(e)}
    ...
    def _update_performance_metrics(self, result: ExecutionResult):
        ...
        try:
            ...
            new_avg_time = (current_avg_time * metrics['total_executions'] + result.execution_time_ms) / total_executions
            
            # Update success rate
            successful_executions = int(current_success_rate * metrics['total_executions']) + (1 if result.success else 0)
    ...
    def get_router_summary(self) -> Dict[str, Any]:
        ...
        for mode in ComputeMode:
            mode_distribution[mode.value] = sum(1 for r in self.routing_history if r.selected_mode == mode)
        
        # Tier distribution
        tier_distribution = {}
        for tier in StrategyTier:
            tier_distribution[tier.value] = sum(1 for s in self.strategies.values() if s.tier == tier)
