...
# Calculate confidence based on state convergence
        confidence_score = unified_math.min()
            1.0, total_weight / len(self.active_states)
...
# Recursive structure
        recursive_component = self.recursive_hash_structure()
...
# Apply fractal weighting
        fractal_weight = self.calculate_fractal_command_weight()
            state.recursive_depth
# Normalize by fractal weight (higher depth = lower collapse)
# probability
