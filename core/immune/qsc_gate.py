...
class QSCGate:
    ...
    def immune_trigger(self, signal_data: ImmuneSignalData) -> Tuple[bool, float, Dict[str, Any]]:
    ...
    def _update_adaptive_threshold(self, activation_strength: float, triggered: bool) -> None:
        """Update adaptive threshold based on recent performance."""
        # Simple adaptive mechanism - adjust threshold based on activation frequency
        recent_triggers = sum(1 for r in self.response_history[-50:] 
                            if hasattr(r, 'trigger_strength') and r.trigger_strength > 0.5)
        
        trigger_rate = recent_triggers / min(50, len(self.response_history)) if self.response_history else 0.5
        ...
        logger.debug(f"🧬 Adaptive tau updated: {self.adaptive_tau:.3f} (trigger rate: {trigger_rate:.3f})")
    
    def process_immune_response(self, signal_data: ImmuneSignalData, 
                              swarm_vector: Optional[np.ndarray] = None,
                              gts_sync_score: Optional[float] = None) -> QSCImmuneResponse:
        ...
        if activation_strength < 0.2:
            level = ImmuneResponseLevel.DORMANT
        ...
        decision = self._make_immune_decision(level, activation_strength, gts_sync_score)
    ...
    def _make_immune_decision(self, level: ImmuneResponseLevel, 
                            activation_strength: float, 
                            gts_sync_score: float) -> str:
    ...
    def get_immune_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_performance": {
                ...
                "avg_trigger_strength": np.mean(trigger_strengths) if trigger_strengths else 0.0,
                "avg_gts_sync": np.mean(gts_sync_scores) if gts_sync_scores else 0.0,
                "activation_distribution": {
                    level: activation_levels.count(level) for level in set(activation_levels)
                }
            },
            "configuration": self.config,
            "last_signal": {
                "timestamp": self.signal_history[-1].timestamp if self.signal_history else None,
                "source": self.signal_history[-1].source if self.signal_history else None
            }
        }
    
    def update_performance_feedback(self, response_id: str, was_successful: bool) -> None:
        ...
        logger.debug(f"🧬 Performance feedback: success_rate={success_rate:.3f}, learning_rate={self.learning_rate:.4f}")


def create_signal_from_market_data(price_delta: float, volume_spike: float, 
                                 entropy_level: float, source: str = "market_data") -> ImmuneSignalData:
...
if __name__ == "__main__":
    ...
    for i, signal in enumerate(test_signals):
        response = qsc_gate.process_immune_response(signal, gts_sync_score=0.7 + i*0.1)
        print(f"Signal {i+1}: {response.activation_level.value} -> {response.decision}")
