...
class SwarmStrategyMatrix:
    ...
    def _initialize_swarm_nodes(self) -> None:
        ...
        strategy_types = self.config.get("strategy_types", ["momentum", "reversal", "breakout"])
        ...
        logger.info(f"🐝 Initialized {len(self.nodes)} swarm nodes across {len(strategy_types)} strategies")
    
    def swarm_vector_response(self, market_conditions: Dict[str, float], 
                            immune_activation: float = 0.5) -> SwarmResponse:
        ...
        for node_id in active_nodes:
            ...
            immune_modulation = self._calculate_immune_modulation(immune_activation, node.strategy_type)
            
            # Calculate effective weight
            effective_weight = (node.profit_weight * 
                              node.confidence * 
                              alignment * 
                              immune_modulation)
        ...
        response = SwarmResponse(
            ...
            metadata={
                ...
                "active_strategies": list(set(self.nodes[nid].strategy_type for nid in active_nodes)),
    ...
    def _determine_swarm_mode(self, immune_activation: float, 
                            market_conditions: Dict[str, float]) -> SwarmMode:
        ...
        elif trend_strength > 0.7 and self.stability_counter >= self.stability_requirement:
    ...
    def _filter_active_nodes(self, swarm_mode: SwarmMode, 
        ...
        for node_id, node in self.nodes.items():
            # Basic health check
            if (time.time() - node.last_update > 300 or  # 5 minutes
                node.confidence < self.config.get("min_confidence", 0.3)):
                continue
    ...
    def _calculate_immune_modulation(self, immune_activation: float, strategy_type: str) -> float:
        """Calculate immune system modulation for different strategies."""
        base_modulation = 1.0 - immune_activation * 0.3  # Reduce activity under immune stress
    ...
    def _make_strategy_recommendation(self, consensus_vector: np.ndarray, 
    ...
    def get_swarm_status(self) -> Dict[str, Any]:
            ...
            if nodes:
                ...
                for node in nodes:
                    if node.success_history:
                        success_rate = sum(node.success_history) / len(node.success_history)
        ...
        return {
            ...
            "recent_activity": {
                ...
                "avg_consensus": np.mean([r.consensus_strength for r in recent_responses]) if recent_responses else 0.0,
                "avg_participating_nodes": np.mean([r.participating_nodes for r in recent_responses]) if recent_responses else 0.0
            },
...
if __name__ == "__main__":
    ...
    for i, conditions in enumerate(test_conditions):
        ...
        print(f"  Vector: [{response.swarm_vector[0]:.2f}, {response.swarm_vector[1]:.2f}, {response.swarm_vector[2]:.2f}]")
