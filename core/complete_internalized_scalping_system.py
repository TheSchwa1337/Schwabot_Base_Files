...
class CompleteInternalizedScalpingSystem:
    ...
    async def _generate_mathematical_analysis(self, market_data: MarketData, 
        ...
        try:
            ...
            math_result = self.math_bridge.analyze_market_data(market_data, entropy_result)
    ...
    async def _generate_scalping_decision(self, market_data: MarketData, 
        ...
        try:
            ...
            enhanced_profit = profit_result.total_profit_score * entropy_result['entropy_score']
            ...
            quantity = self._calculate_scalping_position_size(confidence, entropy_result, math_result)
            
            # Determine risk level
            risk_level = self._assess_scalping_risk_level(enhanced_profit)
    ...
    def _determine_scalping_action(self, profit_result: float, entropy_result: Dict[str, Any],
    ...
    def _calculate_scalping_position_size(self, confidence: float, entropy_result: Dict[str, Any],
    ...
    async def run_scalping_loop(self, interval_seconds: int = 30) -> None:
        ...
        logger.info(f"🔄 Starting scalping loop with {interval_seconds}s intervals")
        ...
        try:
            while True:
                try:
                    ...
                    if execution.success:
                        logger.info(f"✅ Scalping trade executed: {execution.action.value} {execution.executed_quantity} BTC at ${execution.executed_price}")
                    else:
                        logger.info(f"ℹ️ No scalping trade executed: {execution.metadata.get('reason', 'unknown')}")
            ...
            logger.info("🛑 Scalping loop stopped by user")
        except Exception as e:
            logger.error(f"❌ Fatal error in scalping loop: {e}")
...
async def demo_scalping_system():
    ...
    logger.info("🎯 DEMO: Complete Internalized Scalping System")
    ...
    # Show results
    logger.info(f"Demo execution: {execution}")
