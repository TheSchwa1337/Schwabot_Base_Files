...
class BestPracticesEnforcer:
    ...
    def _initialize_default_patterns(self):
        self._patterns.extend([
            ...
            CodePattern(
                ...
                pattern=r'print\s*\(\s*["\']([^"\']*[\u1f527\u2705\u274c\u1f7e0\u1f7e1\u1f7e2\u1f4dd\u1f3af\u1f4ca\u1f389\u26a0\ufe0f\u1f4a1])[^"\']*["\']\s*\)',
                replacement=(
    ...
    def enforce_on_file(self, file_path: str) -> EnforcementResult:
        ...
        for pattern in self._patterns:
            try:
                ...
                result.issues_found.append(f"Pattern {pattern.name} error: {e}")
        ...
        if content != original_content:
            try:
                ...
                result.success = False
                result.issues_found.append(f"Write error: {e}")
        return result
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        skip_patterns = ["__pycache__", ".git", ".mypy_cache", "venv", "env", "build", "dist"]
...
def main() -> None:
    ...
    for result in results:
        if result.success:
            success(f"{result.file_path}: Patterns applied: {result.patterns_applied}")
        else:
            error(f"{result.file_path}: Issues: {result.issues_found}")
