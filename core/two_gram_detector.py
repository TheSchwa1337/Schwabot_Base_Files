...
class TwoGramDetector:
    ...
    async def analyze_sequence(self, sequence: str, context: Optional[Dict[str, Any]] = None) -> List[TwoGramSignal]:
        ...
        try:
            ...
            if significant_signals:
                info(f"🧬 Detected {len(significant_signals)} significant 2-gram patterns")
                for signal in significant_signals:
                    debug(f"  {signal.emoji_symbol} {signal.pattern}: burst={signal.burst_score:.2f}, "
                         f"entropy={signal.entropy:.3f}, health={signal.system_health_score:.2f}")
            
            return signals
            
        except Exception as e:
            error(f"Error in 2-gram sequence analysis: {e}")
            return []
    ...
    def _calculate_burst_score(self, pattern: str, current_frequency: int) -> float:
        ...
        try:
            ...
            logger.error(f"Error calculating burst score for {pattern}: {e}")
            return 0.0
    ...
    async def _check_fractal_resonance(self, 
                                     pattern: str, 
                                     similarity_vector: List[float],
                                     context: Optional[Dict[str, Any]] = None) -> Tuple[Optional[float], Optional[float]]:
    ...
    def _assess_t_cell_response(self, 
                               pattern: str, 
                               frequency: int, 
                               entropy: float, 
                               burst_score: float) -> Tuple[bool, float]:
        ...
        try:
            ...
            if t_cell_activation:
                warn(f"🛡️ T-cell activation for pattern {pattern}: triggers={activation_triggers}")
            ...
            logger.error(f"Error in T-cell assessment: {e}")
            return False, 1.0
    ...
    def _generate_asic_hash(self, pattern: str, frequency: int) -> str:
        ...
        try:
            ...
            hash_input = f"{pattern}:{frequency}:{time.time():.0f}"
        ...
        try:
            ...
            for signal in signals:
                ...
                if len(self.memory.pattern_frequencies[signal.pattern]) > 100:
                    self.memory.pattern_frequencies[signal.pattern] = \
                        self.memory.pattern_frequencies[signal.pattern][-100:]
                ...
                for signal in burst_signals:
                    ...
                    if vector_matrix.shape[1] >= 4:
                        self.fractal_memory.save_snapshot(
                            ...
                            strategy_id=f"2gram_{signal.pattern}",
        ...
        try:
            ...
            if self.memory.entropy_history:
                stats["entropy_stats"] = {
                    ...
                    "entropy_trend": self.memory.entropy_history[-10:] if len(self.memory.entropy_history) >= 10 else self.memory.entropy_history
                }
    ...
    async def trigger_strategy_from_pattern(self, pattern: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            info(f"🎯 Strategy triggered: {signal.strategy_trigger} from pattern {signal.emoji_symbol}{signal.pattern}")
            
            return strategy_packet
            
        except Exception as e:
            logger.error(f"Error triggering strategy from pattern {pattern}: {e}")
            return None
    ...
    async def get_recent_patterns(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(recent_patterns) < limit:
                ...
                recent_bursts = sorted(
                    self.memory.burst_events, 
...
async def test_two_gram_integration():
    ...
    print(f"Detected {len(signals)} 2-gram signals:")
    for signal in signals:
        print(f"  {signal.emoji_symbol} {signal.pattern}: burst={signal.burst_score:.2f}, "
              f"freq={signal.frequency}, entropy={signal.entropy:.3f}")
        
        if signal.strategy_trigger:
            print(f"    → Strategy: {signal.strategy_trigger} (priority: {signal.execution_priority})")
    
    # Get statistics
    stats = await detector.get_pattern_statistics()
    print(f"\nDetector Statistics:")
    print(f"  Active patterns: {stats['active_patterns']}")
    print(f"  System health: {stats['system_health']:.2f}")
    print(f"  T-cell active: {stats['t_cell_active']}")
