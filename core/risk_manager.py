...
class RiskManager:
    ...
    def check_risk_limits(self, position_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if position_percentage > self.config["max_exposure_per_asset"]:
            violations.append(
                f"Position size {position_percentage:.1%} exceeds maximum {self.config['max_exposure_per_asset']:.1%}"
            )
            recommended_size = (
        ...
        if current_volatility > self.config["volatility_threshold"]:
            warnings.append(
                f"Market volatility {current_volatility:.1%} exceeds comfort level {self.config['volatility_threshold']:.1%}"
            )
        ...
        if current_drawdown > self.config["max_drawdown_percent"]:
            violations.append(
                f"Current drawdown {current_drawdown:.1%} exceeds maximum {self.config['max_drawdown_percent']:.1%}"
            )
