...
def add_aggregate_callback(self):
    """Function implementation pending."""
...
def _validate_price_change(self):
    """Function implementation pending."""
...
max_change = self.price_filters["max_price_change"]
            ...
            logger.warning()
                f"Price change too large: {price_change:.4f} > {max_change}"
            )
return False
...
self, symbol: str, interval: int, count: int = 100
    ) -> List[TickAggregate]:"""Get aggregated data for symbol and interval."""
""""""
        try:
            if ():
