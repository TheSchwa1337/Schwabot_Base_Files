...
class MathematicalRelayNavigator:
    ...
    def _calculate_profit_target(self, btc_price: float, btc_volume: float, bit_depth: BitDepth) -> float:
        ...
        try:
            # Base profit target
            base_target = btc_price * 0.001  # 0.1% base target
            ...
            return max(profit_target, btc_price * 0.0001)  # Minimum 0.01% target

        except Exception as e:
            logger.error(f"Error calculating profit target: {e}")
            return btc_price * 0.001  # Default 0.1% target
        ...
        try:
            ...
            for result in results:
                if result.get("success", False):
                    ...
                    step_profit = result.get("target_price", 0) * 0.001  # 0.1% per step
