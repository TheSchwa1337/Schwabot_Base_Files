...
class EnhancedMatrixMapper:
    ...
    def match_hash_to_enhanced_matrix(
        ...
        location: Any,  # GeoLocation type
        threshold: float = 0.8,
        # <-- NEW: Add Schwafit info block
    ) -> Optional[Tuple[str, EnhancedMatrixEntry, float, Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for fname, entry in self.matrices.items():
                ...
                # Optionally, combine with enhanced similarity (location,
                # weather, entropy, etc.)
                enhanced_score = self._compute_enhanced_similarity(schwafit_score, entry, location)
            ...
            if best_file and best_entry:
                # <-- Return Schwafit info block
                return best_file, best_entry, best_score, best_schwafit_info
    ...
    def get_entropy_aligned_matrices(
        self, location: Any, min_entropy_score: float = 0.7  # GeoLocation type
        ...
        for fname, entry in self.matrices.items():
            if entry.entropy_alignment_score >= min_entropy_score and self._is_location_compatible(
                entry, location
            ):
    ...
    def _compute_enhanced_similarity(
        self, base_score: float, entry: EnhancedMatrixEntry, location: Any  # GeoLocation type
    ) -> float:
    ...
    def _is_location_compatible(
        self, entry: EnhancedMatrixEntry, location: Any  # GeoLocation type
    ) -> bool:
    ...
    # GeoLocation type
    def _get_weather_data_for_location(self, location: Any) -> Dict[str, Any]:
    ...
    def _create_weather_data_point(
        self, location: Any, weather_data: Dict[str, Any]  # GeoLocation type
    ):
    ...
    def _get_location_distribution(self) -> Dict[str, int]:
        ...
        for entry in self.matrices.values():
            if entry.location:
                location_key = f"{
                    entry.location.lat:.1f},{
                    entry.location.lon:.1f}"
    ...
    def _get_performance_trends(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {"average_performance": float(avg_performance), "trend": float(trend)}
...
def create_enhanced_matrix_mapper(
    matrix_dir: str, weather_api_key: Optional[str] = None
) -> EnhancedMatrixMapper:
