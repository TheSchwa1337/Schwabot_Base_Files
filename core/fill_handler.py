...
class RetryConfig:
    ...
    retryable_errors: List[str] = field(default_factory=lambda: [
        'network_error', 'rate_limit', 'timeout', 'exchange_error'
    ])
...
class FillHandler:
    ...
    async def process_fill_event(self, fill_data: Dict[str, Any]) -> FillEvent:
        ...
        try:
            ...
            logger.info(f"Processed fill: {fill_event.trade_id} for {fill_event.symbol} "
    ...
    async def handle_partial_fill(self, order_id: str, fill_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not order_state:
                logger.warning(f"Order state not found for partial fill: {order_id}")
    ...
    def clear_completed_orders(self, max_age_hours: int = 24):
        ...
        for order_id, order_state in self.active_orders.items():
            if (order_state.status in [FillStatus.COMPLETE, FillStatus.CANCELLED, FillStatus.FAILED] and
                current_time - order_state.updated_at > max_age_ms):
...
async def handle_partial_fill_scenario(fill_handler: FillHandler, order_id: str, fill_data: Dict[str, Any]) -> Dict[str, Any]:
