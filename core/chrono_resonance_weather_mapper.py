...
class ChronoResonanceWeatherMapper:
    ...
    def _update_correlation_analysis(self):
        ...
        try:
            ...
            logger.debug(
                f"🔄 Updated correlation analysis: {
    ...
    def _calculate_weather_price_correlation(
        ...
        weather_extractor,
        param_name: str,
    ) -> Optional[WeatherPriceCorrelation]:
    ...
    def _calculate_weather_momentum(
        ...
        try:
            ...
            # Humidity as "mass"
            temp_momentum = abs(temp_velocity) * weather_data[-1].humidity / 100
    ...
    def _generate_trading_signals(
        ...
        try:
            ...
            if signal_components:
                ...
                signals["direction"] = (
                    "bullish"
                    if signals["signal_strength"] > 0.1
                    else "bearish" if signals["signal_strength"] < -0.1 else "neutral"
                )
...
def main():
    ...
    print(
        f"✅ Added {len(crwm.weather_history)} weather points and {len(crwm.price_history)} price points"
    )
    ...
    if prediction:
        print(
            f"6-hour prediction: {prediction['predicted_change_percent']:.2f}% change"
        )
    ...
    print(
        f"Data points - Weather: {
            status['data_points']['weather_history']}, "
        f"Price: {
            status['data_points']['price_history']}"
    )
    print(
        f"Active correlations: {
            status['analysis_results']['active_correlations']}"
    )
    print(
        f"Strongest correlation: {
            status['analysis_results']['strongest_correlation']:.3f}"
    )
