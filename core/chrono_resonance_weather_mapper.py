...
- Resonance frequency: f_r = 1/(2(LC)) where L=atmospheric pressure, C=temperature



- Price gradient: P = T + H + W (temp, humidity, wind)



- Chrono-correlation: C() =  W(t)P(t+) dt (weather-price correlation)



- Atmospheric momentum: M = v (density  wind velocity squared)
...
self.GAS_CONSTANT = 287.04  # J/(kgK)
...
if len(self.weather_history) >= 10:
            ...
            logger.debug(f Added weather data for {weather_data.location})
...
self.export_crwm_data(str(state_file))
            ...
            logger.info( ChronoResonance Weather Mapper closed)
...
print( ChronoResonance Weather Mapping Demo)print(=* 50)
