...
PortfolioTracker,
            ...
            logger.critical("
"One or more core trading components failed to import. LiveExecutionMapper may not""
function correctly.""
)
...
self.execution_states[trade_id] = execution_state
            ...
            logger.info("
f"[{trade_id}] Initiating glyph trade for {glyph} on {asset} @ {price} with volume""
{volume}""
)
...
"Signal confidence too low or no signal generated."
)"
...
execution_state.error_message = ("
"Portfolio value is zero or negative. Cannot execute trades."
)"
...
execution_state.status = "portfolio_updated"
            logger.info("
f"[{trade_id}] Portfolio updated for {asset}. Current cash:""
...
self.stats["successful_executions"] += 1
            logger.info("
f"[{trade_id}] Trade executed successfully for {glyph} ({asset}""
{trade_signal.direction.value} {size_to_execute:.4f} @ {price})""
)

        except Exception as e:"
            execution_state.status = "failed"
execution_state.error_message = ("
f"An unexpected error occurred during trade execution: {)""
str(e)}"""
self.stats["failed_executions"] += 1
            logger.critical("
f"[{trade_id}] CRITICAL ERROR: {execution_state.error_message}",
exc_info=True,
)
...
+ self.stats["rejected_by_signal_threshold"]"
+ self.stats["rejected_by_sizing"]"
+ self.stats["failed_executions"]
)
if total_completed > 0:"
                self.stats["avg_execution_flow_time"] = ("
self.stats["avg_execution_flow_time"] * (total_completed - 1)
+ execution_flow_time
) / total_completed"
execution_state.metadata["total_flow_time"] = execution_flow_time
            logger.debug("
f"[{trade_id}] Execution flow completed in {execution_flow_time:.4f} seconds with""
...
if state1.execution_details:
        print("
f"  Executed Size: {'"
                state1.execution_details.get('executed_quantity'):.4f}, """
f"Price: {'"
...
mapper_multi.execute_glyph_trade("hourglass", 2.0e6, "ADA/USD", 0.5)"
    ...
    mapper_multi.execute_glyph_trade("
        "lightning", 0.5e6, "XRP/USD", 0.6, confidence_boost=0.01
