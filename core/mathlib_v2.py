...
class CoreMathLibV2:
    ...
    def moving_average_variants(self, data: Vector, period: int = 20) -> Dict[str, float]:
        ...
        if len(data) >= period and half_period > 0:
            wma_half = np.sum(data[-half_period:] * np.arange(1, half_period + 1)) / np.sum(
                np.arange(1, half_period + 1)
            )
            wma_full = wma
            ...
            result.update(
                {
                    ...
                    "spectral_centroid": float(
                        np.sum(np.arange(len(power_spectrum)) * power_spectrum) / np.sum(power_spectrum)
                    ),
                }
            )
