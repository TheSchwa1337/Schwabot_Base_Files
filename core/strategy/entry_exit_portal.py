...
class EntryExitPortal:
    ...
    def execute_signal(self, signal: TradeSignal,
        ...
        if size_to_execute <= 0:  # No position to take
            logger.info(f"No position to execute for {signal.asset}. Size calculated as 0.")
            return {"status": "no_action", "message": "Position size is zero"}
            
        # Use TradeExecutor if available
        if self.trade_executor:
            if dry_run:
                logger.info(f"Simulating {signal.direction.value} order for {signal.asset} "
                           f"size {size_to_execute:.4f} at {signal.price}")
                execution_result = {
                ...
                logger.info(f"Placing {signal.direction.value} order for {signal.asset} "
                           f"size {size_to_execute:.4f} at {signal.price}")
        ...
        if self.enable_portfolio_tracking and self.portfolio_tracker:
            ...
            logger.debug(f"Portfolio updated for {signal.asset}. Current holdings: "
                       f"{self.portfolio_tracker.get_portfolio_summary()}")
...
def process_glyph_trade_signal(glyph: str, volume: float, asset: str = "BTC/USD",
    ...
    """
    temp_portal = EntryExitPortal(enable_risk_management=False, enable_portfolio_tracking=False)
