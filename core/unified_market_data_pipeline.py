...
class UnifiedMarketDataPipeline:
    ...
    async def get_market_data(self, symbol: str, force_refresh: bool = False) -> MarketDataPacket:
        ...
        try:
            ...
            logger.info(f"Generated market data packet for {symbol} "
                       f"(quality: {data_quality.value}, sources: {len(raw_data)})")
    ...
    def _calculate_technical_indicators(self, symbol: str, data: Dict[str, Any]) -> TechnicalIndicators:
        ...
        try:
            ...
            # Bollinger Bands
            if len(prices) >= 20:
                indicators.bb_upper, indicators.bb_middle, indicators.bb_lower, indicators.bb_position = self._calculate_bollinger_bands(prices)
    ...
    def _cache_data(self, symbol: str, data: MarketDataPacket):
    ...
    def _log_to_registry(self, packet: MarketDataPacket):
