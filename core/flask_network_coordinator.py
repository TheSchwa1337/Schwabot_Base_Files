...
device.current_load = metrics.get('cpu_usage', 0.0)
                    device.total_calculations += metrics.get()
                        'calculations_since_last_heartbeat', 0
                    device.total_profit_contributed += metrics.get()
...
# Calculate totals
total_profit = sum()
    device.total_profit_contributed for device in self.devices.values()
            total_calculations = sum()
    device.total_calculations for device in self.devices.values()
...
avg_response_time = unified_math.unified_math.mean()
    response_times if response_times else 0.0
            else:
...
self.statistics_thread = threading.Thread()
    target = self._update_statistics_loop, daemon = True
