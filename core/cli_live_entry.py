...
class SchwabotCLI:
    ...
    async def initialize_system(self, mode: str = "demo") -> bool:
        ...
        try:
            ...
            self.two_gram_detector = create_two_gram_detector(
                self.config.get("2gram_config", {})
            )

            # Initialize strategy trigger router
            logger.info("🎯 Initializing strategy trigger router...")
            self.strategy_router = create_strategy_trigger_router(
                self.config.get("strategy_router_config", {})
            )
            ...
            self.execution_engine = create_real_time_execution_engine(
                self.config.get("execution_engine_config", {})
            )
            ...
            self.portfolio_balancer = create_portfolio_balancer(
                self.config.get("portfolio_balancer_config", {})
            )

            # Initialize BTC/USDC integration
            logger.info("💰 Initializing BTC/USDC integration...")
            self.btc_integration = create_btc_usdc_integration(
                self.config.get("btc_usdc_config", {})
            )

            # Initialize master profit coordination system
            logger.info("🎛️ Initializing master profit coordination system...")
            self.master_system = create_master_profit_coordination_system(
                self.config.get("master_system_config", {})
            )

            # Initialize new trading components
            logger.info("💼 Initializing CCXT trading executor...")
            self.ccxt_executor = create_ccxt_trading_executor(
                self.config.get("ccxt_executor_config", {})
            )

            logger.info("📊 Initializing order book manager...")
            self.order_book_manager = create_order_book_manager(
                self.config.get("order_book_manager_config", {})
            )
        ...
        try:
            ...
            for exchange_config in exchanges:
                ...
                try:
                    ...
                    exchange_connection = await self._get_exchange_connection(
                        exchange_config
                    )
                    ...
                    if exchange_connection:
                        ...
                        balance = await self._fetch_account_balance(
                            exchange_connection, exchange_name
                        )

                        # Fetch held assets
                        assets = await self._fetch_held_assets(
                            exchange_connection, exchange_name
                        )

                        # Fetch available trading pairs
                        pairs = await self._fetch_trading_pairs(
                            exchange_connection, exchange_name
                        )
                        ...
                        logger.info(
                            f"✅ {exchange_name}: ${balance['total_value']:.2f} total, "
                            f"{len(assets)} assets"
                        )

                except Exception as e:
                    logger.warning(
                        f"⚠️ Failed to discover portfolio from {exchange_name}: {e}"
                    )
                    continue
    ...
    async def _get_exchange_connection(self, exchange_config):
        ...
        try:
            ...
            from core.api.exchange_connection import create_exchange_connection
    ...
    def _update_portfolio_metrics(self, exchange_name, balance, assets, pairs):
    ...
    def _update_config_with_discovered_values(self):
        ...
        try:
            ...
            if self.total_portfolio_value > 0:
                ...
                logger.info(
                    f"💰 Updated initial capital to: ${self.total_portfolio_value:.2f}"
                )
            ...
            if self.held_assets:
                ...
                if "BTC" in self.held_assets and "USDC" in self.held_assets:
                    ...
                    self.config["default_symbol"] = "BTC/USDC"
                logger.info(
                    f"🎯 Updated default symbol to: {self.config['default_symbol']}"
                )
            ...
            if self.held_assets:
                ...
                if total_value > 0:
                    ...
                    self.config["portfolio_balancer_config"]["target_allocation"] = (
                        target_allocation
                    )
    ...
    async def _market_monitoring_loop(self):
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                # Wait for next cycle
                await asyncio.sleep(
                    self.config.get("market_monitoring_interval", 1.0)
                )
            except Exception as e:
        ...
        try:
            ...
            if len(market_sequence) >= 2:
                ...
                two_gram_signals = await self.two_gram_detector.analyze_sequence(
                    market_sequence, context=market_data
                )

                # Process significant signals
                significant_signals = [
                    s for s in two_gram_signals
                    if s.burst_score > self.two_gram_detector.burst_threshold
                ]

                if significant_signals:
                    logger.info(
                        f"🧬 Detected {len(significant_signals)} significant 2-gram patterns"
                    )
                    ...
                    for two_gram_signal in significant_signals:
                        ...
                        if execution_result and execution_result.executed:
                            ...
                            logger.warning(
                                f"⚠️ Strategy execution failed: {execution_result.error_message}"
                            )
    ...
    async def _strategy_processing_loop(self):
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                # Wait for next cycle
                await asyncio.sleep(
                    self.config.get("strategy_processing_interval", 5.0)
                )
    ...
    async def _performance_monitoring_loop(self):
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                # Wait for next cycle
                await asyncio.sleep(
                    self.config.get("performance_monitoring_interval", 30.0)
                )
            except Exception as e:
...
async def main():
    ...
    parser.add_argument("--config", default="config/schwabot_config.yaml",
