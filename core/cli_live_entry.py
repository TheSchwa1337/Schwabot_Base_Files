...
class LiveTradingBot:
    ...
    async def initialize(self):
        ...
        if "market_data_config" in self.config:
            self.trading_pipeline.pipeline_config.update(self.config["market_data_config"])
        
        # Initialize CCXT executor with real exchange connection
        if "exchange_config" in self.config:
            self.ccxt_executor = CCXTTradingExecutor(self.config["exchange_config"])
    ...
    async def execute_live_trade(self, symbol: str, force_refresh: bool = False):
        ...
        if trade_result and trade_result.get("trade_action", {}).get("action") != "hold":
    ...
    async def _execute_ccxt_trade(self, trade_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while self.is_running:
            try:
                ...
                if trade_result:
                    action = trade_result.get("trade_action", {}).get("action", "hold")
                    if action != "hold":
                        print(f"⚡ Trade executed: {action} {self.config.get('symbol')}")
...
def main():
    ...
    parser.add_argument("--config", type=str, required=True, help="Trading bot configuration file")
    parser.add_argument("--symbol", type=str, default="BTCUSDT", help="Trading symbol")
    parser.add_argument("--interval", type=int, default=60, help="Trading interval in seconds")
    parser.add_argument("--force-refresh", action="store_true", help="Force refresh market data")
    parser.add_argument("--safe-mode", action="store_true", help="Run in safe mode, skipping external plugins.")

    # Registry operations
    parser.add_argument("--registry-file", type=str, help="Registry file path")
    parser.add_argument("--asset", type=str, help="Asset for registry operations")
    parser.add_argument("--phase", type=float, help="Phase value for trigger logging")
    parser.add_argument("--drift", type=float, help="Drift value for trigger logging")
