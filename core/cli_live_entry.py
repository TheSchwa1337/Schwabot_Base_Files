...
class SchwabotCLI:
    ...
    def _initialize_system(self) -> None:
        ...
        try:
            ...
            if self.bro_logic:
                ...
                bro_config = self.config.get("system", {}).get("bro_logic_config", {})
                self.bro_logic = create_bro_logic_module(bro_config)
                logger.info("✅ Big Bro Logic Module initialized with configuration")
            
            self.initialized = True
            logger.info("✅ Schwabot CLI system initialized successfully")
            
        except Exception as e:
            logger.error(f"❌ Error initializing system: {e}")
            self.initialized = False

    async def process_market_data(self, symbol: str, price: float, volume: float = 0.0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            bro_result = None
            if self.bro_logic and len(self.price_history[symbol]) >= 20:
                ...
                logger.info(f"🧠 Big Bro analysis for {symbol}:")
    ...
    def _generate_trading_signal(self, symbol: str, price: float, bro_result: Optional[BroLogicResult]) -> str:
        """
    ...
    def _calculate_position_size(self, bro_result: Optional[BroLogicResult]) -> float:
        """
        ...
        try:
            ...
            # Scale Kelly fraction by confidence score
