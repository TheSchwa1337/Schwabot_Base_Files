...
from core.master_profit_coordination_system import MasterProfitCoordinationSystem, create_master_profit_coordination_system
...
class SchwabotCLI:
    ...
    async def _discover_portfolio_and_assets(self):
        ...
        try:
            ...
            for exchange_config in exchanges:
                ...
                try:
                    ...
                    if exchange_connection:
                        ...
                        logger.info(f"✅ {exchange_name}: ${balance['total_value']:.2f} total, {len(assets)} assets")
            ...
            logger.info(f"📊 Portfolio Discovery Complete:")
        ...
        try:
            ...
            if self.held_assets:
                ...
                if total_value > 0:
                    ...
                    logger.info(f"⚖️ Updated target allocation based on held assets")
            ...
            if len(market_sequence) >= 2:
                ...
                if significant_signals:
                    ...
                    for signal in significant_signals:
                        execution_result = await self.trading_strategy_executor.process_2gram_signal(signal, market_data)
                        if execution_result and execution_result.executed:
                            logger.info(f"✅ Strategy executed: {execution_result.strategy_type.value} - {execution_result.symbol}")
                            
                            # Update performance metrics
                            self.total_trades += 1
                            
                            # Log trade execution
                            await self._log_trade_execution(signal, execution_result, market_data)
    ...
    async def _performance_monitoring_loop(self):
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                execution_summary = self.execution_engine.get_performance_summary()
