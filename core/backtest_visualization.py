...
class BacktestVisualizer:
    ...
    def create_performance_charts(self, 
        ...
        if save:
            ...
            filepath = self.results_dir / f"backtest_results_{timestamp}.png"
    ...
    def _plot_portfolio_value(self, ax, portfolio_history: List[float]):
        ...
        ax.text(0, portfolio_history[0], f'${portfolio_history[0]:,.2f}', 
                verticalalignment='bottom')
        ax.text(len(portfolio_history)-1, portfolio_history[-1], 
                f'${portfolio_history[-1]:,.2f}', 
                verticalalignment='bottom')
    ...
    def _plot_trade_distribution(self, ax, trades_df: pd.DataFrame):
        ...
        if not trades_df.empty:
            trades_df['profit'] = trades_df.apply(
                lambda x: x['value'] - x['fees'] if x['type'] == 'sell' else 0, 
                axis=1
            )
            
            # Create profit distribution
            sns.histplot(data=trades_df[trades_df['type'] == 'sell'], 
                        x='profit', bins=30, ax=ax)
    ...
    def _plot_drawdown(self, ax, portfolio_history: List[float]):
        ...
        ax.annotate(f'Max Drawdown: {drawdown[max_drawdown_idx]:.2%}',
    ...
    def _plot_metrics_summary(self, ax, metrics: Dict[str, float]):
        ...
        for i, bar in enumerate(bars):
            ...
            ax.text(width, bar.get_y() + bar.get_height()/2,
                   f'{list(metrics_to_show.values())[i]}',
    ...
    def create_trade_analysis(self, trade_history: List[Dict], save: bool = True):
        ...
        if save:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filepath = self.results_dir / f"trade_analysis_{timestamp}.png"
            plt.savefig(filepath, dpi=300, bbox_inches='tight')
            plt.close()
        else:
            plt.show()
    ...
    def _plot_profit_factor(self, ax, trades_df: pd.DataFrame):
        ...
        ax.bar(['Winning Trades', 'Losing Trades'], 
               [winning_trades, losing_trades],
               color=['lightgreen', 'lightcoral'])
        ax.set_title(f'Profit Factor Analysis (PF: {profit_factor:.2f})')
    ...
    def save_trade_log(self, trade_history: List[Dict]):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = self.results_dir / f"trade_log_{timestamp}.csv"
