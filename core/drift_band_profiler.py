...
class DriftBandProfiler:
    ...
    def observe_drift(self, symbol: str = "BTC/USDC") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            current_data = tick_cache.get(symbol) if 'tick_cache' in globals() else None
            if not current_data:
            ...
            self.logger.debug(f"📊 Drift observed for {symbol}: {drift_rate:.6f} over {delta_time:.1f}s")
    ...
    def evaluate_profit_band(self, tag: str) -> Tuple[str, float]:
        ...
        try:
            ...
            self.logger.error(f"❌ Error evaluating profit band for tag '{tag}': {e}")
            return "baseline", 1.0

    def adjust_volume_by_band(self, base_volume: float, tag: str) -> float:
        """
        ...
        """
        try:
            _, multiplier = self.evaluate_profit_band(tag)
            adjusted_volume = base_volume * multiplier
            
            self.logger.debug(f"📈 Volume adjusted for '{tag}': {base_volume:.4f} -> {adjusted_volume:.4f} (x{multiplier:.2f})")
    ...
    def get_optimal_volume_allocation(self, tag: str, base_volume: float, 
