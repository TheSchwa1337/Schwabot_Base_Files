#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Data Pipeline Visualizer - Real-time Data Flow Visualization.

This module provides comprehensive visualization for Schwabot's data pipeline:
...
class DataPipelineVisualizer:
    ...
    def _create_visualization_ui(self):
        ...
        # Pipeline canvas
        self.canvas = Canvas(overview_frame, height=300, bg='#2a2a2a', highlightthickness=0)
        ...
        ttk.Button(button_frame, text="▶️ Start Pipeline", 
                  command=self._start_pipeline).pack(side='left', padx=5)
        ttk.Button(button_frame, text="⏸️ Pause Pipeline", 
                  command=self._pause_pipeline).pack(side='left', padx=5)
        ttk.Button(button_frame, text="🗑️ Cleanup Data", 
                  command=self._cleanup_pipeline).pack(side='left', padx=5)
        ttk.Button(button_frame, text="📊 Export Stats", 
                  command=self._export_statistics).pack(side='left', padx=5)
        ttk.Button(button_frame, text="🔄 Reset Pipeline", 
                  command=self._reset_pipeline).pack(side='left', padx=5)
        
        # Flow visualization
        flow_frame = ttk.Frame(control_frame)
        flow_frame.pack(fill='x', padx=10, pady=5)
        
        self.flow_canvas = Canvas(flow_frame, height=150, bg='#2a2a2a', highlightthickness=0)
        ...
        self.metrics_text = tk.Text(text_frame, bg='#2a2a2a', fg='#00ff00', 
                                   font=('Courier', 10))
    ...
    def add_data_unit(self, 
        ...
        try:
            unit_id = f"{category.value}_{int(time.time() * 1000)}_{random.randint(1000, 9999)}"
            ...
            if self.config['compression_enabled']:
                data_unit.compression_ratio = self._calculate_compression_ratio(category, data_size)
            ...
            if self._check_tier_capacity(tier, data_size):
                ...
                if next_tier and self._check_tier_capacity(next_tier, data_size):
                    ...
                    logger.info(f"📦 Moved data unit {unit_id} to {next_tier.value}")
                    return unit_id
                else:
                    ...
                    if self._check_tier_capacity(tier, data_size):
                        ...
                        logger.warning(f"⚠️ Unable to store data unit, all tiers full")
                        return ""
    ...
    def remove_data_unit(self, unit_id: str) -> bool:
        ...
        try:
            if unit_id in self.data_units:
                ...
                self._update_tier_metrics(tier)
                logger.debug(f"🗑️ Removed data unit {unit_id} from {tier.value}")
                return True
            return False
    ...
    def move_data_unit(self, unit_id: str, target_tier: DataTier) -> bool:
        ...
        try:
            ...
            if self._check_tier_capacity(target_tier, data_unit.size_bytes):
                ...
                logger.debug(f"📦 Moved data unit {unit_id}: {old_tier.value} -> {target_tier.value}")
                return True
            else:
                logger.warning(f"⚠️ Cannot move {unit_id} to {target_tier.value}: insufficient capacity")
                return False
    ...
    def _calculate_compression_ratio(self, category: DataCategory, size: int) -> float:
        ...
        compression_ratios = {
            DataCategory.BTC_HASHING: 0.8,      # Binary data, moderate compression
    ...
    def _update_tier_metrics(self, tier: DataTier):
        ...
        try:
            ...
            tier_units = [unit for unit in self.data_units.values() if unit.tier == tier]
            
            # Calculate metrics
            total_units = len(tier_units)
            total_size = sum(unit.size_bytes for unit in tier_units)
            compressed_size = sum(unit.size_bytes * unit.compression_ratio for unit in tier_units)
            ...
            compression_savings = ((total_size - compressed_size) / total_size) * 100 if total_size > 0 else 0
    ...
    def _calculate_flow_rate(self, tier: DataTier) -> float:
        ...
        try:
            ...
            recent_units = [
                unit for unit in self.data_units.values()
            ...
            # Convert to Mbps
            mbps = (recent_size * 8) / (60 * 1024 * 1024)  # bits per second to Mbps
    ...
    def _auto_cleanup(self):
        ...
        try:
            ...
            for tier, retention_period in retention_policies.items():
                ...
                old_units = [
                    unit_id for unit_id, unit in self.data_units.items()
    ...
    def _update_pipeline_visualization(self):
        ...
        try:
            ...
            for i, tier in enumerate([DataTier.RAM_CACHE, DataTier.MID_TERM, DataTier.LONG_TERM, DataTier.ARCHIVE]):
                x = i * tier_width + 20
                y = 50
                width = tier_width - 40
                height = 150
                ...
                if bar_height > 0:
                    self.canvas.create_rectangle(x + 5, y + height - bar_height, 
                ...
                # Draw unit count
                self.canvas.create_text(x + width // 2, y + height + 35,
                ...
                if i < 3:  # Don't draw arrow after last tier
                    ...
                    self.canvas.create_line(arrow_x1, arrow_y, arrow_x2, arrow_y,
            ...
            stats_text = (
                ...
                f"Total Size: {self._format_bytes(sum(m.total_size_bytes for m in self.tier_metrics.values()))} | "
                f"Compressed: {self._format_bytes(sum(m.compressed_size_bytes for m in self.tier_metrics.values()))} | "
                f"Uptime: {self._format_uptime(time.time() - self.start_time)}"
            )
            
            self.canvas.create_text(canvas_width // 2, stats_y,
                                  text=stats_text,
                                  fill='#ffffff', font=('Arial', 10))
    ...
    def _update_flow_visualization(self):
        ...
        try:
            ...
            tier_x_positions = [canvas_width * 0.2, canvas_width * 0.4, canvas_width * 0.6, canvas_width * 0.8]
    ...
    def _update_metrics_display(self):
        ...
        try:
            ...
            for tier in DataTier:
                ...
                metrics_lines.append(f"  Size: {self._format_bytes(metrics.total_size_bytes)}")
                metrics_lines.append(f"  Compressed: {self._format_bytes(metrics.compressed_size_bytes)}")
                metrics_lines.append(f"  Usage: {metrics.usage_percentage:.1f}%")
                metrics_lines.append(f"  Flow Rate: {metrics.flow_rate_mbps:.3f} Mbps")
                metrics_lines.append(f"  Compression Savings: {metrics.compression_savings:.1f}%")
            ...
            metrics_lines.append(f"  Animation FPS: {self.config['animation_fps']}")
            ...
            for category, count in sorted(category_counts.items()):
                metrics_lines.append(f"  {category.replace('_', ' ').title()}: {count}")
    ...
    def _update_pipeline_stats(self):
        ...
        try:
            ...
            compressed_size = sum(m.compressed_size_bytes for m in self.tier_metrics.values())
            
            # Update pipeline stats
            self.pipeline_stats.active_units = total_units
            self.pipeline_stats.compression_ratio = compressed_size / total_size if total_size > 0 else 1.0
            ...
            self.pipeline_stats.memory_efficiency = 1.0 - (used_capacity / total_capacity) if total_capacity > 0 else 0.0
            
            # Calculate throughput
            if self.pipeline_stats.uptime_seconds > 0:
                self.pipeline_stats.throughput_mbps = (total_size * 8) / (self.pipeline_stats.uptime_seconds * 1024 * 1024)
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_units': sum(m.total_units for m in self.tier_metrics.values()),
            'total_size_bytes': sum(m.total_size_bytes for m in self.tier_metrics.values()),
            'compressed_size_bytes': sum(m.compressed_size_bytes for m in self.tier_metrics.values()),
            'active_particles': len(self.particles),
            'uptime_seconds': time.time() - self.start_time,
            'tier_metrics': {tier.value: metrics.__dict__ for tier, metrics in self.tier_metrics.items()}
        }
