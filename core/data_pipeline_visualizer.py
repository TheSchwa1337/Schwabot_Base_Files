...
button_frame, text= Start Pipeline, command = self._start_pipeline
...
button_frame, text= Pause Pipeline", command = self._pause_pipeline
...
button_frame, text= Cleanup Data", command = self._cleanup_pipeline
...
self._update_tier_metrics(tier)
        ...
        return unit_id
                        ...
                        logger.warning( Unable to store data unit, all tiers full)return
...
f Removed data unit {unit_id} from {
...
if cleaned_count > 0:
                ...
                logger.info(f Auto-cleanup removed {cleaned_count} old data units)
...
cleaned_count = original_count - len(self.data_units)
            ...
            logger.info(f Manual cleanup removed {cleaned_count} data units)
