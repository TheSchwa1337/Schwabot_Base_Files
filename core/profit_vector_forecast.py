...
def calculate_momentum_rsi_component(self, momentum: float, rsi: float)::: -> float:
...
recent_timestamps = [
p["timestamp"] for p in list(self.price_history)[-lookback:]
]
...
recent_prices = [
p["price"] for p in list(self.price_history)[-self.volatility_window :]
]
returns = [
...
"peak": (0.2, 0.1, 0.8),
# Slight bearish, stable, low time component
"valley": (-0.2, 0.1, 0.2),
# Strong bullish, decreasing volatility
"wave_up": (0.4, -0.1, 0.5),
# Strong bearish, decreasing volatility
"wave_down": (-0.4, -0.1, 0.5),
# Neutral, high volatility, medium time
"consolidation": (0.0, 0.2, 0.3),
}
...
"current_phase": (
self.current_phase.phase_type if self.current_phase else "unknown"
),
"phase_confidence": (
