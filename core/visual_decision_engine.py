...
class VisualDecisionEngine:
    ...
    def __init__(self, max_memory_size: int = 1000, num_agents: int = 5):
        """
        ...
        logger.info(f"Visual Decision Engine initialized (memory: {max_memory_size}, agents: {num_agents})")
    
    def route_with_path_blending(self, strategy_id: str, q_matrix: np.ndarray, 
                                vector: np.ndarray, market_context: Optional[Dict[str, Any]] = None) -> Tuple[str, np.ndarray, str]:
        """
        ...
        try:
            ...
            if cached:
                logger.info(f"🧠 HASH-GLYPH PATH MATCH FOUND → Glyph: {cached.glyph}")
            ...
            self.compressor.store(
                strategy_id, q_matrix, glyph, blended_vector,
                consensus_result.vote_distribution, consensus_result.confidence
            )
            
            decision_type = consensus_result.consensus_vote
            
            logger.debug(f"New path created: {glyph} → {decision_type} (confidence: {consensus_result.confidence:.3f})")
            return glyph, blended_vector, decision_type
            
        except Exception as e:
            logger.error(f"Error in path blending: {e}")
    ...
    def render_strategy_grid(self, asset: str, q_matrix: np.ndarray, profit_vector: np.ndarray) -> None:
        ...
        try:
            ...
            # Print grid
            print(f"\n🎨 Visual Strategy Grid for {asset}:")
            print(f"Glyph: {glyph}")
            print(f"Vector: {profit_vector}")
            print("Grid:")
            for row in grid:
                print("  " + " ".join(row))
            
        except Exception as e:
            logger.error(f"Error rendering strategy grid: {e}")
