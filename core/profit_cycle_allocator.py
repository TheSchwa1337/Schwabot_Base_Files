...
class ProfitCycleAllocator:
    ...
    def allocate_profits(
        ...
        if profit_deltas.shape != weights.shape:
            raise ValueError("profit_deltas and weights must have the same shape")
        ...
        allocations, overflow = self.apply_bit_logic(weighted_profits, bit_mode, log)
        ...
        if overflow > 0 and recursion_depth < max_recursion:
            ...
            if next_bit_mode:
                log.append(f"Overflow {overflow} detected, recursing to bit mode {next_bit_mode}")
    ...
    def apply_bit_logic(
        ...
        """
        n_buckets = bit_mode
        bucket_size = float(np.sum(weighted_profits)) / n_buckets if n_buckets > 0 else 0.0
        allocations = {}
        overflow = 0.0
    ...
    def map_symbol_to_logic(self, symbol: str, value: float, log: List[str]) -> Any:
        """
    ...
    def log_allocation(self, result: AllocationResult):
        ...
        """
        logger.info(f"Profit allocation (bit_mode={result.bit_mode}, recursion={result.recursion_depth}, symbol={result.symbol}):")
...
if __name__ == "__main__":
    ...
    result = allocator.allocate_profits(profit_deltas, weights, bit_mode=8, symbol=symbol)
    print("\\n--- Allocation Result ---")
