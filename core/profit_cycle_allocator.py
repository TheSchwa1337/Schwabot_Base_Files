...
# Import Matrix Mapper
        try:
    from core.matrix_mapper import BasketType, BitPhase, MatrixMapper
...
# Import DLT Waveform Engine
                try:
            from core.dlt_waveform_engine import BitPhase as DLTBitPhase
...
# Create result
                                                                                ...
                                                                                safe_safe_print()
                                                                                f"\\u2705 Enhanced allocation completed: tensor_score={")
                                                                                tensor_score:.4f}, bit_phase={bit_phase}")"
                                                                            return result
...
logger.error(f"Error calculating ZPE metrics: {e}")
    ...
    self, execution_packet: Dict[str, Any], tensor_score: float, bit_phase: Optional[int]) -> None:
...
# High profit scenario - allocate to BTC and USDC
            allocation = {"""""")
                ...
                "USDC": profit * 0.25
            safe_safe_print()
            f"\\u1f7e2 High profit rebalance: BTC={")
            allocation['BTC']:.4f}, USDC={
            allocation['USDC']:.4f}")"
...
execution_packet: Dict[str, Any],
cycles: Sequence[str] | None = None
    ) -> Dict[str, Any]:"""Legacy profit cycle allocation for backward compatibility."""
""""""
        try:
