...
d1, d2, d3 = delta_sequence[-3:]
        ...
        # Check if the deltas are close to each other relative to their
        # magnitude
...
safe_print("\\n[1] Testing Triplet Lock Confirmation...")
    ...
    f"  Stable sequence lock: {"}
        ml4.confirm_triplet_lock(stable_deltas")"
    safe_print()
    f"  Unstable sequence lock: {"}
        ml4.confirm_triplet_lock(unstable_deltas")"
...
safe_print("\\n[3] Testing Greyscale Confidence...")
    strong_match_low_drift=ml4.calculate_greyscale_confidence()
        0.95, drift_velocity=0.1
    weak_match_low_drift=ml4.calculate_greyscale_confidence()
        0.60, drift_velocity=0.1
    strong_match_high_drift=ml4.calculate_greyscale_confidence()
        0.95, drift_velocity=2.0
