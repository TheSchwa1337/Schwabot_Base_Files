...
risk_factor = 1.0 - mathematical_analysis.get("risk_assessment", {}).get(
                "risk_score", 0.5
)
...
# Risk scoring
risk_score = mathematical_analysis.get("risk_assessment", {}).get(
                "risk_score", 0.5
)
...
if composite_confidence < self.config["correlation_threshold"]:
                logger.debug(
f"Signal filtered out due to low composite confidence: {composite_confidence}"
)
return None
...
dlt_confidence = mathematical_analysis.get("dlt_metrics", {}).get(
"confidence", 0.5
)
triplet_lock = mathematical_analysis.get("dlt_metrics", {}).get(
"triplet_lock", False
)
warp_factor = mathematical_analysis.get("dlt_metrics", {}).get(
"warp_factor", 1.0
)
...
if len(self.integration_metrics["correlation_scores"]) > max_scores:
            self.integration_metrics["correlation_scores"] = self.integration_metrics[
"correlation_scores"
][-max_scores // 2 :]

# Update composite confidence average
if self.integration_metrics["correlation_scores"]:
            self.integration_metrics["composite_confidence_avg"] = sum(
...
print(
"  Active Strategies: "
f"{status['orchestration_state'].total_strategies_active}"
)
