...
- Ψ_zpe(t) = Σ_i^n A_i·sin(ω_i·t + φ_i)  # Wave function
- Φ_zpe(x, t) = ∇·Ψ_zpe(x, t) + λ_zpe·(∂Ψ/∂t)  # Field function
- Ξ_zpe = ∫_Ω Φ_zpe(x, t) dx  # Integrated field
- G_zpe = e^(−β·|∇Φ_zpe|²) · tanh(Φ_zpe/Ξ_zpe)  # Field coupling
...
class ZPEMatrixCore:
    ...
    self.config = config or ZPEMatrixConfig()
        ...
        logger.info(f"ZPE Matrix Core initialized with {self.active_math_system} math system")
    ...
    def _select_math_system(self, operation_name: str) -> str:
        ...
        if thermal_temp > self.config.thermal_threshold:
            if self.active_math_system != "legacy":
                logger.warning(f"High thermal conditions ({thermal_temp:.1f}°C) - switching to legacy math")
                self.active_math_system = "legacy"
            return "legacy"
        
        # Use unified system for normal conditions
        if self.active_math_system != "unified":
            logger.info(f"Normal thermal conditions ({thermal_temp:.1f}°C) - using unified math")
                    self.active_math_system = "unified"
        return "unified"
    
    def _execute_with_tracking(self, operation_name: str, operation_func, *args, **kwargs) -> ZPECalculationResult:
    ...
    def zpe_psi(self, amplitudes: Sequence[float], frequencies: Sequence[float],
        ...
        """
        Calculate Ψ_zpe(t) = Σ_i^n A_i·sin(ω_i·t + φ_i).
    ...
    def zpe_phi(self, psi_div: float, psi_time_deriv: float, lambda_zpe: float) -> ZPECalculationResult:
        """
        Calculate Φ_zpe(x, t) = ∇·Ψ_zpe(x, t) + λ_zpe·(∂Ψ/∂t).
    ...
    def zpe_xi(self, phi_values: Sequence[float], domain_width: float = 1.0) -> ZPECalculationResult:
        """
        Calculate Ξ_zpe = ∫_Ω Φ_zpe(x, t) dx using selected integration method.
    ...
    def zpe_g(self, phi_zpe: float, xi_zpe: float, grad_phi_magnitude: float,
        ...
        """
        Calculate G_zpe = e^(−β·|∇Φ_zpe|²) · tanh(Φ_zpe/Ξ_zpe).
...
def main():
    ...
    safe_print("🧠 Testing ZPE Matrix Core")
    safe_print("=" * 40)
