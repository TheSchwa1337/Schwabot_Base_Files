...
class DeterministicValueEngine:
    ...
    def __init__(self) -> None:
        ...
        self.entry_score_weights = {'harmony': 0.3, 'drift': 0.2, 'liquidity': 0.25, 'profit': 0.25}
    ...
    def calculate_deterministic_decision(self, market_state: MarketState) -> DeterministicDecision:
        ...
        try:
            ...
            asset_allocation = self._calculate_asset_allocation(phase_mode, strategy_weights, market_state)
            
            # 8. Calculate position sizing and risk parameters
            position_size = self._calculate_position_size(execution_confidence, entry_score, market_state)
            stop_loss, take_profit = self._calculate_risk_parameters(market_state, strategy_weights)
            
            # 9. Estimate expected return and risk
            expected_return = self._calculate_expected_return(strategy_weights, asset_allocation, market_state)
            risk_adjustment = self._calculate_risk_adjustment(market_state, asset_allocation)
            ...
            decision = DeterministicDecision(
                ...
                supporting_evidence=self._generate_supporting_evidence(market_state, execution_confidence, entry_score)
            )
            ...
            logger.debug(f"🎯 Deterministic decision calculated in {calculation_time:.4f}s")
    ...
    def _calculate_timing_determinism(self, market_state: MarketState) -> float:
        ...
        # Volatility timing
        avg_volatility = np.mean(list(market_state.volatility.values())) if market_state.volatility else 0.5
    ...
    def _calculate_conditional_determinism(self, market_state: MarketState) -> float:
        ...
        volatility_score = np.exp(-((avg_volatility - 0.02) ** 2) / 0.001)  # Optimal around 2%
        ...
        if market_state.available_capital > 0:
            utilization = sum(abs(pos) for pos in market_state.positions.values()) / market_state.available_capital
            portfolio_health = max(0, 1.0 - utilization) if utilization < 1 else 0
        
        # 6. Risk tolerance condition
        risk_score = 1.0
        if market_state.unrealized_pnl < 0:
            drawdown = abs(market_state.unrealized_pnl) / max(market_state.available_capital, 1)
            risk_score = max(0, 1.0 - drawdown * 5)  # Reduce willingness at 20% drawdown
    ...
    def _calculate_strategy_determinism(self, market_state: MarketState) -> Dict[StrategyType, float]:
        ...
        for strategy_type in StrategyType:
            if strategy_type in self.strategy_scorers:
                try:
                    ...
                    logger.warning(f"⚠️ Strategy scoring failed for {strategy_type}: {e}")
    ...
    def _calculate_execution_confidence(self, market_state: MarketState) -> float:
        ...
        T_entropy = np.mean(list(market_state.entropy_levels.values())) if market_state.entropy_levels else 3.0
        ...
        sigma_f = np.std(list(market_state.price_deltas.values())) if market_state.price_deltas else 0.02
        epsilon_sigma_term = epsilon * sigma_f
        
        # τ_p - Profit-time modifier
        time_factor = min((time.time() - market_state.last_trade_time) / 3600, 1.0)  # Hours
    ...
    def _calculate_entry_score(self, market_state: MarketState) -> float:
        ...
        projected_profit = np.mean([abs(pc) for pc in price_changes]) if price_changes else 0.01
    ...
    def _determine_phase_mode(self, market_state: MarketState) -> PhaseMode:
        ...
        if complexity_score < self.phase_thresholds[PhaseMode.FOUR_BIT]['max_complexity']:
            return PhaseMode.FOUR_BIT
        elif complexity_score < self.phase_thresholds[PhaseMode.EIGHT_BIT]['max_complexity']:
            return PhaseMode.EIGHT_BIT
        else:
            return PhaseMode.FORTY_TWO_BIT
    
    def _calculate_asset_allocation(self, phase_mode: PhaseMode, strategy_weights: Dict[StrategyType, float], 
                                   market_state: MarketState) -> Dict[AssetType, float]:
        ...
        if StrategyType.MEAN_REVERSION in strategy_weights:
            ...
            strategy_adjustments += np.array([0.15, 0.05, -0.1, -0.1]) * reversion_weight
        ...
        adjusted_allocation = np.maximum(adjusted_allocation, 0.0)  # No negative allocations
        adjusted_allocation = adjusted_allocation / np.sum(adjusted_allocation)  # Normalize
    ...
    def _calculate_position_size(self, execution_confidence: float, entry_score: float, 
        ...
        volatilities = list(market_state.volatility.values())
        avg_volatility = np.mean(volatilities) if volatilities else 0.02
        risk_adjustment = 1.0 / (1.0 + avg_volatility * 10)  # Reduce size with volatility
        
        # Calculate final position size
        position_size = base_size * confidence_multiplier * entry_multiplier * risk_adjustment
    ...
    def _calculate_risk_parameters(self, market_state: MarketState, 
                                 strategy_weights: Dict[StrategyType, float]) -> Tuple[Optional[float], Optional[float]]:
    ...
    def _calculate_expected_return(self, strategy_weights: Dict[StrategyType, float],
        ...
        expected_asset_return = sum(
            asset_allocation[asset] * base_returns[asset]
    ...
    def _calculate_risk_adjustment(self, market_state: MarketState, 
                                 asset_allocation: Dict[AssetType, float]) -> float:
        ...
        concentration_risk = max(allocation_values) - 0.25  # Penalty above 25% concentration
        concentration_risk = max(concentration_risk, 0)
        
        # Correlation risk (simplified)
        correlation_values = list(market_state.correlations.values())
        avg_correlation = np.mean([abs(c) for c in correlation_values]) if correlation_values else 0.5
    ...
    def _calculate_max_hold_time(self, strategy_weights: Dict[StrategyType, float]) -> float:
    ...
    def _generate_supporting_evidence(self, market_state: MarketState, 
                                    execution_confidence: float, entry_score: float) -> List[str]:
    ...
    def _create_fallback_decision(self, market_state: MarketState) -> DeterministicDecision:
    ...
    def _update_performance_tracking(self, decision: DeterministicDecision) -> None:
        ...
        if len(self.performance_tracker[decision_key]) > 100:
            self.performance_tracker[decision_key] = self.performance_tracker[decision_key][-100:]
    ...
    def _score_momentum_strategy(self, market_state: MarketState) -> float:
        ...
        momentum_consistency = 1.0 - np.std(momentum_values) / (np.mean(np.abs(momentum_values)) + 1e-8)
    ...
    def _score_vault_strategy(self, market_state: MarketState) -> float:
        ...
        if market_state.unrealized_pnl < 0:
            uncertainty += min(abs(market_state.unrealized_pnl) / max(market_state.available_capital, 1), 0.4)
...
if __name__ == "__main__":
    ...
    print(f"   Top Strategy: {max(decision.strategy_weights.items(), key=lambda x: x[1])}") 