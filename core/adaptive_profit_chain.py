...
class APCFSystem:
    ...
    def initialize_system(self, price_array, volume_array):
        ...
        """
        assert len(price_array) == len(volume_array), "Mismatch in price/volume array lengths"
    ...
    def run_backtest(self, data, start_date, end_date, strategy_profile):
        ...
        """
        if data.index.min() > pd.to_datetime(start_date) or data.index.max() < pd.to_datetime(end_date):
            logging.warning("Backtest window exceeds available data.")
        
        logging.info(f"Backtest: {start_date} → {end_date} | Profile: {strategy_profile}")

        # Fix index assignment if necessary
        if 'index' in data.columns:
            data = data.set_index('index')  # Safe to overwrite as it doesn't modify in-place
    ...
    def __repr__(self):
        ...
        """
        return f"<APCFSystem | prices: {len(self.price_data)} | volumes: {len(self.volume_data)}>"
...
def main():
    ...
    try:
        ...
        data_dir = config.get("matrix_response_paths", {}).get("data_directory", "data")
        ...
        result = apcf.run_backtest(df, '2023-10-01', '2023-10-03', 'baseline_strategy')
        print(result)
...
# Or expose via core/__init__.py as from .adaptive_profit_chain import APCFSystem
