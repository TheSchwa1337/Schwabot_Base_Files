...
class RateLimiter:
    ...
    def can_make_request(self) -> bool:
        ...
        with self.lock:
            ...
            while (
                self.requests
                and current_time - self.requests[0] > self.time_window
            ):
    ...
    def get_ticker(self, symbol: str) -> MarketData:
        ...
        try:
            ...
            if cache_key in self.cache:
                ...
                if (
                    time.time() - cached_data["timestamp"] < 5
                ):  # 5 second cache
                    self.cache_hits += 1
                    return MarketData(
                        symbol=symbol,
    ...
    def get_order_book(self, symbol: str, level: int = 2) -> MarketData:
        ...
        try:
            ...
            if cache_key in self.cache:
                cached_data = self.cache[cache_key]
                if (
                    time.time() - cached_data["timestamp"] < 1
                ):  # 1 second cache
                    self.cache_hits += 1
                    return MarketData(
                        symbol=symbol,
    ...
    def place_order(self, order_request: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            order_response = OrderResponse(
                ...
                order_type=OrderType(
                    result.get("type", order_request.order_type.value)
                ),
...
class SimplifiedBTCIntegration:
    ...
    def _initialize_exchanges(self) -> None:
        ...
        try:
            ...
            if default_exchange in self.exchanges:
                ...
                self.safe_log(
                    "info", f"Set default exchange: {default_exchange}"
                )
            else:
                self.active_exchange = list(self.exchanges.values())[0]
                self.safe_log(
                    "info",
                    f"Set default exchange: {list(self.exchanges.keys())[0]}",
                )
    ...
    def add_exchange(
        ...
        try:
            if exchange_type == ExchangeType.COINBASE:
                ...
                self.safe_print(
                    f"⚠️ Exchange type {exchange_type} not yet supported"
                )
                return False
    ...
    def get_ticker(
        ...
        try:
            ...
            self.safe_print(
                f"🔍 Getting ticker for {symbol} on {exchange_name or 'active exchange'}"
            )
    ...
    def get_order_book(
        ...
        try:
            ...
            self.safe_print(
                f"🔍 Getting order book for {symbol} (level {level})"
            )
    ...
    def place_order(
        ...
        try:
            ...
            self.safe_print(
                f"💰 Placing {order_request.side.value} order for {order_request.symbol}"
            )
            ...
            if order_response.status == OrderStatus.FILLED:
                self.safe_print(
                    f"✅ Order filled successfully: {order_response.order_id}"
                )
            else:
                self.safe_print(
                    f"⏳ Order placed: {order_response.order_id} (Status: {order_response.status.value})"
                )
    ...
    def get_balances(
        ...
        try:
            ...
            self.safe_print(
                f"💼 Getting balances from {exchange_name or 'active exchange'}"
            )
    ...
    def _update_performance_metrics(self) -> None:
        ...
        try:
            with self.integration_lock:
                ...
                total_api_calls = sum(
                    exchange.api_calls for exchange in self.exchanges.values()
                )
                total_api_errors = sum(
                    exchange.api_errors for exchange in self.exchanges.values()
                )
                total_cache_hits = sum(
                    exchange.cache_hits for exchange in self.exchanges.values()
                )
                total_cache_misses = sum(
                    exchange.cache_misses
...
def main() -> None:
    ...
    try:
        ...
        btc_integration.safe_print(
            f"   Sandbox mode: {btc_integration.config.get('sandbox_mode', True)}"
        )
        btc_integration.safe_print(
            f"   Default exchange: {btc_integration.config.get('default_exchange', 'coinbase')}"
        )
        ...
        try:
            ...
            btc_integration.safe_print(
                "   ⚠️ Ticker test skipped (requires API credentials)"
            )
        except Exception as e:
        ...
        try:
            # This would require actual API credentials
            btc_integration.safe_print(
                "   ⚠️ Order book test skipped (requires API credentials)"
            )
        except Exception as e:
        ...
        try:
            # This would require actual API credentials
            btc_integration.safe_print(
                "   ⚠️ Order placement test skipped (requires API credentials)"
            )
        except Exception as e:
            btc_integration.safe_print(
                f"   ❌ Order placement test failed: {e}"
            )
        ...
        btc_integration.safe_print(
            f"   Successful orders: {summary.successful_orders}"
        )
        btc_integration.safe_print(
            f"   Failed orders: {summary.failed_orders}"
        )
        ...
        btc_integration.safe_print(
            "💡 Note: Full functionality requires API credentials"
        )
        ...
        )  # Create instance for safe printing
        btc_integration.safe_print(
            f"❌ Simplified BTC Integration test failed: {e}"
        )
        import traceback
