...
from .anomaly_filter_comprehensive import (
    ComprehensiveAnomalyFilter, SystemState, AnomalySignal, validate_system_safety
)
from .deterministic_value_engine import (
    DeterministicValueEngine, MarketState, DeterministicDecision, 
    PhaseMode, StrategyType, AssetType, calculate_trading_decision
)
...
class UnifiedTradingSystem:
        ...
        try:
            ...
            if self._should_run_code_enforcement():
                ...
                processing_result['actions_taken'].extend(
                    [f"Code enforcement: {action}" for action in enforcement_result.patterns_applied]
                )
            ...
            if not safe_to_execute:
                ...
                safe_to_execute, _, _ = validate_system_safety(self.unified_state.system_state)
                processing_result['safe_to_trade'] = safe_to_execute
            ...
            if safe_to_execute:
                ...
                if not risk_acceptable:
                    ...
                    processing_result['actions_taken'].append("Trade blocked due to risk limits")
            ...
            if self._should_run_health_check():
                ...
                if health_status == 'critical':
                    processing_result['safe_to_trade'] = False
                    processing_result['actions_taken'].append("Trading halted due to critical system health")
            ...
            if not final_validation['safe']:
                processing_result['safe_to_trade'] = False
                processing_result['actions_taken'].extend(final_validation['blocking_reasons'])
            
            processing_time = time.time() - start_time
            logger.debug(f"🔄 Market tick processed in {processing_time:.4f}s, safe: {processing_result['safe_to_trade']}")
            ...
            if anomaly.severity in ['high', 'critical']:
                ...
                if anomaly.anomaly_type.value in self.recovery_protocols:
                    try:
                        ...
                        logger.error(f"❌ Recovery protocol failed for {anomaly.anomaly_type.value}: {e}")
    ...
    def _validate_risk_limits(self, decision: DeterministicDecision) -> bool:
        ...
        if decision.position_size > self.config['max_position_risk']:
            logger.warning(f"⚠️ Position size {decision.position_size:.3f} exceeds limit {self.config['max_position_risk']}")
            return False
        
        # Check execution confidence minimum
        if decision.execution_confidence < 0.3:
            logger.warning(f"⚠️ Execution confidence {decision.execution_confidence:.3f} too low")
            return False
        
        # Check for emergency stop loss trigger
        current_pnl_pct = self.unified_state.market_state.unrealized_pnl / max(self.unified_state.market_state.available_capital, 1)
        ...
        if max_strategy_weight > 0.8:  # No single strategy > 80%
            logger.warning(f"⚠️ Strategy concentration too high: {max_strategy_weight:.1%}")
            return False
    ...
    def _perform_health_check(self) -> str:
        ...
        if self.unified_state.uptime_percentage < 95:
            health_issues.append(f"Low uptime: {self.unified_state.uptime_percentage:.1f}%")
        
        if len(self.unified_state.system_errors) > 10:
            health_issues.append(f"High error count: {len(self.unified_state.system_errors)}")
        
        # Check recent anomaly frequency
        recent_anomalies = [
            a for a in self.unified_state.anomaly_history 
            if a.detected_at > time.time() - 3600  # Last hour
        ]
        if len(recent_anomalies) > 20:
            health_issues.append(f"High anomaly frequency: {len(recent_anomalies)}/hour")
    ...
    def _update_performance_metrics(self) -> None:
        ...
        if self.unified_state.total_trades > 0:
            success_rate = self.unified_state.successful_trades / self.unified_state.total_trades
        else:
            success_rate = 0.0
        
        # Calculate recent uptime
        error_count_last_hour = len([
            error for error in self.unified_state.system_errors[-100:]  # Recent errors
            if time.time() - float(error.split(':')[0] if ':' in error else '0') < 3600
        ])
        
        uptime = max(0, 100 - error_count_last_hour * 2)  # Each error reduces uptime by 2%
    ...
    def _get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (
                self.unified_state.successful_trades / max(self.unified_state.total_trades, 1)
            ),
    ...
    def _final_safety_validation(self, processing_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        critical_anomalies = [
            a for a in self.unified_state.active_anomalies 
    ...
    def _serialize_decision(self, decision: DeterministicDecision) -> Dict[str, Any]:
        ...
        return {
            ...
            'asset_allocation': {k.value: v for k, v in decision.asset_allocation.items()},
            'strategy_weights': {k.value: v for k, v in decision.strategy_weights.items()},
...
def demonstrate_complete_system() -> None:
    ...
    scenarios = [
        ...
        },
        {
            ...
            'data': {
                ...
                'volatility': {'BTC': 0.08, 'ETH': 0.12, 'XRP': 0.15},  # High volatility
        ...
        },
        {
            ...
            'data': {
                'prices': {'BTC': 450000.0, 'ETH': 30000.0, 'XRP': 6.0},  # 10x price jump
                'volumes': {'BTC': 100, 'ETH': 80, 'XRP': 200},  # Low volume
                'volatility': {'BTC': 0.5, 'ETH': 0.8, 'XRP': 1.2},  # Extreme volatility
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        if result['decision'] and result['safe_to_trade']:
            ...
            print(f"   - Asset Allocation: USDC:{allocation['USDC']:.1%} "
