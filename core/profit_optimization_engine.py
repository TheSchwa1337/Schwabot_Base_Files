...
try:
    from schwabot.core.phase.phase_transition_monitor import PhaseTransitionMonitor, PhaseState
    from schwabot.core.phase.drift_phase_weighter import DriftPhaseWeighter, DriftType
    from schwabot.core.overlay.aleph_overlay_mapper import AlephOverlayMapper, OverlayType
    from hash_recollection.entropy_tracker import EntropyTracker, EntropyState
    from hash_recollection.pattern_utils import PatternUtils, PatternType
...
class ProfitOptimizationEngine:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.profit_threshold = self.config.get('profit_threshold', 0.005)  # 0.5% minimum
        ...
        logger.info(f"💰 Profit Optimization Engine initialized with {len(self.weights)} mathematical components")
    ...
    def optimize_profit(self, 
        ...
        try:
            ...
            confidence_score = self._calculate_confidence_score(
                hash_similarity, phase_alignment, entropy_score, 
                drift_weight, pattern_confidence
            )
    ...
    def _calculate_phase_alignment(self, price_history: List[float]) -> float:
        ...
        try:
            ...
            drift_weight = np.std(np.diff(price_history)) if len(price_history) > 1 else 0.0
            
            # Evaluate phase state
            phase_state = self.phase_monitor.evaluate_phase_state(entropy_trace, drift_weight)
    ...
    def _calculate_entropy_score(self, price_history: List[float]) -> float:
        ...
        try:
            ...
            # Convert entropy state to score (lower entropy = higher confidence)
            entropy_scores = {
    ...
    def _calculate_pattern_confidence(self, price_history: List[float]) -> float:
        ...
        try:
            ...
            if patterns:
                ...
                pattern_confidence = (pattern_scores.get(max_pattern.pattern_type, 0.5) * 
