...
best_ask = asks[0][0] if asks else float('in')
...
buy_price = buy_book.bids[0][0] if buy_book.bids else 0.0
                sell_price = sell_book.asks[0][0] if sell_book.asks else float('in')

if buy_price > 0 and sell_price < float('in'):
...
print("Market Summary for BTC/USDT:")
