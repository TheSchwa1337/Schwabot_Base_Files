    ...
    async def get_market_summary(self, symbol: str) -> Dict[str, Any]:
        ...
        for exchange_id in self.exchanges.keys():
            try:
                ...
                logger.error("Failed to get market data for %s:%s: %s", exchange_id, symbol, e)
        ...
        if summary['exchanges']:
            spreads = [data['order_book']['spread'] for data in summary['exchanges'].values()]
            mid_prices = [data['order_book']['mid_price'] for data in summary['exchanges'].values()]
...
async def demo_ccxt_integration():
    ...
    try:
        ...
        print(f"  Average Spread: {summary['overall_metrics'].get('avg_spread', 0):.6f}")
        print(f"  Price Volatility: {summary['overall_metrics'].get('price_volatility', 0):.4f}")
        ...
        for exchange_id, data in summary['exchanges'].items():
            ...
            print(f"  Profit Vector: {data['profit_vector']['wall_enhanced_profit']:.6f}")
        ...
        if summary['arbitrage_opportunities']:
            ...
            for i, opp in enumerate(summary['arbitrage_opportunities'], 1):
                ...
                print(f"     Spread: {opp['spread']:.4f}, Profit: {opp['profit_potential']:.4f}")
