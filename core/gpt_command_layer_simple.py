...
safe_safe_print()
    f"\\u26a0\\ufe0f Profile response update failed: {"}
        safe_format_error()
            e, 'profile_response_update'""
...
safe_safe_print()
    f"\\u1f4dd Logged: {command.command_id} - {'Success' if response.success else 'Failed'}"
...
safe_safe_print(f"\\u2705 Test completed - Command: {command_id}")
        safe_safe_print()
    f"\\u2705 Status: {"}
        'Success' if status and status.success else 'Failed'""
        safe_safe_print()
    f"\\u2705 System: {"}
        system_status['total_commands'] commands processed""
