...
def main():
    ...
    try:
        ...
        for op_name, op_stats in final_stats['operations'].items():
            print(f"\n{op_name}:")
            print(
                f"  CPU: {op_stats['cpu']['count']} ops, {op_stats['cpu']['profit']:.2f} profit, {op_stats['cpu']['avg_time']:.4f}s avg"
            )
            print(
                f"  GPU: {op_stats['gpu']['count']} ops, {op_stats['gpu']['profit']:.2f} profit, {op_stats['gpu']['avg_time']:.4f}s avg"
            )
            print(f"  Current recommendation: {op_stats['recommended']}")
