...
class EcoFractal(FractalBase):
    ...
    def eco_response(self, phi_fn: Callable[[float], float], theta_range: np.ndarray = np.linspace(-5, 5, 100)) -> float:
        """
    ...
    def update_memory(self, omega_n: float, delta_psi_n: float, phi_fn: Callable[[float], float], theta_range: np.ndarray = np.linspace(-5, 5, 100)) -> float:
        """
