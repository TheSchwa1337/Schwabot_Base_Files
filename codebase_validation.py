...
class CodebaseValidator:
    ...
    def check_file_for_stubs(self, file_path: Path) -> List[Dict[str, Any]]:
    ...
    def _check_gpu_cpu_implementation(self, content: str, lines: List[str]) -> List[Dict[str, Any]]:
        """Check for proper GPU/CPU implementation patterns."""
        issues = []
        
        # Check for GPU/CPU detection patterns
        gpu_cpu_detected = any(pattern.lower() in content.lower() for pattern in self.gpu_cpu_patterns)
        ...
        if gpu_cpu_detected:
            ...
            if "try:" in content and "except ImportError:" in content:
                ...
                for line_num, line in enumerate(lines, 1):
                    if any(pattern.lower() in line.lower() for pattern in ["cupy", "numba", "cuda"]):
    ...
    def _check_imports(self, content: str, lines: List[str], file_path: Path) -> List[Dict[str, Any]]:
        """Check for import issues."""
        issues = []
        ...
        try:
            ...
            for import_name in imports:
                ...
                if import_name.startswith("core.") and not self._check_module_exists(import_name):
                    issues.append({
                        "type": "missing_import", 
    ...
    def validate_codebase(self) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("Codebase validation passed! All checks completed successfully.")
        ...
        if report["detailed_issues"]:
            ...
            for file_issue in report["detailed_issues"]:
                ...
                for issue in file_issue["issues"]:
                    print(f"    [{issue['severity'].upper()}] Line {issue.get('line', 'N/A')}: {issue['message']}")
                    if 'content' in issue:
    ...
    def save_report(self, report: Dict[str, Any], filename: str = "codebase_validation_report.json"):
