    ...
    for test in test_cases:
        ...
        print(
            f"{
                test['size']:>4}    {
                test['type']:>8}  {
                processing_time:>7.4f}  " f"{
                    analysis['security_score']:>8.1f}  {
                        result.omega_state.recursion_depth:>7}  " f"{
                            result.beta_state.bayesian_entropy:>9.4f}  {
                                len(
                                    result.gamma_state.frequency_components):>11}")
...
async def demo_session_analytics():
    ...
    for agent, count in analytics['agent_usage'].items():
        avg_security = sum(r['security_score']
                           for r in encryption_results if r['agent'] == agent) / count
    ...
    for enc_type, results in type_stats.items():
        ...
        print(
            f"   {enc_type}: {
                len(results)} uses | Security: {
                avg_sec:.1f} | Avg Depth: {
                avg_depth:.1f}")
