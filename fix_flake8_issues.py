...
def fix_function_class_spacing(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if (re.match(r'^\s*(def|class)\s+\w+', line) and 
...
def remove_unused_imports(content: str, file_path: str) -> str:
    ...
    preserved_imports = {
        'typing', 'numpy', 'pandas', 'logging', 'time', 'json',
        'asyncio', 'threading', 'decimal', 'enum', 'dataclasses',
        'pathlib', 'os', 'sys', 're', 'hashlib', 'math', 'random'
    }
