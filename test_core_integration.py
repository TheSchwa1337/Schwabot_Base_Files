...
def test_core_integration_pipeline():
    ...
    try:
        ...
        for i, symbol in enumerate(brain_symbols):
            ...
            if 'brain_results' in results and i < len(results['brain_results']):
                ...
                profit = brain_result['profit_score'] / (brain_result['price'] * brain_result['volume'])
    ...
    try:
        from core.clean_unified_math import CleanUnifiedMathSystem, optimize_brain_profit, calculate_position_size
        ...
        if 'brain_results' in results:
            for brain_result in results['brain_results']:
                ...
                position_size = calculate_position_size(confidence, 100000, 0.1)  # $100k portfolio
        ...
        input_data = {
            'tensor': [[price, volume] for price, volume in
                      [(50000, 1200), (51000, 1100), (49000, 1300)]],
    ...
    try:
        ...
        if 'brain_results' in results and 'router_results' in results:
            ...
            print(f"✅ Brain → Symbolic: {brain_data['scenario']} → {router_data['symbol']}")
        ...
        if 'router_results' in results and 'math_results' in results:
            ...
            print(f"✅ Symbolic → Math: {router_data['symbol']} → {math_data['scenario']}")
        ...
        if all(results.get(key, False) for key in ['brain_engine', 'symbolic_router', 'math_system']):
            print("✅ Complete pipeline: Market Data → Brain → Symbolic → Math → Decision")
            data_flow_success = True
        else:
    ...
    try:
        if all(results.get(key, False) for key in ['brain_engine', 'symbolic_router', 'math_system']):
            ...
            print(f"   Vault Storage Success: {sum(1 for r in router_data if r['vault_stored'])}/{len(router_data)}")
            ...
            results['performance_metrics'] = {
                ...
                'vault_success_rate': sum(1 for r in router_data if r['vault_stored']) / len(router_data)
            }
