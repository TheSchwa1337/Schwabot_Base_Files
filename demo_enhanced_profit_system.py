...
class SimpleProfitOptimizer:
    ...
    def __init__(self):
        ...
        self.profit_threshold = 0.005  # 0.5% minimum
    ...
    def optimize_profit(self, btc_price: float, usdc_volume: float, 
        ...
        try:
            ...
            phase_alignment = self._calculate_phase_alignment(market_data.get('price_history', []))
            entropy_score = self._calculate_entropy_score(market_data.get('price_history', []))
            drift_weight = self._calculate_drift_weight(market_data.get('price_history', []))
            pattern_confidence = self._calculate_pattern_confidence(market_data.get('price_history', []))
    ...
    def _calculate_hash_similarity(self, btc_price: float, usdc_volume: float) -> float:
        ...
        try:
            ...
            # Simulate similarity calculation (in real system, this would compare with historical hashes)
    ...
    def _calculate_profit_potential(self, btc_price: float, usdc_volume: float,
                                   confidence_score: float, market_data: Dict[str, Any]) -> float:
    ...
    def _determine_trade_parameters(self, profit_potential: float, confidence_score: float,
    ...
    def _calculate_risk_score(self, profit_vector: ProfitVector, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            total_risk = (volatility_risk * 0.4 + position_risk * 0.3 + confidence_risk * 0.3)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if self.stats["total_optimizations"] > 0:
            success_rate = self.stats["profitable_decisions"] / self.stats["total_optimizations"]
...
def run_demo():
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        print(f"  Should Trade: {'✅' if result.should_trade else '❌'} {result.should_trade}")
