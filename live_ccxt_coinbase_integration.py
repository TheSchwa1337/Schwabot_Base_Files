...
class LiveCCXTCoinbaseIntegration:
    ...
    def initialize_exchange(self) -> None:
        ...
        try:
            ...
            if self.config.api_key:  # Only test if credentials provided
                ...
                self.logger.info(f"✅ Connected to Coinbase Pro ({'Sandbox' if self.config.sandbox else 'Live'})")
    ...
    async def execute_signal(self, signal: TradeSignal) -> bool:
        ...
        if not self.config.trading_enabled:
            self.logger.info(f"📝 PAPER TRADE: {signal.action} {self.config.symbol} (Confidence: {signal.confidence:.2f})")
            return True
    ...
    def log_performance_summary(self) -> None:
        ...
        self.logger.info(f"   Daily Trades: {self.daily_trade_count}/{self.config.max_daily_trades}")
