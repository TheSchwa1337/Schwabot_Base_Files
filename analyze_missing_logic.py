...
def analyze_common_error_patterns(errors_by_file: Dict[str, List[str]]) -> Dict[str, int]:
...
def main():
    ...
    important_analyses.sort(key=lambda x: (
        x['has_math_logic'], 
        x['e999_errors'] > 0,
        x['size_kb']
    ), reverse=True)
    ...
    for analysis in important_analyses[:20]:  # Top 20
        if analysis['e999_errors'] > 0 or analysis['has_stubs']:
            ...
            print(f"   Classes: {analysis['has_classes']}, Functions: {analysis['has_functions']}")
            
            if analysis['stub_indicators']:
                print(f"   Stub Indicators: {', '.join(analysis['stub_indicators'][:3])}")
