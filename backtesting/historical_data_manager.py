...
class HistoricalDataManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"HistoricalDataManager initialized for {start_date.date()} to {end_date.date()} at {interval_minutes} min interval.")
    ...
    async def generate_mock_data(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        while current_date <= self.end_date:
            ...
            high_price = max(open_price, close_price) * Decimal(str(1 + random.uniform(0, self.volatility_factor * 0.5)))
    ...
    async def get_historical_data(self, source: str = "mock", file_path: Optional[str] = None) -> AsyncGenerator[Dict[str, Any], None]:
        """
