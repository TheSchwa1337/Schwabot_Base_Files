...
class InternalBacktester:
    ...
    def __init__(
        ...
        self.ccxt_integration = CCXTIntegration(config={'exchanges': ['binance']}) # Simulate one exchange
    ...
    def _self_correct(self, idx: int) -> None:
        ...
        if len(self.price_buffer) < self.window:
            print(f"⚠️ Not enough data for matrix analysis at step {idx}. Need at least {self.window} samples, have {len(self.price_buffer)}.")
            return
        ...
        if analysis is None or analysis.get("stability_score") is None:
            print(f"❌ Matrix analysis failed or returned incomplete data at step {idx}.")
            return
    ...
    def analyze_micro_trends_and_apply_strategy_hooks(self, all_prices: List[float], tick_index: int):
        """
        ...
        for gran in decimal_granularities:
            ...
            if len(rounded_prices) < 2: # Need at least 2 points for std_dev and momentum
                continue
            ...
            if mathematical_state_analysis is None or mathematical_state_analysis.get("stability_score") is None:
                # print(f"❌ Micro-trend matrix analysis failed for granularity {gran}.")
                continue

            std_dev = mathematical_state_analysis.get("volatility", np.std(rounded_prices)) # Use volatility if present, else calculate
            
            # Simulate when we would consider switching strategies
            # Using a dynamic threshold based on granularity and price range
            price_range_factor = (max(rounded_prices) - min(rounded_prices)) / max(rounded_prices, 1e-9)
            dynamic_threshold = 0.0001 * (10**(8-gran)) * (1 + price_range_factor * 0.5)

            if std_dev < dynamic_threshold:
                # Simulate current market conditions for Ghost Core
                market_conditions = {
                    "volatility": std_dev,
                    "momentum": (rounded_prices[-1] - rounded_prices[0]) / rounded_prices[0] if rounded_prices[0] != 0 else 0.0,
