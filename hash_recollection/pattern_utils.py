...
class PatternUtils:
    ...
    def _detect_trend_patterns(self, price_data: List[float]) -> List[PatternMatch]:
        ...
        if trend.strength > self.config['trend_confidence_threshold']:
            pattern_type = PatternType.TREND_UP if trend.trend_direction == "up" else PatternType.TREND_DOWN
    ...
    def _detect_consolidation_patterns(self, price_data: List[float]) -> List[PatternMatch]:
