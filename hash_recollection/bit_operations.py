...
class BitOperations:
    ...
    def analyze_bit_phase(self, value: float) -> BitPhase:
        ...
        try:
            ...
            transition_probability = self._calculate_transition_probability(normalized_value)
        ...
        for pattern_length in range(2, min(8, len(bits) // 2)):
            for start in range(len(bits) - pattern_length * 2):
                ...
                if pattern == next_pattern:
                    ...
                    repeat_count = 1
                    for i in range(start + pattern_length * 2, len(bits) - pattern_length + 1, pattern_length):
