...
Tests the functionality of the PlotSignEngine class, including tensor decomposition,
...
def test_confidence_computation(engine, sample_tensor):
    ...
    confidence = engine._compute_confidence(latent_factors, 'directional_consensus')
...
def test_plot_sign_history(engine, sample_tensor):
    ...
    assert all(isinstance(sign, PlotSign) for signs in history for sign in signs)
