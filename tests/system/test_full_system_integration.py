    ...
    try:
        ...
        async def test_price_fetch():
            try:
                ...
                print(f"✓ Price data fetched: {price_data.get('symbol', 'N/A')} @ ${price_data.get('price', 0):.2f}")
                return True
        ...
        async def test_order_book():
            try:
                ...
                print(f"✓ Order book fetched: {len(order_book.get('bids', []))} bids, {len(order_book.get('asks', []))} asks")
                return True
            except Exception as e:
...
def test_hash_relay_system():
    ...
    try:
        ...
        def relay_callback(hash_str: str, data: Dict[str, Any]):
            ...
            print(f"✓ Relay received: {hash_str[:8]}... for {data.get('decision', 'unknown')}")
...
def test_trading_channel_simulation():
    ...
    try:
        ...
        async def simulate_trading_cycle():
            ...
            for i in range(5):
                # Fetch price data
                price_data = await bridge.fetch_price_data("BTC/USDC")
                market_data = {
                    "current_price": price_data.get('price', 62000.0 + i*100),
                    "rsi": 40.0 + i*5,
                    "volume_change": 0.1 + i*0.05,
                    "moving_average": price_data.get('price', 62000.0) * 0.99,
                    "previous_close": price_data.get('price', 62000.0) * 0.98,
                    "price_history": [62000.0 + j*100 for j in range(i+1)],
                    "volume_history": [100.0] * (i+1),
                    "phase_data": [0.5] * 4,
                    "volatility": 0.5,
                    "consensus_signal": "neutral",
}
                # Process and get decision
                engines.process_market_data(market_data, thermal_state="stable")
                await asyncio.sleep(0.2)  # Simulate time between cycles

            print(f"✓ Trading simulation completed: {len(trading_decisions)} decisions made")
...
def main():
    """Main test runner."""
    print("="*70)
    print("SCHWABOT FULL SYSTEM INTEGRATION TEST")
    print("="*70)

    tests = [
        ("API Bridge", test_api_bridge),
        ("Dualistic Thought Engines", test_dualistic_thought_engines),
        ("Hash Relay System", test_hash_relay_system),
        ("Integrated Pipeline", test_integrated_pipeline),
        ("Trading Channel Simulation", test_trading_channel_simulation)
    ]
    ...
    if passed_count == len(tests):
        ...
        print("\n⚠️ SOME SYSTEM INTEGRATION TESTS FAILED. PLEASE REVIEW LOGS. ⚠️")
