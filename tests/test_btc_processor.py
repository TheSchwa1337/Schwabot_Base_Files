...
class TestBTCDataProcessor(unittest.TestCase):
    ...
    async def test_websocket_connection(self, mock_connect):
        ...
        pipeline_task = asyncio.create_task(self.processor.start_processing_pipeline())
    ...
    def test_buffer_management(self):
        ...
        # Verify buffer size
        self.assertEqual(len(self.processor.data_buffer), self.config['buffer_size'])
    ...
    async def test_gpu_stream_management(self, mock_stream):
        ...
        with torch.cuda.stream(self.processor.streams[0]):
            tensor = torch.tensor([1.0, 2.0, 3.0], device=self.processor.device)
            result = tensor * 2
            
        self.assertTrue(torch.all(result == torch.tensor([2.0, 4.0, 6.0], device=self.processor.device)))
        
    async def test_error_handling(self):
        """Test error handling in processing pipeline"""
        # Simulate processing error
        with patch.object(self.processor, '_process_price_data', side_effect=Exception("Test error")):
            # Add data to queue
