...
class TestPatternUtils(unittest.TestCase):
    ...
    def test_entry_exit_detection(self):
        ...
        is_exit, reasons = self.pattern_utils.is_exit_phase(phase_state, pattern_analysis)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_modular_integration(self):
        ...
        for i in range(50):
            ...
            bit_pattern = bit_ops.calculate_42bit_float(entropy_state.price_entropy)
            ...
            pattern_match = pattern_utils.check_pattern_match(
                12345, phase_state, pattern_analysis, mock_hash_db, entropy_vector
            )
            
            # Verify we get valid results
            self.assertIsNotNone(pattern_match)
            self.assertIn(pattern_match.action, ['entry', 'exit', 'hold', 'wait'])
