...
Tests event-driven architecture, configuration management, and fractal integration.
"""
...
from core.gpu_flash_engine import GPUFlashEngine, FlashState, GPUFlashConfig
from core.bus_core import BusEvent, BusCore
...
class TestGPUFlashEngine(unittest.TestCase):
    ...
    def test_engine_initialization(self, mock_load_config):
        ...
        for event in expected_events:
            self.mock_bus.register_handler.assert_any_call(event, unittest.mock.ANY)
    ...
    def test_flash_permission_safe_case(self, mock_load_config):
        ...
        engine.math_core.classify_entropy_shell = MagicMock(return_value='stable')
        ...
        engine.math_core.compute_matrix_stability = MagicMock(return_value='matrix_safe')
    ...
    def test_flash_permission_critical_entropy(self, mock_load_config):
        ...
        engine = GPUFlashEngine(bus_core=self.mock_bus)
        engine.math_core.classify_entropy_shell = MagicMock(return_value='critical_bloom')
    ...
    def test_event_handling_entropy_update(self, mock_load_config):
        ...
        self.assertEqual(len(engine.entropy_cascade), initial_cascade_length + 1)
    ...
    def test_memory_trimming(self, mock_load_config):
        ...
        # Trigger trim
        event = BusEvent(type='entropy.update', data={'z_score': 999}, timestamp=time.time())
        engine._handle_entropy_update(event)
        
        # Should be trimmed to max_cascade_memory
        self.assertEqual(len(engine.entropy_cascade), 5)
        self.assertEqual(engine.entropy_cascade[-1], 999)  # Latest value preserved
