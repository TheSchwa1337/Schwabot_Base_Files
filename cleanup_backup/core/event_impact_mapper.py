...
# Determine event priority
            priority = self._calculate_event_priority(event_data, sentiment_score, relevance_score)
...
# Update performance metrics
            ...
            logger.debug(f"Processed event: {event_id}, priority: {priority}, sentiment: {sentiment_score:.3f}")
...
# Sort by priority (descending)
            active_vectors.sort(key = lambda x: self.active_events[x[0]].priority, reverse = True)
...
# Validate timestamp if present
        if 'timestamp' in event_data:
            ...
            current_time = time.time()
            if not (0 <= timestamp <= current_time + 3600):  # Allow 1 hour future
                return False
...
# Boost for specific event types
        ...
        if event_type in ['news_sentiment', 'market_event', 'regulatory_announcement']:
            relevance_score += 0.2
