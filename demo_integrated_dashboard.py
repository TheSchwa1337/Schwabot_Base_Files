...
from core.ghost_trigger_manager import create_ghost_trigger, get_trigger_performance, AnchorStatus, TriggerType
    ...
    def update_system_stats(self):
        ...
        try:
            ...
            self.current_stats.update({
                ...
                "system_health": self._determine_system_health(tick_stats, balance_stats),
                "alif_aleph_status": self._determine_alif_aleph_status(balance_stats)
            })
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            "system_overview": {
                ...
                "uptime": time.time() - self.data_history[0]["timestamp"] if self.data_history else 0,
                "total_ticks": self.current_stats["tick_stats"].get("total_ticks", 0),
                "success_rate": self.current_stats["tick_stats"].get("success_rate", 0)
            },
            "tick_management": {
                "current_mode": self.current_stats["tick_stats"].get("current_compression_mode", "LO_SYNC"),
                "valid_ticks": self.current_stats["tick_stats"].get("valid_ticks", 0),
                "hollow_ticks": self.current_stats["tick_stats"].get("hollow_ticks", 0),
                "compressed_ticks": self.current_stats["tick_stats"].get("compressed_ticks", 0),
                "success_rate": self.current_stats["tick_stats"].get("success_rate", 0)
            },
            "balance_loader": {
                "current_mode": self.current_stats["balance_stats"].get("current_mode", "balanced"),
                "alif_load": self.current_stats["balance_stats"].get("alif_load", 0),
                "aleph_load": self.current_stats["balance_stats"].get("aleph_load", 0),
                "compression_ratio": self.current_stats["balance_stats"].get("compression_ratio", 0),
                "balance_needed": self.current_stats["balance_stats"].get("balance_needed", False)
            },
            "ghost_triggers": {
                "total_triggers": self.current_stats["trigger_stats"].get("total_triggers", 0),
                "anchored_triggers": self.current_stats["trigger_stats"].get("anchored_triggers", 0),
                "unanchored_triggers": self.current_stats["trigger_stats"].get("unanchored_triggers", 0),
                "fallback_triggers": self.current_stats["trigger_stats"].get("fallback_triggers", 0),
                "total_profit": self.current_stats["trigger_stats"].get("total_profit", 0)
            },
            "performance_metrics": {
    ...
    while True:
        try:
            ...
            if tick_context:
                ...
                if tick_context.validated:
                    trigger = create_ghost_trigger(
                        ...
                        anchor_status=AnchorStatus.ANCHORED if tick_context.echo_strength > 0.6 else AnchorStatus.UNANCHORED,
                        confidence=tick_context.echo_strength,
                        trigger_type=TriggerType.REAL_BLOCK if tick_context.echo_strength > 0.6 else TriggerType.ALIF_ENTROPY,
            ...
            if tick_context and tick_context.tick_id % 10 == 0:
                logger.info(
                    f"📊 Dashboard updated - Tick {
                        tick_context.tick_id}, " f"Health: {
                        dashboard_data['system_overview']['health']}, " f"Status: {
                        dashboard_data['system_overview']['alif_aleph_status']}")
