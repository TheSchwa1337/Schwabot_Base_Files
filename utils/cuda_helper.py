...
def safe_tensor_contraction(A: np.ndarray, B: np.ndarray, axes: Optional[Tuple[int, ...]] = None) -> np.ndarray:
    """Safe tensor contraction with CUDA fallback."""
    return safe_cuda_operation(
        lambda: xp.tensordot(A, B, axes=axes),
...
def safe_eigenvalue_decomposition(A: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return safe_cuda_operation(
        lambda: (xp.linalg.eigvals(A), xp.linalg.eigh(A)[1]),
        lambda: linalg.eigh(A)
    )

def safe_matrix_inverse(A: np.ndarray) -> np.ndarray:
    """Safe matrix inverse with CUDA fallback."""
    return safe_cuda_operation(
        lambda: xp.linalg.inv(A),
        lambda: linalg.inv(A)
    )
