...
class SchwabotPriceBridge:
    ...
    def __init__(self, use_secure_coordinator: bool = True):
        ...
        if use_secure_coordinator:
            try:
                ...
                logger.warning(f"⚠️  Secure API Coordinator unavailable: {e}")
    ...
    async def get_coinmarketcap_price(self, symbol: str = "BTC") -> Optional[PriceData]:
    ...
    async def get_coingecko_price(self, symbol: str = "bitcoin") -> Optional[PriceData]:
...
async def get_secure_price(symbol: str = "BTC") -> Optional[PriceData]:
    """Global function to get secure price data."""
    return await price_bridge.get_price(symbol)


async def get_multiple_secure_prices(symbols: List[str]) -> Dict[str, PriceData]:
