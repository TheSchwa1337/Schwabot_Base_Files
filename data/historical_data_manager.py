...
from . import (
    HISTORICAL_DIR, LIVE_DIR, PREPROCESSED_DIR, CACHE_DIR,
    BTC_USDC_DIR, BTC_USDC_COMBINED, BTC_USDC_HASH_MEMORY, BTC_USDC_PRECISION_ANALYSIS
)
...
class HistoricalDataManager:
    ...
    def load_historical_data(self, force_refresh: bool = False) -> bool:
        ...
        try:
            ...
            if raw_data is None or len(raw_data) < self.config["min_data_points"]:
                logger.error(f"❌ Insufficient historical data: {len(raw_data) if raw_data is not None else 0} records")
                return False
            ...
            logger.info(f"✅ Successfully loaded and processed {self.total_records:,} historical records")
            return True
    ...
    def _preprocess_historical_data(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        ...
        processed_data['volatility'] = processed_data['price_change'].rolling(window=20).std()
    ...
    def get_historical_context(self, current_price: float, lookback_days: int = 30) -> Dict[str, Any]:
        ...
        if self.precision_analysis is not None:
            ...
            if not recent_precision.empty:
                precision_context = {
                    ...
                    'avg_standard_score': recent_precision['standard_profit_score'].mean(),
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hash_memory_records': len(self.hash_memory) if self.hash_memory is not None else 0,
            'precision_analysis_records': len(self.precision_analysis) if self.precision_analysis is not None else 0,
            'data_files': {
...
def create_historical_data_manager(config: Optional[Dict[str, Any]] = None) -> HistoricalDataManager:
    ...
    if not manager.load_historical_data():
        logger.warning("⚠️ No historical data loaded. Please add CSV files to data/historical/btc_usdc/")
