...
execution_time_hint: float = 0.5  # Fast thermal response
...
execution_time_hint: float = 0.1  # Very fast fallback
...
current_volatility = ()
                    event.metadata.get("volatility", 0.2)
                        if event.metadata:
else 0.2:
)
...
self.current_market_data["thermal_state"] += DEFAULT_INTERVAL
        ...
        self.current_market_data["thermal_state"] = max()
            0.0, self.current_market_data["thermal_state"] - 0.5
        )
