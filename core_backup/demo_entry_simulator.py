...
# Generate price
entry_price = base_price * (1 + np.random.normal(0, 0.1))
...
# Generate price
entry_price = base_price * (1 + np.random.normal(0, 0.5))

# Generate low entropy
entropy = np.random.uniform(0.5, 0.3)
...
fractal_factor = np.unified_math.sin(simulation_index * 0.1) * 0.1
    entry_price = base_price * (1 + fractal_factor + np.random.normal(0, 0.5))
...
tick_delta = np.random.normal(0, 0.2)
