...
class RightBrain:
    ...
    async def _analyze_market_conditions(self) -> None:
        ...
        try:
            ...
            base_price = 65000.0
            price_change = np.random.normal(0, 0.2)  # 2% daily volatility
            ...
            self.market_analysis = {}
                "current_price": current_price,
                    "volume_24h": np.random.uniform(20e9, 50e9),
                        ...
                        "moving_average": current_price * (1 + np.random.uniform(-0.1, 0.1)),
                        "sentiment_score": np.random.uniform(0.2, 0.8),
                        "volume_change": np.random.uniform(-0.3, 0.3),
                        "previous_close": base_price,
                        "price_history": [base_price * (1 + np.random.normal(0, 0.1)) for _ in range(50)],
