...
if __name__ == "__main__":
    ...
    in_zone_1 = is_in_site_zone(tick_drift=0.5, orderbook_imbalance=1.0, liquidity_depth=15000.0, compression=0.7)
    print(f"Is in Site Zone: {in_zone_1}")

    # Scenario 2: Tick Drift too low (should be out of zone)
    print("\nScenario 2: Low Tick Drift")
    in_zone_2 = is_in_site_zone(tick_drift=0.1, orderbook_imbalance=1.0, liquidity_depth=15000.0, compression=0.7)
    ...
    in_zone_3 = is_in_site_zone(tick_drift=0.5, orderbook_imbalance=1.5, liquidity_depth=15000.0, compression=0.7)
    print(f"Is in Site Zone: {in_zone_3}")

    # Scenario 4: Liquidity Depth too low (should be out of zone)
    print("\nScenario 4: Low Liquidity Depth")
    in_zone_4 = is_in_site_zone(tick_drift=0.5, orderbook_imbalance=1.0, liquidity_depth=10000.0, compression=0.7)
    ...
    in_zone_5 = is_in_site_zone(tick_drift=0.5, orderbook_imbalance=1.0, liquidity_depth=15000.0, compression=0.9)
    print(f"Is in Site Zone: {in_zone_5}")

    # Scenario 6: Edge case (just barely in zone)
    print("\nScenario 6: Edge Case (Just In)")
    in_zone_6 = is_in_site_zone(tick_drift=0.31, orderbook_imbalance=0.8, liquidity_depth=12001.0, compression=0.84)
