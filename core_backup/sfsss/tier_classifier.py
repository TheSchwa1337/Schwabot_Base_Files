...
class ProfitTierClassifier:
    ...
    def __init__()
        ...
        self.tier_boundaries = ()
            ...
            if tier_boundaries is not None
                ...
                0: 0.0,  # <= 0% return / Experimental/Unstable
                1: 0.5,  # 0.5% return / Low Profit
                2: 0.2,  # 2% return / Medium Profit
                3: 0.5,  # 5% return / High Profit
}
    ...
    def classify_profit_tier(self, trade_return_percentage: float) -> int:
        ...
        if scaled_return < 1.0:  # If scaled return is less than 1 (e.g., 0.1 * 100 = 0.1)
        ...
        try:
            ...
            if classified_tier >= 3 and trade_return_percentage >= self.tier_boundaries.get(3, 0.5):
                final_tier = 3
            elif classified_tier >= 2 and trade_return_percentage >= self.tier_boundaries.get(2, 0.2):
                final_tier = 2
            elif classified_tier >= 1 and trade_return_percentage >= self.tier_boundaries.get(1, 0.5):
                final_tier = 1
            else:
...
if __name__ == "__main__":
    ...
    test_returns = []
        -0.1,  # Loss
        0.1,  # 0.1% gain (should be Tier 0)
        0.4,  # 0.4% gain (should be Tier 0)
        0.5,  # 0.5% gain (boundary, should be Tier 1)
        0.1,  # 1% gain (Tier 1)
        0.19,  # 1.9% gain (Tier 1)
        0.2,  # 2% gain (boundary, should be Tier 2)
        0.35,  # 3.5% gain (Tier 2)
        0.49,  # 4.9% gain (Tier 2)
        0.5,  # 5% gain (boundary, should be Tier 3)
        0.75,  # 7.5% gain (Tier 3)
