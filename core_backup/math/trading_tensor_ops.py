...
try:
    ...
    logging.warning("SciPy not available. Some tensor operations will use fallbacks.")
...
class TradingTensorOps:
    ...
    def calculate_profit_surface(self, price_tensor: NDArray, volume_tensor: NDArray) -> NDArray:
        ...
        try:
            ...
            if SCIPY_AVAILABLE:
                ...
                profit_surface = np.mean(profit_surface) * np.ones_like(profit_surface)
    ...
    def calculate_profit_optimization_tensor()
        self, price_tensor: NDArray, cost_tensor: NDArray, risk_tensor: NDArray
    ) -> NDArray:
        ...
        try:
            ...
            price_resized = ()
                price_tensor[: min_shape[0]] if price_tensor.ndim == 1 else price_tensor[: min_shape[0], : min_shape[1]]
            )
            cost_resized = ()
                cost_tensor[: min_shape[0]] if cost_tensor.ndim == 1 else cost_tensor[: min_shape[0], : min_shape[1]]
            )
            risk_resized = ()
                risk_tensor[: min_shape[0]] if risk_tensor.ndim == 1 else risk_tensor[: min_shape[0], : min_shape[1]]
            )
    ...
    def calculate_phase_transition_tensor(self, market_phases: List[str], transition_matrix: NDArray) -> NDArray:
        ...
        try:
            ...
            # Map phases to numerical representation (example: 0, 1, 2, 3)
            phase_to_int = {phase: i for i, phase in enumerate(sorted(list(set(market_phases))))}
            numerical_phases = np.array([phase_to_int.get(phase, 0) for phase in market_phases])

            # Use numerical phases to select transitions from the matrix
            # For simplicity, we assume transition_matrix is square and matches number of unique phases
            num_unique_phases = len(set(market_phases))
            if transition_matrix.shape[0] != num_unique_phases:
                logger.warning("Transition matrix shape does not match number of unique phases. Using identity.")
                return np.eye(num_unique_phases)

            # Create a transition tensor based on the sequence
            phase_transition_tensor = np.zeros((len(market_phases) - 1, num_unique_phases, num_unique_phases))
            for i in range(len(market_phases) - 1):
                current_phase_idx = numerical_phases[i]
                next_phase_idx = numerical_phases[i + 1]
                # Populate a one-hot like representation of the transition
                transition_slice = np.zeros((num_unique_phases, num_unique_phases))
                transition_slice[current_phase_idx, next_phase_idx] = transition_matrix[]
                    current_phase_idx, next_phase_idx
]
                phase_transition_tensor[i] = transition_slice

            return phase_transition_tensor

        except Exception as e:
            logger.error(f"Phase transition tensor calculation failed: {e}")
            return np.eye(4)

    def calculate_sma(self, data: NDArray, window: int) -> NDArray:
        """"""
        Calculate Simple Moving Average (SMA).
        SMA_t = (1/N) * sum_{i=t-N+1 to t} p_i
        """"""
        if len(data) < window:
            return np.array([])
        return np.convolve(data, np.ones(window) / window, mode="valid")

    def calculate_ema(self, data: NDArray, span: int) -> NDArray:
        """"""
        ...
        ema[span - 1] = np.mean(data[:span])  # Initial SMA for EMA
        for i in range(span, len(data)):
            ema[i] = alpha * data[i] + (1 - alpha) * ema[i - 1]
        return ema[span - 1 :]

    def calculate_delta_price(self, price_data: NDArray) -> NDArray:
        """"""
        Calculate change in price (Delta P).
        Delta_p = p_t - p_{t-1}
        """"""
        if len(price_data) < 2:
            return np.array([])
    ...
    def calculate_historical_volatility(self, price_data: NDArray, window: int) -> NDArray:
        """"""
        Calculate historical volatility.
        volatility_t = sqrt((1/N) * sum_{i=1 to N} (p_i - mu)^2)
        """"""
        ...
        for i in range(window, len(price_data) + 1):
            ...
            volatility = np.sqrt(np.mean((window_data - mean) ** 2))
