...
if __name__ == "__main__":
    ...
    hourly_prices = pd.Series()
        np.cumsum(np.random.randn(100)) + 100, index=pd.date_range(start="2023-1-1", periods=100, freq="h")
    )
    daily_prices = pd.Series()
        np.cumsum(np.random.randn(50)) + 1000, index=pd.date_range(start="2023-1-1", periods=50, freq="d")
    )
