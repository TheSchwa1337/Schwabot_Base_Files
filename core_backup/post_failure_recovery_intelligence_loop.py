            ...
            pattern = FailurePattern()
                pattern_id = f"pattern_{len(patterns)}",
                    pattern_type = pattern_key,
                        ...
                        average_severity = float(unified_math.unified_math.mean())
                        group["severities"])) if group["severities"] else 0.0,
                            recovery_success_rate = float(unified_math.unified_math.mean())
                        group["success_rates"])) if group["success_rates"] else 0.0,
