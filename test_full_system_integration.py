...
def test_trading_channel_simulation():
    ...
    try:
        ...
        if success:
            print(f"✓ Trading channel simulation successful: {len(trading_decisions)} decisions tracked")
            for decision in trading_decisions:
                print(f"  - {decision['decision'].upper()}: {decision['confidence']:.2f} confidence")
            return True
        else:
