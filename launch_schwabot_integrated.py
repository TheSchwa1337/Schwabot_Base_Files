...
def check_dependencies():
    ...
    if missing_optional:
        logger.info("📝 Some optional features may not be available due to missing dependencies")
        logger.info("To install all dependencies, run: pip install numpy scipy psutil cryptography requests aiohttp")
...
def run_demo_without_ui(components):
    ...
    if components['crwm_mapper']:
        ...
        if signature:
            print(f"  Current temperature: {signature['current_conditions']['temperature']:.1f}°C")
